...
#include "bot.h"
#include "bot_util.h"
#include "da_bot.h"		// BOTPORT: Remove this CF dependency
...
const char* BotProfile::GetWeaponPreferenceAsString( int i ) const
{
	...
	return WeaponIDToAlias( (SDKWeaponID)m_weaponPreference[ i ] );
}
...
bool BotProfile::HasPrimaryPreference( void ) const
{
	...
	{
		if ( m_weaponPreference[i] != SDK_WEAPON_NONE )
			return true;
	}
 ...
 */
const BotProfile *BotProfileManager::GetRandomProfile( BotDifficultyType difficulty, int team, weapontype_t weaponType ) const
{
	...
	FOR_EACH_LL( m_profileList, it )
	{
		...
		if ( weaponType != WT_NONE )
		{
			...
			if ( weaponType != CWeaponSDKBase::GetWeaponType( (SDKWeaponID)profile->GetWeaponPreference( 0 ) ) )
				continue;
		}
