 ...
 */
inline float StayOnLadderLine( CDABot *me, const CNavLadder *ladder )
{
...
//--------------------------------------------------------------------------------------------------------------
void CDABot::ComputeLadderAngles( float *yaw, float *pitch )
{
	if ( !yaw || !pitch )
		return;
 ...
 */
void CDABot::UpdateLookAngles( void )
{
	VPROF_BUDGET( "CDABot::UpdateLookAngles", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CDABot::SetLookAt( const char *desc, const Vector &pos, PriorityType pri, float duration, bool clearIfClose, float angleTolerance, bool attack )
{
 ...
 */
void CDABot::InhibitLookAround( float duration )
{
 ...
 */
void CDABot::UpdatePeripheralVision()
{
	VPROF_BUDGET( "CDABot::UpdatePeripheralVision", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CDABot::UpdateLookAround( bool updateNow )
{
	VPROF_BUDGET( "CDABot::UpdateLookAround", VPROF_BUDGETGROUP_NPCS );
	...
	if (IsAtHidingSpot() || IsNotMoving( minStillTime ))
	{
		...
		for( int i=0; i<m_approachPointCount; ++i )
		{
			...
			if (TheDABots()->GetElapsedRoundTime() >= spotTime)
			{
	...
	if (m_spotEncounter)
	{
		...
		if (!IsSafe() && !IsLookingAtSpot( PRIORITY_LOW ))
		{
			...
			FOR_EACH_LL( m_spotEncounter->spotList, it )
			{
				...
				{
/*					if (TheDABots()->GetElapsedRoundTime() < spotOrder->spot->GetArea()->GetEarliestOccupyTime( OtherTeam( GetTeamNumber() ) ))
					{
						continue;
					}*/
				}
 ...
 */
bool CDABot::BendLineOfSight( const Vector &eye, const Vector &target, Vector *bend, float angleLimit ) const
{
	VPROF_BUDGET( "CDABot::BendLineOfSight", VPROF_BUDGETGROUP_NPCS );
 ...
 */
bool CDABot::IsNoticable( const CSDKPlayer *player, unsigned char visParts ) const
{
	...
	if (visParts & VIS_HEAD)
	{
		const float chance = 10.0f;
		coverRatio += chance;
	}

	if (visParts & VIS_LEFT_SIDE)
	{
		const float chance = 20.0f;
		coverRatio += chance;
	}

	if (visParts & VIS_RIGHT_SIDE)
	{
		const float chance = 20.0f;
		coverRatio += chance;
	}

	if (visParts & VIS_FEET)
	{
		const float chance = 10.0f;
		coverRatio += chance;
	}
 ...
 */
CSDKPlayer *CDABot::FindMostDangerousThreat( void )
{
	VPROF_BUDGET( "CDABot::FindMostDangerousThreat", VPROF_BUDGETGROUP_NPCS );
	...
	CSDKPlayer *currentThreat = m_enemyQueue[ prevIndex ].player;
	...
	{
		VPROF_BUDGET( "CDABot::Collect Threats", VPROF_BUDGETGROUP_NPCS );
		...
		for( i = 1; i <= gpGlobals->maxClients; ++i )
		{
			...
			if (player->InSameTeam( this ))
			{
				...
				if (IsVisible( entity->WorldSpaceCenter(), false, this ))
				{
					...
					m_watchInfo[idx].isEnemy = (SDKGameRules()->PlayerRelationship(this, player) == GR_ENEMY);
			...
			if (!IsVisible( player, true, &visParts ))
				continue;
			...
			if (IsSniperRifle( player->GetActiveSDKWeapon() ))
			{
			...
			{
				VPROF_BUDGET( "CDABot::Sort Threats", VPROF_BUDGETGROUP_NPCS );
	...
	{
		VPROF_BUDGET( "CDABot::Count nearby Friends & Enemies", VPROF_BUDGETGROUP_NPCS );
	...
	{
		VPROF_BUDGET( "CDABot::Track enemy Place", VPROF_BUDGETGROUP_NPCS );
		...
		for( i=0; i<threatCount; ++i )
		{
			...
			CNavArea *area;
			CDABot *bot = dynamic_cast<CDABot *>(threat[i].enemy);
			if (bot && bot->IsBot())
			{
			...
			{
				Vector enemyOrigin = threat[i].enemy->GetCentroid();
	...
	{
		VPROF_BUDGET( "CDABot::Select Threat", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CDABot::UpdateReactionQueue( void )
{
	VPROF_BUDGET( "CDABot::UpdateReactionQueue", VPROF_BUDGETGROUP_NPCS );
	...
	CSDKPlayer *threat = FindMostDangerousThreat();
 ...
 */
CSDKPlayer *CDABot::GetRecognizedEnemy( void )
{
 ...
 */
bool CDABot::IsRecognizedEnemyReloading( void )
{
	if (m_enemyQueueAttendIndex >= m_enemyQueueCount)
		return false;
 ...
 */
bool CDABot::IsRecognizedEnemyProtectedByShield( void )
{
	if (m_enemyQueueAttendIndex >= m_enemyQueueCount)
		return false;
 ...
 */
float CDABot::GetRangeToNearestRecognizedEnemy( void )
{
	const CSDKPlayer *enemy = GetRecognizedEnemy();
 ...
 */
void CDABot::Blind( float holdTime, float fadeTime, float startingAlpha )
{
...
{
public:
	CheckLookAt( const CDABot *me, bool testFOV )
	{
		m_me = me;
		m_testFOV = testFOV;
	}
	...
	bool operator() ( CBasePlayer *player )
	{
		...
		if (m_testFOV && !(const_cast< CDABot * >(m_me)->FInViewCone( player->WorldSpaceCenter() )))
			return true;
 ...
 */
bool CDABot::IsAnyVisibleEnemyLookingAtMe( bool testFOV ) const
{
 ...
 * Do panic behavior
 */
void CDABot::UpdatePanicLookAround( void )
{
