 ...
 */
CBaseEntity * CDABot::FindEntitiesOnPath( float distance, CPushAwayEnumerator *enumerator, bool checkStuck )
{
	Vector goal;
...
//--------------------------------------------------------------------------------------------------------------
void CDABot::PushawayTouch( CBaseEntity *pOther )
{
#if DEBUG_BREAKABLES
 ...
 * stopped by it.
 */
void CDABot::BreakablesCheck( void )
{
	if ( !HasPath() )
		return;
	...
	{
		if ( IsUsingGrenade() || ( !isNear && IsUsingPrimaryMelee() ) )
		{
 ...
 */
void CDABot::DoorCheck( void )
{
	if ( IsAttacking() && !IsUsingPrimaryMelee() )
	{
 ...
 */
void CDABot::ResetStuckMonitor( void )
{
	if (m_isStuck)
	{
 ...
 */
bool CDABot::DiscontinuityJump( float ground, bool onlyJumpDown, bool mustJump )
{
 ...
 */
bool CDABot::GetSimpleGroundHeightWithFloor( const Vector &pos, float *height, Vector *normal )
{
 ...
 */
Place CDABot::GetPlace( void ) const
{
	if (m_lastKnownArea)
 ...
 */
void CDABot::MoveTowardsPosition( const Vector &pos )
{
 ...
 */
void CDABot::MoveAwayFromPosition( const Vector &pos )
{
	// compute our current forward and lateral vectors
	float angle = EyeAngles().y;
 ...
 */
void CDABot::StrafeAwayFromPosition( const Vector &pos )
{
	// compute our current forward and lateral vectors
	float angle = EyeAngles().y;
 ...
 */
void CDABot::ComputeApproachPoints( void )
{
	...
	Vector eye = GetCentroid(); // + pev->view_ofs;	// eye position
...
//--------------------------------------------------------------------------------------------------------------
void CDABot::DrawApproachPoints( void ) const
{
	for( int i=0; i<m_approachPointCount; ++i )
	{
		if (TheDABots()->GetElapsedRoundTime() >= m_approachPoint[i].m_area->GetEarliestOccupyTime( OtherTeam( GetTeamNumber() ) ))
 ...
 */
bool CDABot::FindApproachPointNearestPath( Vector *pos )
{
	if (!HasPath())
		return false;
 ...
 */
bool CDABot::IsAtEnemySpawn( void ) const
{
	CBaseEntity *spot;
	const char *spawnName = "info_player_teamspawn";
	...
	{
		if (spot->GetTeamNumber() == GetTeamNumber())
			continue;
