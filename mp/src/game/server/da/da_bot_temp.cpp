...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t *pEdict, const char *playername )
	{
		...
		CDABot *pPlayer = static_cast<CDABot *>( CreateEntityByName( "da_bot" ) );
		if ( pPlayer )
		{
...
CBasePlayer *BotPutInServer( bool bFrozen )
{
	...
	DAWeaponID eWeapon = DA_WEAPON_NONE;
	
	while (eWeapon == DA_WEAPON_NONE || eWeapon == DA_WEAPON_BRAWL)
		eWeapon = (DAWeaponID)RandomInt(1, WEAPON_MAX-1);
...
void Bot_RunAll( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		{
			CDABot *pBot = dynamic_cast< CDABot* >( pPlayer );
			if ( pBot )
				Bot_Think( pBot );
		}
	}
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CDAPlayer *fakeclient, CUserCmd &cmd, float frametime )
{
	if ( !fakeclient )
		return;
...
void Bot_UpdateStrafing( CDABot *pBot, CUserCmd &cmd )
{
...
void Bot_UpdateDirection( CDABot *pBot )
{
...
void Bot_FlipOut( CDABot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleSendCmd( CDABot *pBot )
{
...
void Bot_ForceFireWeapon( CDABot *pBot, CUserCmd &cmd )
{
...
void Bot_SetForwardMovement( CDABot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleRespawn( CDABot *pBot, CUserCmd &cmd )
{
