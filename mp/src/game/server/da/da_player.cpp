...
// Tables.
// -------------------------------------------------------------------------------- //
BEGIN_DATADESC( CDAPlayer )
DEFINE_THINKFUNC( SDKPushawayThink ),
END_DATADESC()

LINK_ENTITY_TO_CLASS( player, CDAPlayer );
PRECACHE_REGISTER(player);

// CDAPlayerShared Data Tables
//=============================
...
BEGIN_SEND_TABLE_NOBASE( CDAPlayerShared, DT_DAPlayerShared )
	...
	SendPropBool (SENDINFO (m_bIsManteling)),
...
// main table
IMPLEMENT_SERVERCLASS_ST( CDAPlayer, DT_DAPlayer )
...
class CDARagdoll : public CBaseAnimatingOverlay
{
public:
	DECLARE_CLASS( CDARagdoll, CBaseAnimatingOverlay );
	DECLARE_SERVERCLASS();
...
LINK_ENTITY_TO_CLASS( da_ragdoll, CDARagdoll );

IMPLEMENT_SERVERCLASS_ST_NOBASE( CDARagdoll, DT_DARagdoll )
...
void CDARagdoll::Spawn()
{
	BaseClass::Spawn();

	SetThink(&CDARagdoll::DisappearThink);
	SetNextThink(gpGlobals->curtime + 15);
}

void CDARagdoll::DisappearThink()
{
	if (IsRagdollVisible())
	{
...
bool CDARagdoll::IsRagdollVisible()
{
...
void CDAPlayer::SetupVisibility( CBaseEntity *pViewEntity, unsigned char *pvs, int pvssize )
{
...
CDAPlayer::CDAPlayer()
{
	...
	m_PlayerAnimState = CreateDAPlayerAnimState( this );
...
CDAPlayer *CDAPlayer::CreatePlayer( const char *className, edict_t *ed )
{
	CDAPlayer::s_PlayerEdict = ed;
	return (CDAPlayer*)CreateEntityByName( className );
}

int CDAPlayer::UpdateTransmitState()
{
	if (DAGameRules()->GetBountyPlayer() == this)
		return SetTransmitState( FL_EDICT_ALWAYS );

	if (DAGameRules()->GetLeader() == this)
		return SetTransmitState( FL_EDICT_ALWAYS );

	if (DAGameRules()->GetFrontRunner1() == this)
		return SetTransmitState( FL_EDICT_ALWAYS );

	if (DAGameRules()->GetFrontRunner2() == this)
		return SetTransmitState( FL_EDICT_ALWAYS );

	return BaseClass::UpdateTransmitState();
}

void CDAPlayer::LeaveVehicle( const Vector &vecExitPoint, const QAngle &vecExitAngles )
{
...
bool CDAPlayer::FVisible(CBaseEntity *pEntity, int iTraceMask, CBaseEntity **ppBlocker)
{
	if (pEntity->IsPlayer())
		return IsVisible(dynamic_cast<CDAPlayer*>(pEntity));
	else
		return CBasePlayer::FVisible(pEntity, iTraceMask, ppBlocker);
}

bool CDAPlayer::IsVisible( const Vector &pos, bool testFOV, const CBaseEntity *ignore ) const
{
	// is it in my general viewcone?
	if (testFOV && !const_cast<CDAPlayer*>(this)->FInViewCone( pos ))
		return false;
	...
	UTIL_TraceLine( const_cast<CDAPlayer*>(this)->EyePosition(), pos, MASK_OPAQUE, &traceFilter, &result );
...
bool CDAPlayer::IsVisible(CDAPlayer *pPlayer, bool testFOV, unsigned char *visParts) const
{
	if (!pPlayer)
		return false;
	...
	if (testFOV && !(const_cast<CDAPlayer*>(this)->FInViewCone( pPlayer->WorldSpaceCenter() )))
	{
		return false;
	}
...
Vector CDAPlayer::GetPartPosition(CDAPlayer* player, VisiblePartType part) const
{
	...
	if (gpGlobals->framecount > info->m_validFrame)
	{
		...
		const_cast<CDAPlayer*>(this)->ComputePartPositions( player );
...
CDAPlayer::PartInfo CDAPlayer::m_partInfo[ MAX_PLAYERS ];

//--------------------------------------------------------------------------------------------------------------
/**
 * Compute part positions from bone location.
 */
void CDAPlayer::ComputePartPositions( CDAPlayer *player )
{
	const int headBox = 1;
	const int gutBox = 3;
...
Vector CDAPlayer::GetCentroid( ) const
{
...
CDAPlayer* CDAPlayer::FindClosestFriend(float flMaxDistance, bool bFOV)
{
	CDAPlayer* pClosest = NULL;
	for (int i = 1; i < gpGlobals->maxClients; i++)
	{
		CDAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(i));
		...
		if (DAGameRules()->PlayerRelationship(this, pPlayer) == GR_NOTTEAMMATE)
			continue;
...
inline bool CDAPlayer::IsReloading( void ) const
{
	CWeaponDABase *pPrimary = GetActiveDAWeapon();
...
void CDAPlayer::PostThink()
{
	...
	if (DAGameRules()->CoderHacks())
		m_nCoderHacksButtons = m_nButtons;
}

bool CDAPlayer::CanHearAndReadChatFrom( CBasePlayer *pPlayer )
{
...
void CDAPlayer::GiveDefaultItems()
{
	char szName[128];
	...
	if ( State_Get() == STATE_ACTIVE )
	{
		...
		CWeaponDABase* pHeaviestWeapon = NULL;

		for (int i = 1; i < MAX_LOADOUT; i++)
		{
			DAWeaponID eBuyWeapon = (DAWeaponID)i;
			...
			if (*pWeaponInfo->m_szSingle)
			{
				...
				DAWeaponID eSingleWeapon = AliasToWeaponID(pWeaponInfo->m_szSingle);
			...
			if (bBuy)
			{
				...
				CWeaponDABase* pWeapon = static_cast<CWeaponDABase*>(GiveNamedItem( szName ));

				if (!pHeaviestWeapon)
					pHeaviestWeapon = pWeapon;
				else if (pWeapon && pWeapon->GetWeaponID() != DA_WEAPON_GRENADE && pWeapon->GetSDKWpnData().iWeight > pHeaviestWeapon->GetSDKWpnData().iWeight)
					pHeaviestWeapon = pWeapon;
		...
		{
			CWeaponDABase* pWeapon = dynamic_cast<CWeaponDABase*>(GetWeapon(i));
			CWeaponDABase* pLastWeapon = dynamic_cast<CWeaponDABase*>(Weapon_GetLast());
			...
			if (pWeapon->GetWeaponID() == DA_WEAPON_BRAWL)
				continue;

			if (!pLastWeapon || pLastWeapon->GetWeaponID() == DA_WEAPON_BRAWL)
			{
...
void CDAPlayer::SDKPushawayThink(void)
{
...
//		GiveDefaultItems();
	...
	SetContextThink( &CDAPlayer::SDKPushawayThink, gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL, SDK_PUSHAWAY_THINK_CONTEXT );
	pl.deadflag = false;
	...
	DAGameRules()->CalculateSlowMoForPlayer(this);
...
bool CDAPlayer::SelectSpawnSpot( const char *pEntClassName, CBaseEntity* &pSpot )
{
	...
	DevMsg("CDAPlayer::SelectSpawnSpot: couldn't find valid spawn point.\n");

	return true;
}


CBaseEntity* CDAPlayer::EntSelectSpawnPoint()
{
...
void CDAPlayer::ChangeTeam( int iTeamNum )
{
	if ( !GetGlobalTeam( iTeamNum ) )
	{
		Warning( "CDAPlayer::ChangeTeam( %d ) - invalid team index.\n", iTeamNum );
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CDAPlayer::CommitSuicide( bool bExplode /* = false */, bool bForce /*= false*/ )
{
...
void CDAPlayer::InitialSpawn( void )
{
	m_flTotalStyle = 0;
...
void CDAPlayer::OnDive()
{
}

void CDAPlayer::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
int CDAPlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	CDAPlayer *pSDKAttacker = ToDAPlayer(info.GetAttacker());
	if (pSDKAttacker)
	{
		// Turn down easy bots if the player is the bounty, to make it a tad harder.
		if (DAGameRules()->GetBountyPlayer() != this && DAGameRules()->GetBountyPlayer() != pSDKAttacker)
		{
...
int CDAPlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	{
		CDAPlayer* pAttackerSDK = ToDAPlayer(pAttacker);
		...
		if ( DAGameRules()->PlayerRelationship(this, pAttackerSDK) != GR_TEAMMATE )
...
CWeaponDABase* CDAPlayer::FindAnyWeaponButBrawl()
{
	for (int i = 0; i < WeaponCount(); i++)
	{
		CWeaponDABase *pWeapon = (CWeaponDABase *)GetWeapon(i);
		if (!pWeapon)
			continue;
...
void CDAPlayer::Event_Killed( const CTakeDamageInfo &info )
{
	CTakeDamageInfo subinfo = info;
	subinfo.SetDamageForce( m_vecTotalBulletForce );

	m_hKiller = ToDAPlayer(info.GetAttacker());
	...
	if (GetActiveDAWeapon() && GetActiveDAWeapon()->GetWeaponID() == DA_WEAPON_GRENADE)
	{
		CWeaponGrenade* pGrenadeWeapon = static_cast<CWeaponGrenade*>(GetActiveDAWeapon());
	...
	{
		CDAPlayer* pSDKAttacker = ToDAPlayer(pAttacker);
	...
	DAGameRules()->PlayerSlowMoUpdate(this);
}

void CDAPlayer::OnDamagedByExplosion( const CTakeDamageInfo &info )
{
	UTIL_ScreenShake( info.GetInflictor()->GetAbsOrigin(), 10.0, 1.0, 0.5, 2000, SHAKE_START, false );
}

void CDAPlayer::SetKilledByString( string_t sKilledBy )
{
...
void CDAPlayer::AwardStylePoints(CDAPlayer* pVictim, bool bKilledVictim, const CTakeDamageInfo &info)
{
	...
	CWeaponDABase* pWeapon = dynamic_cast<CWeaponDABase*>(info.GetWeapon());
...
void CDAPlayer::SendAnnouncement(announcement_t eAnnouncement, style_point_t ePointStyle, float flPoints)
{
	CSingleUserRecipientFilter user( this );
	user.MakeReliable();
...
void CDAPlayer::SendNotice(notice_t eNotice)
{
...
void CDAPlayer::SendBroadcastNotice(notice_t eNotice, CDAPlayer* pSubject)
{
...
int CDAPlayer::GetMaxHealth() const
{
	if (IsStyleSkillActive(SKILL_RESILIENT))
...
bool CDAPlayer::Weapon_Switch( CBaseCombatWeapon *pWeapon, int viewmodelindex )
{
	if (GetCurrentTime() < m_flDisarmRedraw)
		return false;

	m_hSwitchFrom = GetActiveDAWeapon(); 
...
void CDAPlayer::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	m_flDisarmRedraw = -1;

	BaseClass::Weapon_Equip( pWeapon );
	dynamic_cast<CWeaponDABase*>(pWeapon)->SetDieThink( false );	//Make sure the context think for removing is gone!!
}

int CDAPlayer::FindCurrentWeaponsWeight()
{
	...
	for (int i = 0; i < WeaponCount(); i++)
	{
		...
		CWeaponDABase* pWeapon = GetSDKWeapon(i);
		...
		if (pWeapon->GetWeaponID() == DA_WEAPON_GRENADE)
		{
			...
			iWeaponsWeight += iGrenades * CSDKWeaponInfo::GetWeaponInfo(DA_WEAPON_GRENADE)->iWeight;
		}
		else
...
void CDAPlayer::DropWeaponsToPickUp(CWeaponDABase* pWeapon)
{
	...
	for (int i = 0; i < WeaponCount(); i++)
	{
		...
		CWeaponDABase* pThisWeapon = GetSDKWeapon(i);
	...
	if (m_Shared.m_iStyleSkill == SKILL_TROLL && aiWeapons[DA_WEAPON_GRENADE])
		aiWeapons[DA_WEAPON_GRENADE] -= 1;

	if (bDebug)
	{
		DevMsg("Weapon count:\n");
		for (int i = 0; i < aiWeapons.Count(); i++)
			DevMsg(" %s: %d\n", WeaponIDToAlias((DAWeaponID)i), aiWeapons[i]);
	...
	while (pWeapon->GetWeight() + iNewWeaponsWeight > MAX_LOADOUT_WEIGHT)
	{
		...
		for (int i = 0; i < aiWeapons.Count(); i++)
		{
			...
			CSDKWeaponInfo* pLightestWeaponInfo = CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)iLightestWeapon);
			CSDKWeaponInfo* pWeaponInfo = CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)i);
			...
			if (pWeaponInfo->iWeight == pLightestWeaponInfo->iWeight)
			{
				...
				if (pWeapon->GetWeaponID() != DA_WEAPON_GRENADE && i == DA_WEAPON_GRENADE)
				{
					iLightestWeapon = i;
					continue;
				}
		...
		iNewWeaponsWeight -= CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)iLightestWeapon)->iWeight;

		if (bDebug)
			DevMsg("Tossing %s, new weight %d\n", WeaponIDToAlias((DAWeaponID)iLightestWeapon), iNewWeaponsWeight);
	}
	...
	for (int i = 0; i < aiWeapons.Count(); i++)
	{
		...
		if (pWeapon->GetWeight() + iNewWeaponsWeight + pWeaponInfo->iWeight <= MAX_LOADOUT_WEIGHT)
		{
			...
			if (bDebug)
				DevMsg("Restoring %s, new weight %d\n", WeaponIDToAlias((DAWeaponID)i), iNewWeaponsWeight);
		}
	}
	...
	for (int i = 0; i < aiTossWeapons.Count(); i++)
	{
		for (int j = 0; j < aiTossWeapons[i]; j++)
		{
			...
			SDKThrowWeapon(FindWeapon((DAWeaponID)i), vecForward, gunAngles, flDiameter);
		}
	}
...
void CDAPlayer::SDKThrowWeapon( CWeaponDABase *pWeapon, const Vector &vecForward, const QAngle &vecAngles, float flDiameter  )
{
	Assert(pWeapon);
	if (!pWeapon)
		return;
	...
	if (pWeapon->HasAkimbo())
	{
		...
		{
			// Toss out a single.
			CWeaponDABase *pThrow = (CWeaponDABase*)Weapon_Create(pWeapon->GetName());
			pThrow->VPhysicsDestroyObject(); 
			pThrow->VPhysicsInitShadow(true, true);
	...
	if (pWeapon->GetWeaponID() == DA_WEAPON_GRENADE && GetAmmoCount(GetAmmoDef()->Index("grenades")) > 1)
	{
...
void CDAPlayer::SDKThrowWeaponDir( CWeaponDABase *pWeapon, const Vector &vecForward, Vector *pVecThrowDir )
{
	VMatrix zRot;
...
void CDAPlayer::PlayerDeathThink()
{
	//overridden, do nothing - our states handle this now
}
void CDAPlayer::CreateRagdollEntity()
{
	// If we already have a ragdoll, don't make another one.
	CDARagdoll *pRagdoll = dynamic_cast< CDARagdoll* >( m_hRagdoll.Get() );
	...
	{
		// create a new one
		pRagdoll = dynamic_cast< CDARagdoll* >( CreateEntityByName( "da_ragdoll" ) );
	}
...
//-----------------------------------------------------------------------------
void CDAPlayer::DestroyRagdoll( void )
{
	CDARagdoll *pRagdoll = dynamic_cast<CDARagdoll*>( m_hRagdoll.Get() );	
	if( pRagdoll )
	{
...
void CDAPlayer::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	m_PlayerAnimState->DoAnimationEvent( event, nData );
	TE_PlayerAnimEvent( this, event, nData );	// Send to any clients who can see this guy.
}

CWeaponDABase* CDAPlayer::GetActiveDAWeapon() const
{
	Assert(!GetActiveWeapon() || dynamic_cast<CWeaponDABase*>(GetActiveWeapon()));
	return static_cast< CWeaponDABase* >( GetActiveWeapon() );
}

CWeaponDABase* CDAPlayer::GetSDKWeapon(int i) const
{
	Assert(!GetWeapon(i) || dynamic_cast<CWeaponDABase*>(GetWeapon(i)));
	return static_cast< CWeaponDABase* >( GetWeapon(i) );
}

void CDAPlayer::CreateViewModel( int index /*=0*/ )
{
...
void CDAPlayer::ImpulseCommands()
{
...
void CDAPlayer::CheatImpulseCommands( int iImpulse )
{
...
void CDAPlayer::Instructor_LessonLearned(const char* pszLesson)
{
	if (gpGlobals->eLoadType == MapLoad_Background)
		return;
...
void CDAPlayer::FlashlightTurnOn( void )
{
	AddEffects( EF_DIMLIGHT );
}

void CDAPlayer::FlashlightTurnOff( void )
{
	RemoveEffects( EF_DIMLIGHT );
}

int CDAPlayer::FlashlightIsOn( void )
{
	return IsEffectActive( EF_DIMLIGHT );
}

bool CDAPlayer::ClientCommand( const CCommand &args )
{
	...
	{
#if defined ( SDK_USE_PLAYERCLASSES )
		CDATeam *pTeam = GetGlobalDATeam( GetTeamNumber() );
...
bool CDAPlayer::HandleCommand_JoinTeam( int team )
{
...
bool CDAPlayer::HandleCommand_JoinClass( int iClass )
{
	...
	if( !DAGameRules()->IsPlayerClassOnTeam( iClass, GetTeamNumber() ) )
		return false;

	const char *classname = DAGameRules()->GetPlayerClassName( iClass, GetTeamNumber() );

	if( DAGameRules()->CanPlayerJoinClass( this, iClass ) )
	{
...
void CDAPlayer::ShowClassSelectMenu()
{
#if defined ( SDK_USE_TEAMS )
...
void CDAPlayer::SetClassMenuOpen( bool bOpen )
{
	m_bIsClassMenuOpen = bOpen;
}

bool CDAPlayer::IsClassMenuOpen( void )
{
	return m_bIsClassMenuOpen;
}
#endif // SDK_USE_PLAYERCLASSES

void CDAPlayer::ShowFolderPanel( const char* pszName )
{
	CSingleUserRecipientFilter filter( this );
	filter.MakeReliable();
...
#ifdef SDK_USE_TEAMS
void CDAPlayer::SetTeamMenuOpen( bool bOpen )
{
	m_bIsTeamMenuOpen = bOpen;
}

bool CDAPlayer::IsTeamMenuOpen( void )
{
	return m_bIsTeamMenuOpen;
}

void CDAPlayer::ShowTeamMenu()
{
	ShowFolderPanel( PANEL_TEAM );
}
#endif

void CDAPlayer::ShowCharacterMenu()
{
	ShowFolderPanel( PANEL_CLASS );
}

void CDAPlayer::SetBuyMenuOpen( bool bOpen )
{
	m_bIsBuyMenuOpen = bOpen;
}

bool CDAPlayer::IsBuyMenuOpen( void )
{
	return m_bIsBuyMenuOpen;
}

void CDAPlayer::ShowBuyMenu()
{
	ShowFolderPanel( PANEL_BUY );
}

void CDAPlayer::ClearLoadout()
{
...
void CDAPlayer::AddToLoadout(DAWeaponID eWeapon)
{
...
void CDAPlayer::RemoveFromLoadout(DAWeaponID eWeapon)
{
...
void CDAPlayer::CountLoadoutWeight()
{
	m_iLoadoutWeight = 0;
...
void CDAPlayer::BuyRandom()
{
	ClearLoadout();

	// Buy a primary weapon
	DAWeaponID eWeapon = WEAPON_NONE;
	do
	{
...
void CDAPlayer::PickRandomSkill()
{
	SetStyleSkill((SkillID)random->RandomInt(SKILL_NONE+1, SKILL_LAST_CHOOSEABLE));
}

void CDAPlayer::SelectItem(const char *pstr, int iSubType)
{
	bool bPreviousWeaponWasOutOfAmmo = false;
	if (GetActiveDAWeapon() && !GetActiveDAWeapon()->HasPrimaryAmmo())
	...
	if (bPreviousWeaponWasOutOfAmmo && GetActiveDAWeapon() && GetActiveDAWeapon()->HasPrimaryAmmo())
		Instructor_LessonLearned("outofammo");
	else
		Instructor_LessonLearned("switchweapons");
}

void CDAPlayer::ShowSkillMenu()
{
	ShowFolderPanel( PANEL_BUY_EQUIP_CT );
}

void CDAPlayer::SetStyleSkill(SkillID eSkill)
{
...
#if defined ( SDK_USE_SPRINTING )
void CDAPlayer::InitSprinting( void )
{
...
void CDAPlayer::State_Transition( DAPlayerState newState )
{
	State_Leave();
...
void CDAPlayer::State_Enter( DAPlayerState newState )
{
...
CDAPlayerStateInfo* CDAPlayer::State_LookupInfo( DAPlayerState state )
{
	// This table MUST match the 
	static CDAPlayerStateInfo playerStateInfos[] =
	{
		{ STATE_ACTIVE,			"STATE_ACTIVE",			&CDAPlayer::State_Enter_ACTIVE, &CDAPlayer::State_Leave_ACTIVE, &CDAPlayer::State_PreThink_ACTIVE },
		{ STATE_WELCOME,		"STATE_WELCOME",		&CDAPlayer::State_Enter_WELCOME, NULL, &CDAPlayer::State_PreThink_WELCOME },
		{ STATE_MAPINFO,        "STATE_MAPINFO",        &CDAPlayer::State_Enter_MAPINFO, NULL, &CDAPlayer::State_PreThink_MAPINFO },
#if defined ( SDK_USE_TEAMS )
		{ STATE_PICKINGTEAM,	"STATE_PICKINGTEAM",	&CDAPlayer::State_Enter_PICKINGTEAM, NULL,	&CDAPlayer::State_PreThink_WELCOME },
#endif
#if defined ( SDK_USE_PLAYERCLASSES )
		{ STATE_PICKINGCLASS,	"STATE_PICKINGCLASS",	&CDAPlayer::State_Enter_PICKINGCLASS, NULL,	&CDAPlayer::State_PreThink_WELCOME },
#endif
		{ STATE_PICKINGCHARACTER, "STATE_PICKINGCHARACTER",	&CDAPlayer::State_Enter_PICKINGCHARACTER, NULL, &CDAPlayer::State_PreThink_WELCOME },
		{ STATE_BUYINGWEAPONS,	"STATE_BUYINGWEAPONS",	&CDAPlayer::State_Enter_BUYINGWEAPONS, NULL, &CDAPlayer::State_PreThink_WELCOME },
		{ STATE_PICKINGSKILL,   "STATE_PICKINGSKILL",	&CDAPlayer::State_Enter_PICKINGSKILL, NULL, &CDAPlayer::State_PreThink_WELCOME },
		{ STATE_DEATH_ANIM,		"STATE_DEATH_ANIM",		&CDAPlayer::State_Enter_DEATH_ANIM,	NULL, &CDAPlayer::State_PreThink_DEATH_ANIM },
		{ STATE_OBSERVER_MODE,	"STATE_OBSERVER_MODE",	&CDAPlayer::State_Enter_OBSERVER_MODE,	NULL, &CDAPlayer::State_PreThink_OBSERVER_MODE }
	};
...
void CDAPlayer::PhysObjectSleep()
{
...
void CDAPlayer::State_Enter_WELCOME()
{
...
void CDAPlayer::State_Enter_MAPINFO()
{
	// Important to set MOVETYPE_NONE or our physics object will fall while we're sitting at one of the intro cameras.
	SetMoveType( MOVETYPE_NONE );
	AddSolidFlags( FSOLID_NOT_SOLID );
...
void CDAPlayer::MoveToNextIntroCamera()
{
...
void CDAPlayer::State_PreThink_WELCOME()
{
...
void CDAPlayer::State_PreThink_MAPINFO()
{
	// Update whatever intro camera it's at.
	if( m_pIntroCamera && (gpGlobals->curtime >= m_fIntroCamTime) )
	{
...
void CDAPlayer::State_Enter_DEATH_ANIM()
{
	if ( HasWeapons() )
	{
...
void CDAPlayer::State_PreThink_DEATH_ANIM()
{
...
void CDAPlayer::State_Enter_OBSERVER_MODE()
{
	m_nRenderFX = kRenderNormal;
...
void CDAPlayer::State_PreThink_OBSERVER_MODE()
{
	if ((m_nButtons & IN_ATTACK) && !(m_afButtonLast & IN_ATTACK))
		State_Transition( STATE_ACTIVE );
...
#if defined ( SDK_USE_PLAYERCLASSES )
void CDAPlayer::State_Enter_PICKINGCLASS()
{
	ShowClassSelectMenu();
	PhysObjectSleep();
...
#if defined ( SDK_USE_TEAMS )
void CDAPlayer::State_Enter_PICKINGTEAM()
{
...
void CDAPlayer::State_Enter_PICKINGCHARACTER()
{
	StopObserverMode();
	ShowCharacterMenu();
	PhysObjectSleep();
...
void CDAPlayer::State_Enter_BUYINGWEAPONS()
{
	StopObserverMode();
	ShowBuyMenu();
	PhysObjectSleep();
...
void CDAPlayer::State_Enter_PICKINGSKILL()
{
	StopObserverMode();
	ShowSkillMenu();
	PhysObjectSleep();
...
void CDAPlayer::State_Enter_ACTIVE()
{
	SetMoveType( MOVETYPE_WALK );
	RemoveSolidFlags( FSOLID_NOT_SOLID );
...
void CDAPlayer::State_Leave_ACTIVE()
{
	DropBriefcase();
}

void CDAPlayer::State_PreThink_ACTIVE()
{
...
void CDAPlayer::NoteWeaponFired( void )
{
...
bool CDAPlayer::WantsLagCompensationOnEntity( const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	...
	if ( DAGameRules()->PlayerRelationship(const_cast<CDAPlayer*>(this), const_cast<CBasePlayer*>(pPlayer)) == GR_TEAMMATE )
		return false;
...
bool CDAPlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	CWeaponDABase* pSDKWeapon = NULL;
	if (pWeapon)
		pSDKWeapon = dynamic_cast<CWeaponDABase*>(pWeapon);

	if (m_Shared.m_iStyleSkill == SKILL_TROLL && pSDKWeapon && pSDKWeapon->GetWeaponID() == DA_WEAPON_GRENADE)
...
void CDAPlayer::Disarm()
{
	CWeaponDABase* pActiveWeapon = GetActiveDAWeapon();
	if (!pActiveWeapon)
		return;

	if (pActiveWeapon->GetWeaponID() == DA_WEAPON_BRAWL)
		return;
...
CBaseEntity	*CDAPlayer::GiveNamedItem( const char *pszName, int iSubType )
{
	CBaseEntity* pEnt;
...
void CDAPlayer::AddStylePoints(float points, style_sound_t eStyle, announcement_t eAnnouncement, style_point_t ePointStyle)
{
	if (DAGameRules()->GetBountyPlayer() == this)
		TakeHealth(points, 0);
...
void CDAPlayer::SetStylePoints(float flPoints)
{
	if (flPoints < 0)
	{
		m_flStylePoints = 0;
...
bool CDAPlayer::UseStylePoints (void)
{
	if (m_flStylePoints >= da_stylemeteractivationcost.GetFloat())
	{
		m_flStylePoints -= da_stylemeteractivationcost.GetFloat();
...
void CDAPlayer::ActivateMeter()
{
	...
	if (m_Shared.m_bSuperSkill || DAGameRules()->GetBountyPlayer() == this)
		return;
	...
	if (m_Shared.m_iStyleSkill == SKILL_MARKSMAN)
	{
		for (int i = 0; i < WeaponCount(); i++)
		{
			...
			CWeaponDABase* pSDKWeapon = static_cast<CWeaponDABase*>(pWeapon);
			CSDKWeaponInfo* pInfo = CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)pSDKWeapon->GetWeaponID());
...
void CDAPlayer::GiveSlowMo(float flSeconds)
{
	float flMaxSlow = 5;
...
void CDAPlayer::PickUpBriefcase(CBriefcase* pBriefcase)
{
	DropBriefcase();
...
void CDAPlayer::SendBroadcastSound(const char* pszName)
{
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		CDAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(i));
...
bool CDAPlayer::CanDoCoderHacks()
{
...
void CDAPlayer::CoderHacks(bool bOn)
{
	if (!CanDoCoderHacks())
		return;
...
bool CDAPlayer::SetCharacter(const char* pszCharacter)
{
	for (int i = 0; ; i++)
	{
		if (pszPossiblePlayerModels[i] == NULL)
...
void CDAPlayer::SetVCollisionState( const Vector &vecAbsOrigin, const Vector &vecAbsVelocity, int collisionState )
{
...
void CDAPlayer::PostThinkVPhysics (void)
{
	int collisionState;
...
void CDAPlayer::SetupVPhysicsShadow( const Vector &vecAbsOrigin, const Vector &vecAbsVelocity, CPhysCollide *pStandModel, const char *pStandHullName, CPhysCollide *pCrouchModel, const char *pCrouchHullName )
{
	solid_t solid;
...
void CDAPlayer::VPhysicsDestroyObject ()
{
	IPhysicsObject *hulls[] =
	{/*TODO: Just put these all into a member table?*/
		m_pShadowStand,
...
float CDAPlayer::GetUserInfoFloat(const char* pszCVar, float flBotDefault)
{
	if (IsBot())
		return flBotDefault;
...
int CDAPlayer::GetUserInfoInt(const char* pszCVar, int iBotDefault)
{
	if (IsBot())
		return iBotDefault;
...
float CDAPlayer::GetDKRatio(float flMin, float flMax, bool bDampen) const
{
...
void CC_Buy(const CCommand& args)
{
	...
	if (args.ArgC() == 3 && Q_strncmp(args[1], "remove", 6) == 0)
	{
		...
		if (eWeapon == WEAPON_NONE)
			eWeapon = (DAWeaponID)atoi(args[2]);
		pPlayer->StopObserverMode();
	...
	if (eWeapon)
	{
		...
		if (!pPlayer->IsBot())
		{
			...
			{
				DAWeaponID eAkimboInfo = AliasToWeaponID(pInfo->m_szAkimbo);
...
void CC_CoderHacks_f(const CCommand &args)
{
	...
	else if (FStrEq(args[2], "changelevel") && pPlayer->CanDoCoderHacks() && args.ArgC() >= 4)
	{
		...
		DAGameRules()->ChangeLevel();
	}
	else if (FStrEq(args[2], "list") && pPlayer->CanDoCoderHacks())
	{
		for (int i = 1; i < gpGlobals->maxClients; i++)
		{
			CDAPlayer* pDAPlayer = ToDAPlayer(UTIL_PlayerByIndex(i));

			if (!pDAPlayer)
				continue;

			CSteamID ID;
			pDAPlayer->GetSteamID(&ID);

			if (ID.IsValid())
				UTIL_SayText(UTIL_VarArgs("%s (%d)\n", pDAPlayer->GetPlayerName(), ID.GetAccountID()), pDAPlayer);
			else
				UTIL_SayText(UTIL_VarArgs("%s (INVALID)\n", pDAPlayer->GetPlayerName()), pDAPlayer);
		}
	}
...
CON_COMMAND( give_weapon, "Give weapon to player.\n\tArguments: <weapon_name>" )
{
		...
		&& args.ArgC() >= 2 )
	{
		...
		{
			CWeaponDABase* pDrop = (CWeaponDABase*)pPlayer->Weapon_Create(item_to_give);
