...
class CDAPlayerStateInfo
{
public:
	DAPlayerState m_iPlayerState;
	const char *m_pStateName;
	
	void (CDAPlayer::*pfnEnterState)();	// Init and deinit the state.
	void (CDAPlayer::*pfnLeaveState)();
	void (CDAPlayer::*pfnPreThink)();	// Do a PreThink() in this state.
};
...
// >> SDK Game player
//=============================================================================
class CDAPlayer : public CBaseMultiplayerPlayer
{
public:
	DECLARE_CLASS( CDAPlayer, CBaseMultiplayerPlayer );
	DECLARE_SERVERCLASS();
	DECLARE_PREDICTABLE();
	DECLARE_DATADESC();
	...
	static CDAPlayer *CreatePlayer( const char *className, edict_t *ed );
	static CDAPlayer* Instance( int iEnt );
	...
	virtual Vector      GetPartPosition(CDAPlayer* player, VisiblePartType part) const;	///< return world space position of given part on player
	virtual void        ComputePartPositions(CDAPlayer *player);					///< compute part positions from bone location
	virtual Vector      GetCentroid() const;
	virtual CDAPlayer* FindClosestFriend(float flMaxDistance, bool bFOV = true);
	virtual bool        RunMimicCommand( CUserCmd& cmd );
	virtual bool        IsReloading( void ) const;

	void         AwardStylePoints(CDAPlayer* pVictim, bool bKilledVictim, const CTakeDamageInfo &info);
	...
	static void  SendBroadcastNotice(notice_t eNotice, CDAPlayer* pSubject = NULL);
	...
	CWeaponDABase*    GetSDKWeapon( int i ) const;
	...
	virtual bool ThrowWeapon( CWeaponDABase* pWeapon, bool bAutoSwitch = true );
	...
	void DropWeaponsToPickUp(CWeaponDABase* pWeapon);
	...
	CWeaponDABase* FindAnyWeaponButBrawl();
	...
	void AddToLoadout(DAWeaponID eWeapon);
	void RemoveFromLoadout(DAWeaponID eWeapon);
...
public:
	...
	CNetworkVarEmbedded( CDAPlayerShared, m_Shared );
...
public:
	...
	void State_Transition( DAPlayerState newState );
	DAPlayerState State_Get() const;				// Get the current state.
...
private:
	...
	void State_Enter( DAPlayerState newState );	// Initialize the new state.
...
private:
	...
	static CDAPlayerStateInfo* State_LookupInfo( DAPlayerState state );
...
private:
	...
	void SDKThrowWeapon( CWeaponDABase *pWeapon, const Vector &vecForward, const QAngle &vecAngles, float flDiameter  );
	void SDKThrowWeaponInternal( CWeaponDABase *pWeapon, const Vector &vecForward, const QAngle &vecAngles, float flDiameter  );
	void SDKThrowWeaponDir( CWeaponDABase *pWeapon, const Vector &vecForward, Vector *pVecThrowDir );
...
inline CDAPlayer *ToDAPlayer( CBaseEntity *pEntity )
{
	if ( !pEntity || !pEntity->IsPlayer() )
		return NULL;

#ifdef _DEBUG
	Assert( dynamic_cast<CDAPlayer*>( pEntity ) != 0 );
#endif
	return static_cast< CDAPlayer* >( pEntity );
}

inline DAPlayerState CDAPlayer::State_Get() const
{
