...
bool CBriefcase::MyTouch( CBasePlayer *pPlayer )
{
	...
	VPhysicsDestroyObject();
	pDAPlayer->PickUpBriefcase(this);
...
void CBriefcase::Dropped( CDAPlayer* pPlayer )
{
	Touch();
	...
	if (random->RandomFloat(0, 1) < da_ctb_changecap.GetFloat())
		DAGameRules()->ChooseRandomCapturePoint(GetAbsOrigin());

	if (GetAbsOrigin().z < DAGameRules()->GetLowestSpawnPoint().z - 400)
...
void CBriefcaseCaptureZone::CaptureThink()
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		{
			StopParticleEffects( DAGameRules()->GetBriefcase() );
			DispatchParticleEffect( "dinero_splode", GetAbsOrigin(), GetAbsAngles() );
			...
			DAGameRules()->PlayerCapturedBriefcase(pPlayer);
			return;
		}
	}
}
...
void CRatRaceWaypoint::WaypointThink()
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if ((pPlayer->GetAbsOrigin() - GetAbsOrigin()).LengthSqr() < flRadiusSqr)
			DAGameRules()->PlayerReachedWaypoint(pPlayer, this);
	}
}
