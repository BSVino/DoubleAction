...
//Tony; when using teams, the player can only select classes on blue or red.
			...
			CDATeam *pTeam = GetGlobalDATeam( iTeam );
			...
			else if ( iClass < GetGlobalDATeam(iTeam)->GetNumPlayerClasses() )
			{
				const CDAPlayerClassInfo &pInfo = GetGlobalDATeam(iTeam)->GetPlayerClassInfo( iClass );
