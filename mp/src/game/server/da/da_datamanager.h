...
public:
	...
	void AddKillInfo(const CTakeDamageInfo& info, CDAPlayer* pKilled);

	void AddCharacterChosen(const char* pszCharacter);
	void AddWeaponChosen(DAWeaponID eWeapon);
	void AddSkillChosen(SkillID eSkill);

	da::protobuf::PlayerList* GetPlayerInList(CDAPlayer* pPlayer);
	void AddStyle(CDAPlayer* pPlayer, float flStyle);
...
private:
	...
	public:
		...
		CUtlVector<DAWeaponID> m_aeWeaponsChosen;
