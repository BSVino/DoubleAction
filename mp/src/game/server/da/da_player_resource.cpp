...
// Datatable
IMPLEMENT_SERVERCLASS_ST(CDAPlayerResource, DT_DAPlayerResource)
...
BEGIN_DATADESC( CDAPlayerResource )
#if defined ( SDK_USE_PLAYERCLASSES )
	// DEFINE_ARRAY( m_iPlayerClass, FIELD_INTEGER, MAX_PLAYERS+1 ),
#endif
END_DATADESC()

LINK_ENTITY_TO_CLASS( da_player_manager, CDAPlayerResource );

CDAPlayerResource::CDAPlayerResource( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CDAPlayerResource::UpdatePlayerData( void )
{
	int i;
	...
	{
		CDAPlayer *pPlayer = (CDAPlayer*)UTIL_PlayerByIndex( i );
...
void CDAPlayerResource::Spawn( void )
{
	int i;
