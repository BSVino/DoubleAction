...
LINK_ENTITY_TO_CLASS( bot, CDABot );

void CDABot::Initialize()
{
	// set bot skills
...
// Output : virtual void
//-----------------------------------------------------------------------------
void CDABot::RunPlayerMove( CUserCmd &cmd, float frametime )
{
...
void CDABot::HandleRespawn( CUserCmd &cmd )
{
...
bool CDAPlayer::RunMimicCommand( CUserCmd& cmd )
{
	if ( !IsBot() )
		return false;
