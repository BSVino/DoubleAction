...
void CDABot::ResetNavigationParams()
{
...
void CDABot::AddWaypoint( Vector center, NavTraverseType transient, int attribute, int id, bool AddToTail)
{
	NavAreaData_t data;
...
void CDABot::DealWithObstacles( CBaseEntity *pTouchEnt, CUserCmd &cmd )
{
	if( !pTouchEnt )
		return;
...
void CDABot::AddRandomPath( float randomStartAngle )
{
...
void CDABot::CheckStuck( CUserCmd &cmd )
{
...
void CDABot::CheckNavMeshAttrib( trace_t *ptr, CUserCmd &cmd )
{
...
bool CDABot::CreatePath( CBasePlayer *pPlayer, Vector OptionalOrg )
{
...
bool CDABot::CreateHidePath( Vector &HiDeSpot )
{
	//DevMsg("Let's hide\n");
...
void CDABot::SelectSchedule( bool forcePath )
{
	Vector HideSpot;

	if (DAGameRules()->GetWaypoint(0))
	{
		if (m_iRaceWaypoint == 0 && CreatePath(NULL, DAGameRules()->GetWaypoint(0)->GetAbsOrigin()))
		{
			m_nBotState = BOT_NAVIG_PATH;
			m_nBotSchedule = BOT_SCHED_COMBAT;
		}
		else if (m_iRaceWaypoint == 1 && CreatePath(NULL, DAGameRules()->GetWaypoint(1)->GetAbsOrigin()))
		{
			m_nBotState = BOT_NAVIG_PATH;
			m_nBotSchedule = BOT_SCHED_COMBAT;
		}
		else if (m_iRaceWaypoint == 2 && CreatePath(NULL, DAGameRules()->GetWaypoint(2)->GetAbsOrigin()))
		{
			m_nBotState = BOT_NAVIG_PATH;
			m_nBotSchedule = BOT_SCHED_COMBAT;
		}
	...
	else if (DAGameRules()->GetBountyPlayer() && CreatePath(NULL, DAGameRules()->GetBountyPlayer()->GetAbsOrigin()))
	{
		m_nBotState = BOT_NAVIG_PATH;
		m_nBotSchedule = BOT_SCHED_COMBAT;
	}
	else if (HasBriefcase() && CreatePath(NULL, DAGameRules()->GetCaptureZone()->GetAbsOrigin()))
	{
		m_nBotState = BOT_NAVIG_PATH;
		m_nBotSchedule = BOT_SCHED_COMBAT;
	}
	else if (DAGameRules()->GetBriefcase() && CreatePath(NULL, DAGameRules()->GetBriefcase()->GetAbsOrigin()))
	{
		m_nBotState = BOT_NAVIG_PATH;
		m_nBotSchedule = BOT_SCHED_COMBAT;
	}
...
bool CDABot::SafePathAhead( Vector origin )
{
...
void CDABot::Navigation( CUserCmd &cmd )
{
	int iLikelihood = 3;
	if (m_bEnemyOnSights)
		iLikelihood = 1;
	...
	if (DAGameRules()->GetBriefcase() && !DAGameRules()->GetBriefcase()->GetOwnerEntity())
		pWalkToTarget = DAGameRules()->GetBriefcase();
	else if (HasBriefcase())
		pWalkToTarget = DAGameRules()->GetCaptureZone();
