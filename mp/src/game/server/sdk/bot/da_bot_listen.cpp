...
//--------------------------------------------------------------------------------------------------------------
bool CDABot::IsNoiseHeard( void ) const
{
 ...
 */
bool CDABot::HeardInterestingNoise( void )
{
 ...
 * -1 == infinite range
 */
bool CDABot::CanHearNearbyEnemyGunfire( float range ) const
{
	...
	if (IsAttacking() && m_enemy != NULL && GetTimeSinceLastSawEnemy() < 1.0f)
	{
		...
		float enemyDistSq = (m_enemy->GetCentroid() - myOrigin).LengthSqr();
 ...
 */
bool CDABot::CanSeeNoisePosition( void ) const
{
	trace_t result;
 ...
 */
bool CDABot::UpdateLookAtNoise( void )
{
