...
#include "bot/bot.h"
#include "bot/da_bot_manager.h"
#include "bot/da_bot_chatter.h"
...
{
public:
	virtual void OnEnter( CDABot *bot ) { }				///< when state is entered
	virtual void OnUpdate( CDABot *bot ) { }			///< state behavior
	virtual void OnExit( CDABot *bot ) { }				///< when state exited
...
{
public:
	virtual void OnEnter( CDABot *bot );
	virtual void OnUpdate( CDABot *bot );
...
public:
	virtual void OnEnter( CDABot *bot );
	virtual void OnUpdate( CDABot *bot );
	virtual void OnExit( CDABot *bot );
...
protected:
	...
	void StopAttacking( CDABot *bot );
	void Dodge( CDABot *bot );									///< do dodge behavior
};
...
private:
	void AttendCurrentNoise( CDABot *bot );						///< move towards currently heard noise
...
public:
	...
	void SetLeader( CSDKPlayer *player )				{ m_leader = player; }

private:
	CHandle< CSDKPlayer > m_leader;								///< the player we are following
 ...
 */
class CDABot : public CBot< CSDKPlayer >
{
public:
	DECLARE_CLASS( CDABot, CBot< CSDKPlayer > );
	DECLARE_DATADESC();

	CDABot( void );												///< constructor initializes all values to zero
	virtual ~CDABot();
	...
	void Follow( CSDKPlayer *player );							///< begin following given Player
	...
	CSDKPlayer *GetFollowLeader( void ) const;					///< return the leader we are following
	...
	//- scenario / gamestate -----------------------------------------------------------------------------------------
	DAGameState *GetGameState( void );							///< return an interface to this bot's gamestate
	const DAGameState *GetGameState( void ) const;				///< return an interface to this bot's gamestate
	...
	bool RespondToHelpRequest( CSDKPlayer *player, Place place, float maxRange = -1.0f );	///< decide if we should move to help the player, return true if we will
	...
	void SetBotEnemy( CSDKPlayer *enemy );						///< set given player as our current enemy
	CSDKPlayer *GetBotEnemy( void ) const;
	...
	CSDKPlayer *GetClosestVisibleFriend( void ) const;			///< return the closest friend that we can see
	CSDKPlayer *GetClosestVisibleHumanFriend( void ) const;		///< return the closest human friend that we can see
	...
	CSDKPlayer *GetImportantEnemy( bool checkVisibility = false ) const;	///< return the closest "important" enemy for the given scenario (bomb carrier, VIP, hostage escorter)

	void UpdateReactionQueue( void );							///< update our reaction time queue
	CSDKPlayer *GetRecognizedEnemy( void );						///< return the most dangerous threat we are "conscious" of
	...
	CSDKPlayer *GetAttacker( void ) const;						///< return last enemy that hurt us
	...
	float GetTravelDistanceToPlayer( CSDKPlayer *player ) const;	///< return shortest path travel distance to this player	
	bool DidPlayerJustFireWeapon( const CSDKPlayer *player ) const;	///< return true if the given player just fired their weapon
	...
	bool IsNoticable( const CSDKPlayer *player, unsigned char visibleParts ) const;	///< return true if we "notice" given player 
	...
	bool IsSignificantlyCloser( const CSDKPlayer *testPlayer, const CSDKPlayer *referencePlayer ) const;	///< return true if testPlayer is significantly closer than referencePlayer
	...
	bool IsUsing( SDKWeaponID weapon ) const;						///< returns true if using the specific weapon
	...
	CWeaponSDKBase *GetActiveCFWeapon( void ) const;					///< get our current Counter-Strike weapon
...
private:
	...
	CHandle< CSDKPlayer > m_leader;									///< the ID of who we are following
	...
	DAGameState m_gameState;										///< our current knowledge about the state of the scenario
	...
	mutable CHandle< CSDKPlayer > m_enemy;							///< our current enemy
	...
	mutable CHandle< CSDKPlayer > m_closestVisibleFriend;			///< the closest friend we can see
	mutable CHandle< CSDKPlayer > m_closestVisibleHumanFriend;		///< the closest human friend we can see

	IntervalTimer m_attentionInterval;								///< time between attention checks

	CSDKPlayer *m_attacker;											///< last enemy that hurt us (may not be same as m_enemy)
	...
	bool DoEquip( CWeaponSDKBase *gun );								///< equip the given item
	...
	CSDKPlayer *FindMostDangerousThreat( void );						///< return most dangerous threat in my field of view (feeds into reaction time queue)
...
inline float CDABot::GetFeetZ( void ) const
{
	return GetAbsOrigin().z;
}

inline const Vector *CDABot::GetNoisePosition( void ) const
{
...
inline bool CDABot::IsAwareOfEnemyDeath( void ) const
{
...
inline bool CDABot::IsPanicking( void ) const
{
	return !m_panicTimer.IsElapsed();
}

inline void CDABot::StopPanicking( void )
{
	m_panicTimer.Invalidate();
}

inline bool CDABot::IsNotMoving( float minDuration ) const
{
	return (m_stillTimer.HasStarted() && m_stillTimer.GetElapsedTime() >= minDuration);
}

inline CWeaponSDKBase *CDABot::GetActiveCFWeapon( void ) const
{
	return reinterpret_cast<CWeaponSDKBase *>( GetActiveWeapon() );
}


inline float CDABot::GetCombatRange( void ) const
{ 
	return m_combatRange; 
}

inline void CDABot::SetRogue( bool rogue )
{ 
	m_isRogue = rogue;
}

inline void CDABot::Hurry( float duration )
{ 
	m_hurryTimer.Start( duration ); 
}

inline float CDABot::GetSafeTime( void ) const
{
	return m_safeTime;
}

inline bool CDABot::IsUnhealthy( void ) const
{
	return (GetHealth() <= 40);
}

inline bool CDABot::IsAlert( void ) const
{
	return !m_alertTimer.IsElapsed();
}

inline void CDABot::BecomeAlert( void )
{
	const float alertCooldownTime = 10.0f;
	m_alertTimer.Start( alertCooldownTime );
}

inline bool CDABot::IsSneaking( void ) const
{
	return !m_sneakTimer.IsElapsed();
}

inline void CDABot::Sneak( float duration )
{
	m_sneakTimer.Start( duration );
}

inline bool CDABot::IsFollowing( void ) const
{ 
	return m_isFollowing;
}

inline CSDKPlayer *CDABot::GetFollowLeader( void ) const
{ 
	return m_leader;
}

inline float CDABot::GetFollowDuration( void ) const
{
	return gpGlobals->curtime - m_followTimestamp;
}

inline bool CDABot::CanAutoFollow( void ) const
{ 
	return (gpGlobals->curtime > m_allowAutoFollowTime);
}

inline void CDABot::AimAtEnemy( void )
{ 
	m_isAimingAtEnemy = true;
}

inline void CDABot::StopAiming( void )
{ 
	m_isAimingAtEnemy = false;
}

inline bool CDABot::IsAimingAtEnemy( void ) const
{
	return m_isAimingAtEnemy;
}

inline float CDABot::GetStateTimestamp( void ) const
{
	return m_stateTimestamp;
}

inline DAGameState *CDABot::GetGameState( void )
{
	return &m_gameState;
}

inline const DAGameState *CDABot::GetGameState( void ) const
{
	return &m_gameState;
}

inline void CDABot::SetTask( TaskType task, CBaseEntity *entity )
{
	m_task = task;
	m_taskEntity = entity;
}

inline CDABot::TaskType CDABot::GetTask( void ) const
{
	return m_task;
}

inline CBaseEntity *CDABot::GetTaskEntity( void )
{
	return static_cast<CBaseEntity *>( m_taskEntity );
}

inline CDABot::MoraleType CDABot::GetMorale( void ) const
{
	return m_morale;
}

inline void CDABot::Surprise( float duration )
{
	m_surpriseTimer.Start( duration );
}

inline bool CDABot::IsSurprised( void ) const
{
	return !m_surpriseTimer.IsElapsed();
}

inline CNavArea *CDABot::GetNoiseArea( void ) const
{
	return m_noiseArea;
}

inline void CDABot::ForgetNoise( void )
{
	m_noiseTimestamp = 0.0f;
}

inline float CDABot::GetNoiseRange( void ) const
{
	if (IsNoiseHeard())
...
inline PriorityType CDABot::GetNoisePriority( void ) const
{ 
	return m_noisePriority;
}

inline BotChatterInterface *CDABot::GetChatter( void )
{
	return &m_chatter;
}

inline CSDKPlayer *CDABot::GetBotEnemy( void ) const
{
	return m_enemy;
}

inline int CDABot::GetNearbyEnemyCount( void ) const
{ 
	return min( GetEnemiesRemaining(), m_nearbyEnemyCount );
}

inline unsigned int CDABot::GetEnemyPlace( void ) const
{
	return m_enemyPlace;
}

inline int CDABot::GetNearbyFriendCount( void ) const
{
	return min( GetFriendsRemaining(), m_nearbyFriendCount );
}

inline CSDKPlayer *CDABot::GetClosestVisibleFriend( void ) const
{
	return m_closestVisibleFriend;
}

inline CSDKPlayer *CDABot::GetClosestVisibleHumanFriend( void ) const
{
	return m_closestVisibleHumanFriend;
}

inline float CDABot::GetTimeSinceAttacked( void ) const
{
	return gpGlobals->curtime - m_attackedTimestamp;
}

inline float CDABot::GetFirstSawEnemyTimestamp( void ) const
{
	return m_firstSawEnemyTimestamp;
}

inline float CDABot::GetLastSawEnemyTimestamp( void ) const
{
	return m_lastSawEnemyTimestamp;
}

inline float CDABot::GetTimeSinceLastSawEnemy( void ) const
{
	return gpGlobals->curtime - m_lastSawEnemyTimestamp;
}

inline float CDABot::GetTimeSinceAcquiredCurrentEnemy( void ) const
{
	return gpGlobals->curtime - m_currentEnemyAcquireTimestamp;
}

inline const Vector &CDABot::GetLastKnownEnemyPosition( void ) const
{
	return m_lastEnemyPosition;
}

inline bool CDABot::IsEnemyVisible( void ) const			
{
	return m_isEnemyVisible;
}

inline float CDABot::GetEnemyDeathTimestamp( void ) const	
{
	return m_enemyDeathTimestamp;
}

inline int CDABot::GetLastVictimID( void ) const
{
	return m_lastVictimID;
}

inline bool CDABot::CanSeeSniper( void ) const
{
	return m_isEnemySniperVisible;
}

inline bool CDABot::HasSeenSniperRecently( void ) const
{
	return !m_sawEnemySniperTimer.IsElapsed();
}

inline float CDABot::GetTravelDistanceToPlayer( CSDKPlayer *player ) const
{
	if (player == NULL)
		return -1.0f;
...
inline bool CDABot::HasPath( void ) const
{
	return (m_pathLength) ? true : false;
}

inline void CDABot::DestroyPath( void )		
{
	m_isStopping = false;
	m_pathLength = 0;
	m_pathLadder = NULL;
...
inline CNavArea *CDABot::GetLastKnownArea( void ) const		
{
	return m_lastKnownArea;
}

inline const Vector &CDABot::GetPathEndpoint( void ) const		
{
	return m_path[ m_pathLength-1 ].pos;
}

inline const Vector &CDABot::GetPathPosition( int index ) const
{
	return m_path[ index ].pos;
}

inline bool CDABot::IsUsingLadder( void ) const	
{
	return (m_pathLadder) ? true : false;
}

inline void CDABot::SetGoalEntity( CBaseEntity *entity )	
{
	m_goalEntity = entity;
}

inline CBaseEntity *CDABot::GetGoalEntity( void )
{
	return m_goalEntity;
}

inline void CDABot::ForceRun( float duration )
{
	Run();
	m_mustRunTimer.Start( duration );
}

inline void CDABot::Wait( float duration )			
{
	m_waitTimer.Start( duration );
}

inline bool CDABot::IsWaiting( void ) const		
{
	return !m_waitTimer.IsElapsed();
}

inline void CDABot::StopWaiting( void )			
{
	m_waitTimer.Invalidate();
}

inline bool CDABot::HasVisitedEnemySpawn( void ) const		
{
	return m_hasVisitedEnemySpawn;
}

inline const Vector &CDABot::EyePositionConst( void ) const		
{
	return m_eyePosition;
}
	
inline void CDABot::SetLookAngles( float yaw, float pitch )
{
	m_lookYaw = yaw;
	m_lookPitch = pitch;
}

inline void CDABot::SetForwardAngle( float angle ) 
{
	m_forwardAngle = angle;
}

inline void CDABot::SetLookAheadAngle( float angle ) 
{
	m_lookAheadAngle = angle;
}

inline void CDABot::ClearLookAt( void )
{ 
...
inline bool CDABot::IsLookingAtSpot( PriorityType pri ) const
{ 
...
inline bool CDABot::IsViewMoving( float angleVelThreshold ) const
{
...
inline bool CDABot::HasViewBeenSteady( float duration ) const
{
	return (m_viewSteadyTimer.GetElapsedTime() > duration);
}

inline bool CDABot::HasLookAtTarget( void ) const
{
	return (m_lookAtSpotState != NOT_LOOKING_AT_SPOT);
}

inline bool CDABot::IsEnemyPartVisible( VisiblePartType part ) const
{ 
	VPROF_BUDGET( "CDABot::IsEnemyPartVisible", VPROF_BUDGETGROUP_NPCS );
...
inline bool CDABot::IsSignificantlyCloser( const CSDKPlayer *testPlayer, const CSDKPlayer *referencePlayer ) const
{
	if ( !referencePlayer )
		return true;
...
inline void CDABot::ClearApproachPoints( void )	
{
	m_approachPointCount = 0;
}

inline const CNavArea *CDABot::GetInitialEncounterArea( void ) const
{
	return m_initialEncounterArea;
}

inline void CDABot::SetInitialEncounterArea( const CNavArea *area )		
{
	m_initialEncounterArea = area;
}

inline bool CDABot::IsThrowingGrenade( void ) const		
{
	return m_grenadeTossState != NOT_THROWING;
}

inline bool CDABot::IsAvoidingGrenade( void ) const
{
	return !m_isAvoidingGrenade.IsElapsed();
}

inline void CDABot::PrimaryAttack( void )
{
	BaseClass::PrimaryAttack();
}

inline CDABot::ZoomType CDABot::GetZoomLevel( void )
{
	if (GetFOV() > 60.0f)
		return NO_ZOOM;
	if (GetFOV() > 25.0f)
...
inline bool CDABot::IsWaitingForZoom( void ) const		
{
	return !m_zoomTimer.IsElapsed();
}

inline bool CDABot::IsUsingVoice() const
{
	return m_voiceEndTimestamp > gpGlobals->curtime; 
}

inline bool CDABot::IsOpeningDoor( void ) const
{
 ...
 */
inline bool IsSniperRifle( CWeaponSDKBase *weapon )
{
	if (weapon == NULL)
		return false;
...
{
public:
	PathCost( CDABot *bot, RouteType route = SAFEST_ROUTE )
	{
		m_bot = bot;
		m_route = route;
	}
...
// Prototypes
//
extern int GetBotFollowCount( CSDKPlayer *leader );
extern const Vector *FindNearbyRetreatSpot( CDABot *me, float maxRange = 250.0f );
