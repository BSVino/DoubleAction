 ...
 * Begin attacking
 */
void AttackState::OnEnter( CDABot *me )
{
 ...
 */
void AttackState::StopAttacking( CDABot *me )
{
	if (me->GetTask() == CDABot::SNIPING)
	{
 ...
 * Do dodge behavior
 */
void AttackState::Dodge( CDABot *me )
{
	//
	// Dodge.
 ...
 */
void AttackState::OnUpdate( CDABot *me )
{
	...
	Vector enemyOrigin = ToSDKPlayer(enemy)->GetCentroid();
	...
	if (me->IsUsingSniperRifle())
	{
		...
		if (me->GetZoomLevel() == CDABot::NO_ZOOM && me->AdjustZoom( targetRange ))
	...
	if (!me->IsEnemyVisible())
	{
		...
		{
			if (me->GetDisposition() == CDABot::ENGAGE_AND_INVESTIGATE)
			{
	...
	if (!me->IsEnemyVisible() && (notSeenEnemyTime > chaseTime || !m_haveSeenEnemy))
	{
		...
		else
		{
			...
			me->SetTask( CDABot::MOVE_TO_LAST_KNOWN_ENEMY_POSITION, enemy );
 ...
 * Finish attack
 */
void AttackState::OnExit( CDABot *me )
{
