 ...
 */
void CDABot::SetState( BotState *state )
{
	...
	if ( IsDefusingBomb() )
	{
		...
		if ( bombPos != NULL )
		{
			if ( TheDABots()->GetBombDefuser() == this )
			{
				if ( TheDABots()->IsBombPlanted() )
				{
...
//--------------------------------------------------------------------------------------------------------------
void CDABot::Follow( CSDKPlayer *player )
{
	if (player == NULL)
		return;
 ...
 */
void CDABot::ContinueFollowing( void )
{
	SetTask( FOLLOW );
 ...
 * Stop following
 */
void CDABot::StopFollowing( void )
{
	m_isFollowing = false;
	m_leader = NULL;
 ...
 * Use the entity
 */
void CDABot::UseEntity( CBaseEntity *entity )
{
 ...
 */
void CDABot::OpenDoor( CBaseEntity *door )
{
 ...
 */
void CDABot::Hide( CNavArea *searchFromArea, float duration, float hideRange, bool holdPosition )
{
	DestroyPath();
 ...
 */
void CDABot::Hide( const Vector &hidingSpot, float duration, bool holdPosition )
{
 ...
 * If 'searchFromArea' is non-NULL, hiding spots are looked for from that area first.
 */
bool CDABot::TryToHide( CNavArea *searchFromArea, float duration, float hideRange, bool holdPosition, bool useNearest )
{
	CNavArea *source;
	Vector sourcePos;
	if (searchFromArea)
 ...
 */
bool CDABot::TryToRetreat( float maxRange, float duration )
{
 ...
 */
void CDABot::Attack( CSDKPlayer *victim )
{
	if (victim == NULL)
		return;
	...
	Vector victimOrigin = victim->GetCentroid();
	...
	Vector toEnemy = victimOrigin - GetCentroid();
 ...
 */
void CDABot::StopAttacking( void )
{
...
//--------------------------------------------------------------------------------------------------------------
bool CDABot::IsAttacking( void ) const
{
 ...
 */
bool CDABot::IsHiding( void ) const
{
 ...
 */
bool CDABot::IsAtHidingSpot( void ) const
{
	if (!IsHiding())
		return false;
 ...
 */
float CDABot::GetHidingTime( void ) const
{
	if (IsHiding())
	{
 ...
 */
bool CDABot::IsHunting( void ) const
{
 ...
 */
bool CDABot::IsMovingTo( void ) const
{
 ...
 */
bool CDABot::IsBuying( void ) const
{
...
//--------------------------------------------------------------------------------------------------------------
bool CDABot::IsInvestigatingNoise( void ) const
{
 ...
 */
void CDABot::MoveTo( const Vector &pos, RouteType route )
{
 ...
 */
void CDABot::InvestigateNoise( void )
{
 ...
 */
bool CDABot::MoveToInitialEncounter( void )
{
	int myTeam = GetTeamNumber();
	int enemyTeam = OtherTeam( myTeam );

	// build a path to an enemy spawn point
	CBaseEntity *enemySpawn = TheDABots()->GetRandomSpawn( enemyTeam );
