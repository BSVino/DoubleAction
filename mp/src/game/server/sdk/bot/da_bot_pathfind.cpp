 ...
 */
bool CDABot::ComputePathPositions( void )
{
	if (m_pathLength == 0)
		return false;
 ...
 */
void CDABot::SetupLadderMovement( void )
{
...
void CDABot::ComputeLadderEndpoint( bool isAscending )
{
	trace_t result;
	Vector from, to;
 ...
 */
bool CDABot::UpdateLadderMovement( void )
{
 ...
 */
bool CDABot::FindClosestPointOnPath( const Vector &worldPos, int startIndex, int endIndex, Vector *close ) const
{
 ...
 */
int CDABot::FindOurPositionOnPath( Vector *close, bool local ) const
{
	if (!HasPath())
		return -1;
 ...
 */
bool CDABot::IsStraightLinePathWalkable( const Vector &goal ) const
{
 ...
 */
int CDABot::FindPathPoint( float aheadRange, Vector *point, int *prevIndex )
{
	Vector myOrigin = GetCentroid();
 ...
 */
void CDABot::SetPathIndex( int newIndex )
{
 ...
 */
bool CDABot::IsNearJump( void ) const
{
 ...
 */
float CDABot::GetApproximateFallDamage( float height ) const
{
 ...
 */
bool CDABot::IsFriendInTheWay( const Vector &goalPos )
{
	...
	for( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		...
		if ((pos - player->GetCentroid()).IsLengthLessThan( friendRadius ))
		{
 ...
 */
void CDABot::FeelerReflexAdjustment( Vector *goalPosition )
{
 ...
 */
bool CDABot::IsRunning( void ) const
{
 ...
 */
CDABot::PathResult CDABot::UpdatePathMovement( bool allowSpeedChange )
{
	VPROF_BUDGET( "CDABot::UpdatePathMovement", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CDABot::BuildTrivialPath( const Vector &goal )
{
	Vector myOrigin = GetCentroid();
 ...
 */
bool CDABot::ComputePath( const Vector &goal, RouteType route )
{
	VPROF_BUDGET( "CDABot::ComputePath", VPROF_BUDGETGROUP_NPCS );
	...
	if (IsSafe() && SDKGameRules()->IsTeamplay())
	{
 ...
 */
float CDABot::GetPathDistanceRemaining( void ) const
{
	if (!HasPath())
		return -1.0f;
