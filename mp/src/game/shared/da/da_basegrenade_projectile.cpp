...
CDAPlayer* CBaseGrenadeProjectile::GetSDKOwner()
{
	return ToDAPlayer(GetOwnerEntity());
}
...
float CBaseGrenadeProjectile::GetCurrentTime()
{
	...
	int CBaseGrenadeProjectile::DrawModel( int flags )
	{
		...
		if ( GetThrower() != C_BasePlayer::GetLocalPlayer() )
		{
			...
			{
//Tony; FIXME!
//				C_DAPlayer *pPlayer = dynamic_cast<C_DAPlayer*>( GetThrower() );
...
//				}
		...
		if (pLocalPlayer->GetActiveDAWeapon() && pLocalPlayer->GetActiveDAWeapon()->HasAimInSpeedPenalty())
