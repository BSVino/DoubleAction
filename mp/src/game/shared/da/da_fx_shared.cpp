...
#ifdef CLIENT_DLL
	C_DAPlayer *pPlayer = ToDAPlayer( ClientEntityList().GetBaseEntity( iPlayerIndex ) );
#else
	CDAPlayer *pPlayer = ToDAPlayer( UTIL_PlayerByIndex( iPlayerIndex) );
#endif

	CSDKWeaponInfo *pWeaponInfo = CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)iWeaponID);
	if ( !pWeaponInfo )
	{
