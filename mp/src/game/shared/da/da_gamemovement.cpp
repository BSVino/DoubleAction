...
#else
	#include "da_player.h"
	#include "bots/da_bot.h"
#endif

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

ConVar	da_clamp_back_speed( "da_clamp_back_speed", "0.9", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY );
ConVar  da_clamp_back_speed_min("da_clamp_back_speed_min", "100", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar  da_dive_speed("da_dive_speed", "330", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar da_slidetime("da_slidetime", "1.5", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
...
class CDAGameMovement : public CGameMovement
{
public:
	DECLARE_CLASS( CDAGameMovement, CGameMovement );

	CDAGameMovement();
	virtual ~CDAGameMovement();
...
static CDAGameMovement g_GameMovement;
...
void CDAGameMovement::SetPlayerSpeed( void )
{
	mv->m_flClientMaxSpeed = m_pDAPlayer->GetPlayerMaxSpeed(!!(mv->m_nButtons & IN_DUCK));

	if (m_pDAPlayer->PlayerFrozen())
		mv->m_flClientMaxSpeed *= m_pDAPlayer->m_flFreezeAmount;

#if defined ( SDK_USE_PRONE )
	if ( m_pDAPlayer->GetGroundEntity() != NULL )
	{
		if( m_pDAPlayer->m_Shared.IsGoingProne() )
		{
			float pronetime = m_pDAPlayer->m_Shared.m_flGoProneTime - m_pDAPlayer->GetCurrentTime();
			...
			if ( m_pDAPlayer->m_bUnProneToDuck )
				maxSpeed *= 0.33;
			
			mv->m_flClientMaxSpeed = ( ( 1 - flProneFraction ) * m_pDAPlayer->m_Shared.m_flProneSpeed ) + ( flProneFraction * maxSpeed );
		}
		else if ( m_pDAPlayer->m_Shared.IsGettingUpFromProne() )
		{
			float pronetime = m_pDAPlayer->m_Shared.m_flUnProneTime - m_pDAPlayer->GetCurrentTime();
			...
			mv->m_flClientMaxSpeed = ( flProneFraction * m_pDAPlayer->m_Shared.m_flProneSpeed ) + ( ( 1 - flProneFraction ) * maxSpeed );
		}
	}	
#endif // SDK_USE_PRONE

	mv->m_flClientMaxSpeed = m_pDAPlayer->m_Shared.ModifySkillValue(mv->m_flClientMaxSpeed, 0.2f, SKILL_ATHLETIC);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CDAGameMovement::CheckParameters( void )
{
	QAngle	v_angle;
...
//Tony; left for playing a sound if you want.
		...
		if ( m_pDAPlayer->m_Shared.IsJumping() )
		{
			m_pDAPlayer->m_Shared.SetJumping( false );
		}
	}
	...
	if ( player->GetGroundEntity() != NULL && !IsDead() && player->m_Local.m_flFallVelocity >= 600 )
	{
		...
		{
			if (!m_pDAPlayer->m_Shared.IsDiving() && !m_pDAPlayer->m_Shared.IsSliding())
			{
				m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_STAND_TO_ROLL );
				m_pDAPlayer->m_Shared.StartRolling();
				//m_pDAPlayer->FreezePlayer(0.6f, RemapValClamped(m_pDAPlayer->m_Local.m_flFallVelocity, 600, 1000, 0.4f, 0.8f));
			}
		}
	}
...
void CDAGameMovement::ProcessMovement( CBasePlayer *pBasePlayer, CMoveData *pMove )
{
	//Store the player pointer
	m_pDAPlayer = ToDAPlayer( pBasePlayer );
	Assert( m_pDAPlayer );

	m_pDAPlayer->InitSpeeds();

	float flFrameTime = gpGlobals->frametime;

	gpGlobals->frametime *= m_pDAPlayer->GetSlowMoMultiplier();
...
bool CDAGameMovement::CanAccelerate()
{
	// Only allow the player to accelerate when in certain states.
	DAPlayerState curState = m_pDAPlayer->State_Get();
...
void CDAGameMovement::AirAccelerate( Vector& wishdir, float wishspeed, float accel )
{
	float addspeed, accelspeed, currentspeed;
	float wishspd;

	if (m_pDAPlayer->m_Shared.IsSuperFalling())
	{
	...
	if (m_pDAPlayer->m_Shared.IsDiving())
		return;
	...
	//if (m_pDAPlayer->m_Shared.runtime <= 0)
	{
...
void CDAGameMovement::WalkMove( void )
{
	bool bSprintButtonPressed = ( mv->m_nButtons & IN_SPEED ) > 0;

	if( bSprintButtonPressed )
		m_pDAPlayer->m_Shared.SetAimIn( true );
	else
		m_pDAPlayer->m_Shared.SetAimIn( false );
	...
	if (m_pDAPlayer->m_Shared.IsSliding() && !m_pDAPlayer->m_Shared.IsGettingUpFromSlide())
		vecWishDirection = Vector(0,0,0);

	/*else if (m_pDAPlayer->m_Shared.IsRolling())
	{
		if (vecWishDirection.LengthSqr() > 0)
			vecWishDirection = (m_pDAPlayer->m_Shared.GetRollDirection() * mv->m_flMaxSpeed + vecWishDirection)/2;
		else
			vecWishDirection = m_pDAPlayer->m_Shared.GetRollDirection() * mv->m_flMaxSpeed;
	...
	if (da_clamp_back_speed.GetFloat() < 1.0 && VectorLength(mv->m_vecVelocity) > da_clamp_back_speed_min.GetFloat())
	{
		...
		if ( flDot < 0 )
		{
			...
			float flMaxBackSpeed = (mv->m_flMaxSpeed * da_clamp_back_speed.GetFloat());
...
void CDAGameMovement::PlayerMove (void)
{
#ifdef STUCK_DEBUG
	...
	if (!player_stuck_before)
	{
		...
		if (PlayerIsStuck())
		{
			for (int i = 0; i < 54; i++)
			{
				m_flStuckCheckTime[ m_pDAPlayer->entindex() ][ 0 ] = 0;
				CheckStuck();
			}
	...
	if (m_flStuckCheck)
	{
		if (m_pDAPlayer->IsBot())
		{
			CDABot* pBot = dynamic_cast<CDABot*>(m_pDAPlayer);
...
void CDAGameMovement::RealPlayerMove (void)
{
	VPROF( "CDAGameMovement::PlayerMove" );
	if (m_pDAPlayer->PlayerFrozen())
	{
		mv->m_flForwardMove *= m_pDAPlayer->m_flFreezeAmount;
		mv->m_flSideMove *= m_pDAPlayer->m_flFreezeAmount;
		mv->m_flUpMove *= m_pDAPlayer->m_flFreezeAmount;
...
void CDAGameMovement::CategorizePosition( void )
{
	// Observer.
	...
	else if (m_pDAPlayer->m_Shared.IsDiving() && m_pDAPlayer->GetCurrentTime() < m_pDAPlayer->m_Shared.GetDiveTime() + DIVE_RISE_TIME)
	{
		SetGroundEntity(NULL);
	}
	else
	...
	if (m_pDAPlayer->m_Shared.IsSliding())
		player->m_surfaceFriction *= m_pDAPlayer->m_Shared.GetSlideFriction();
	else if (m_pDAPlayer->m_Shared.IsProne() && m_pDAPlayer->m_Shared.m_bProneSliding)
		player->m_surfaceFriction *= 0.2f;
	else if (m_pDAPlayer->m_Shared.IsRolling ())
		player->m_surfaceFriction *= da_acro_roll_friction.GetFloat ();
}

inline void CDAGameMovement::TracePlayerBBoxWithStep( const Vector &vStart, const Vector &vEnd, 
							unsigned int fMask, int collisionGroup, trace_t &trace )
{
	VPROF( "CDAGameMovement::TracePlayerBBoxWithStep" );
...
bool CDAGameMovement::ResolveStanding( void )
{
	VPROF( "CDAGameMovement::ResolveStanding" );
...
//-----------------------------------------------------------------------------
void CDAGameMovement::ReduceTimers( void )
{
	Vector vecPlayerVelocity = m_pDAPlayer->GetAbsVelocity();

	float flAimInSpeed = gpGlobals->frametime*3;

	float flAimGoal;
	if (m_pDAPlayer->m_Shared.IsAimedIn())
		flAimGoal = 1;
	else
		flAimGoal = 0;

	if (mv->m_vecVelocity.Length2DSqr() > 10*10)
		m_pDAPlayer->UseStyleCharge(SKILL_ATHLETIC, 2 * gpGlobals->frametime);

	m_pDAPlayer->m_Shared.SetAimIn(Approach(flAimGoal, m_pDAPlayer->m_Shared.GetAimIn(), flAimInSpeed));

	m_pDAPlayer->m_Shared.RampSlowAimIn(flAimGoal);

	m_pDAPlayer->DecayStyle();

#if defined ( SDK_USE_STAMINA ) || defined ( SDK_USE_SPRINTING )
	float flStamina = m_pDAPlayer->m_Shared.GetStamina();
#endif
	...
	{
		m_pDAPlayer->m_Shared.ResetSprintPenalty();
	}
	...
	if ( bSprinting && fl2DVelocitySquared > 10000 ) //speed > 100
	{
		...
		m_pDAPlayer->m_Shared.SetStamina( flStamina );
	}
	else
...
#if defined ( SDK_USE_STAMINA ) || defined ( SDK_USE_SPRINTING )
	{
		...
		else if ( ( m_pDAPlayer->GetFlags() & FL_ONGROUND ) && 
					( mv->m_nButtons & IN_DUCK ) &&
					( m_pDAPlayer->GetFlags() & FL_DUCKING ) )
		{
...
#if 0
	if (m_pDAPlayer->m_Shared.taptime > 0)
	{/*TODO: Don't use milliseconds?*/
		m_pDAPlayer->m_Shared.taptime -= 1000*gpGlobals->frametime;
		if (m_pDAPlayer->m_Shared.taptime <= 0)
		{
			m_pDAPlayer->m_Shared.taptime = 0;
		}
	}
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CDAGameMovement::CheckJumpButton( void )
{
	if (!m_pDAPlayer->IsAlive())
	{
		mv->m_nOldButtons |= IN_JUMP ;	// don't jump again until released
		return false;
	}
#if defined ( SDK_USE_PRONE )
	if( m_pDAPlayer->m_Shared.IsProne() ||
		m_pDAPlayer->m_Shared.IsGettingUpFromProne() || 
		m_pDAPlayer->m_Shared.IsGoingProne() )
	{
	...
	if (m_pDAPlayer->m_Shared.IsRolling() || (m_pDAPlayer->m_Shared.IsSliding() && m_pDAPlayer->m_Shared.IsGettingUpFromSlide()))
		return false;
	...
	if ( m_pDAPlayer->GetWaterLevel() >= 2 )
	{	
		// swimming, not jumping
		SetGroundEntity( NULL );

		if(m_pDAPlayer->GetWaterType() == CONTENTS_WATER)    // We move up a certain amount
			mv->m_vecVelocity[2] = 100;
		else if (m_pDAPlayer->GetWaterType() == CONTENTS_SLIME)
	...
	if ( !m_pDAPlayer->m_Shared.IsGettingUpFromSlide() )
		m_pDAPlayer->PlayStepSound( (Vector &)mv->GetAbsOrigin(), player->GetSurfaceData(), 1.0, true );

	m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_JUMP );

//Tony; liek the landing sound, leaving this here if as an example for playing a jump sound.
	// make the jump sound
	CPASFilter filter( m_pDAPlayer->GetAbsOrigin() );
	filter.UsePredictionRules();
	m_pDAPlayer->EmitSound( filter, m_pDAPlayer->entindex(), "Player.Jump" );
	...
	if ( (  m_pDAPlayer->m_Local.m_bDucking ) || (  m_pDAPlayer->GetFlags() & FL_DUCKING ) )
	{
	...
	m_pDAPlayer->m_Shared.SetJumping( true );

	m_pDAPlayer->ReadyWeapon();
...
//-----------------------------------------------------------------------------
bool CDAGameMovement::CanUnduck()
{
	int i;
	trace_t trace;
	Vector newOrigin;
...
#if defined ( SDK_USE_PRONE )
bool CDAGameMovement::CanUnprone()
{
	if (m_pDAPlayer->m_Shared.IsGoingProne())
		return false;

	if (m_pDAPlayer->m_Shared.IsGettingUpFromProne())
		return false;
...
//-----------------------------------------------------------------------------
void CDAGameMovement::FinishUnDuck( void )
{
	int i;
	trace_t trace;
	Vector newOrigin;
...
//-----------------------------------------------------------------------------
void CDAGameMovement::FinishDuck( void )
{
	Vector hullSizeNormal = VEC_HULL_MAX - VEC_HULL_MIN;
	Vector hullSizeCrouch = VEC_DUCK_HULL_MAX - VEC_DUCK_HULL_MIN;
...
// Input  : duckFraction - 
//-----------------------------------------------------------------------------
void CDAGameMovement::SetSDKDuckedEyeOffset( float duckFraction )
{
...
#if defined ( SDK_USE_PRONE )
void CDAGameMovement::SetProneEyeOffset( float proneFraction )
{
	Vector vecPropViewOffset = VEC_PRONE_VIEW;
	Vector vecStandViewOffset = GetPlayerViewOffset( player->m_Local.m_bDucked || m_pDAPlayer->m_bUnProneToDuck );
...
void CDAGameMovement::FinishUnProne( void )
{
	m_pDAPlayer->m_Shared.m_flUnProneTime = 0.0f;
...
void CDAGameMovement::FinishProne( void )
{	
	m_pDAPlayer->m_Shared.SetProne( true );
	m_pDAPlayer->m_Shared.m_flGoProneTime = 0.0f;
...
bool CDAGameMovement::LadderMove()
{
	trace_t pm;
	bool onFloor;
	Vector floor;
	...
	m_pDAPlayer->m_Shared.EndSlide();
	m_pDAPlayer->m_Shared.SetProne(false, true);
	m_pDAPlayer->m_Shared.SetJumping(false);
	SetProneEyeOffset(1);
	SetRollEyeOffset(1);
	return true;
...
void CDAGameMovement::SetSlideEyeOffset( float flFraction )
{
	if (flFraction < 0)
		flFraction = 0;
...
void CDAGameMovement::SetUnSlideEyeOffset( float flFraction )
{
	Vector vecStartViewOffset = m_pDAPlayer->m_Shared.m_vecUnSlideEyeStartOffset;
...
void CDAGameMovement::SetRollEyeOffset( float flFraction )
{
	if (flFraction < 0.5f)
	{
		Vector vecStartViewOffset = GetPlayerViewOffset( false );
		if (m_pDAPlayer->m_Shared.IsRolling() && m_pDAPlayer->m_Shared.IsRollingFromDive())
...
//-----------------------------------------------------------------------------
void CDAGameMovement::FixPlayerDiveStuck( bool upward )
{
...
//-----------------------------------------------------------------------------
void CDAGameMovement::Duck( void )
{
	...
	if (m_pDAPlayer->m_Shared.IsProne())
	{
		if (mv->m_nButtons & (IN_FORWARD|IN_BACK|IN_MOVELEFT|IN_MOVERIGHT))
			m_pDAPlayer->m_Shared.m_bProneSliding = false;

		if (gpGlobals->curtime - m_pDAPlayer->m_Shared.GetDiveToProneLandTime() > da_d2p_stunt_forgiveness.GetFloat())
			m_pDAPlayer->m_Shared.m_bProneSliding = false;
	}

	// Prone / UnProne - we don't duck if this is happening
	if( m_pDAPlayer->m_Shared.IsGettingUpFromProne() == true )
	{
		float pronetime = m_pDAPlayer->m_Shared.m_flUnProneTime - m_pDAPlayer->GetCurrentTime();
		...
		if( pronetime < 0 )
		{
			...
			if ( !m_pDAPlayer->m_bUnProneToDuck && ( mv->m_nButtons & IN_DUCK ) )
			{
	...
	else if ( (m_pDAPlayer->m_Shared.IsGoingProne() == true) && !m_pDAPlayer->m_Shared.IsGettingUpFromSlide() )
	{
		float pronetime = m_pDAPlayer->m_Shared.m_flGoProneTime - m_pDAPlayer->GetCurrentTime();
	...
	else if( m_pDAPlayer->m_Shared.IsSliding() )
	{
		if (!m_pDAPlayer->m_Shared.IsGettingUpFromSlide() && !m_pDAPlayer->m_Shared.IsAirSliding())
		{
			trace_t tr;
			Ray_t vecRay;
			...
			if (tr.DidHitWorld())
			{
				...
				if (tr.plane.normal.Dot(Vector(0, 0, 1)) < 0.99f && tr.plane.normal.Dot(vecVelocityFlat) > 0)
					m_pDAPlayer->m_Shared.AddSlideTime(gpGlobals->frametime);
			}
		}

		/*if (!m_pDAPlayer->GetGroundEntity())
		{
			// if we just left the ground
			if (!m_pDAPlayer->m_Shared.IsAirSliding())
			{
				m_pDAPlayer->m_Shared.PlayEndSlideSound();
				m_pDAPlayer->m_Shared.SetAirSliding( true );
			}
		}
		else */if (m_pDAPlayer->GetCurrentTime() > m_pDAPlayer->m_Shared.GetSlideAutoEndTime() && !m_pDAPlayer->m_Shared.IsGettingUpFromSlide())
		{
			// if there's no room to crouch indicate transition to prone
			if( !CanUnprone() )
				m_pDAPlayer->m_Shared.m_flGoProneTime = m_pDAPlayer->GetCurrentTime();
			
			m_pDAPlayer->m_Shared.StandUpFromSlide();
			SetUnSlideEyeOffset( 0.0 );
		}
		else if (m_pDAPlayer->m_Shared.IsAirSliding())
		{
			// if we hit the ground from the air while sliding start slide sound
			m_pDAPlayer->m_Shared.PlayStartSlideSound();
			m_pDAPlayer->m_Shared.SetAirSliding( false );
		}
		else if (m_pDAPlayer->m_Shared.IsDiveSliding())
		{
			SetSlideEyeOffset(1);
		}
		else
		{
			float fraction = (m_pDAPlayer->GetCurrentTime() - m_pDAPlayer->m_Shared.GetSlideStartTime()) / TIME_TO_SLIDE;
	...
	else if( m_pDAPlayer->m_Shared.IsRolling() )
	{
		...
		else */if ( m_pDAPlayer->GetCurrentTime() > m_pDAPlayer->m_Shared.GetRollTime() + ROLL_TIME + ROLLFINISH_TIME )
		{
			m_pDAPlayer->m_Shared.EndRoll();
			SetRollEyeOffset( 0.0 );
		...
		else if ( m_pDAPlayer->GetCurrentTime() > m_pDAPlayer->m_Shared.GetRollTime() + ROLL_TIME )
		{
			...
			{
				float fraction = (m_pDAPlayer->GetCurrentTime() - m_pDAPlayer->m_Shared.GetRollTime()) / (ROLL_TIME + ROLLFINISH_TIME);
	...
	else if( m_pDAPlayer->m_Shared.IsDiving() )
	{
		trace_t	tr;
		Vector start, end;
		...
		if (m_pDAPlayer->GetGroundEntity() &&
			m_pDAPlayer->GetCurrentTime() > m_pDAPlayer->m_Shared.GetDiveTime() + DIVE_RISE_TIME)
		{
			m_pDAPlayer->m_Shared.EndDive();
			m_pDAPlayer->SetViewOffset( GetPlayerViewOffset( false ) );
			...
			Vector vecLocalVelocity = m_pDAPlayer->GetLocalVelocity();
			...
			{
				m_pDAPlayer->m_Shared.DiveLandedProne();
				SetProneEyeOffset( 1.0 );

				CPASFilter filter( m_pDAPlayer->GetAbsOrigin() );
				filter.UsePredictionRules();
				m_pDAPlayer->EmitSound( filter, m_pDAPlayer->entindex(), "Player.DiveLand" );
			}
			else if (bWantsSlide && m_pDAPlayer->m_Shared.CanRoll())
			{
				Vector vecAbsVelocity = m_pDAPlayer->GetAbsVelocity();
				...
				if (flVelocity >= 1e-5)
				{
					...
					m_pDAPlayer->SetAbsVelocity(vecAbsVelocity);

					m_pDAPlayer->m_Shared.StartSliding(true);

					float flSpeedFraction = RemapValClamped(flVelocity/da_dive_speed.GetFloat(), 0, 1, 0.2f, 1);

					mv->m_vecVelocity = m_pDAPlayer->m_Shared.GetSlideDirection() * (m_pDAPlayer->GetMaxSlideSpeed() * flSpeedFraction);
					mv->m_flClientMaxSpeed = m_pDAPlayer->GetMaxSlideSpeed();
					mv->m_flMaxSpeed = m_pDAPlayer->GetMaxSlideSpeed();
					player->m_surfaceFriction = m_pDAPlayer->m_Shared.GetSlideFriction();

					SetSlideEyeOffset( 1.0 );
				}
			}
			else if (bWantsRoll && m_pDAPlayer->m_Shared.ShouldRollAfterDiving() && m_pDAPlayer->m_Shared.CanRoll())
			{
				m_pDAPlayer->m_Shared.StartRolling(true);
				SetRollEyeOffset( 0.0 );
				m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_DIVE_TO_ROLL );

				CPASFilter filter( m_pDAPlayer->GetAbsOrigin() );
				filter.UsePredictionRules();
				m_pDAPlayer->EmitSound( filter, m_pDAPlayer->entindex(), "Player.GoRoll" );
			}
			else
			{
				m_pDAPlayer->m_Shared.SetProne(false, false);

				if (!CanUnduck())
				{
					m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_PRONE_TO_CROUCH );
					m_pDAPlayer->m_bUnProneToDuck = true;
				...
				{
					m_pDAPlayer->m_Shared.StandUpFromProne();
					m_pDAPlayer->m_bUnProneToDuck = false;
					m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_PRONE_TO_STAND );
				}
	...
	if ( m_pDAPlayer->m_Shared.CanChangePosition() )
	{
		...
		bool bGetUpFromProne = (m_pDAPlayer->GetCurrentTime() > m_pDAPlayer->m_Shared.m_flDisallowUnProneTime) && (bGetUp || !!(mv->m_nButtons & (IN_BACK|IN_FORWARD|IN_MOVELEFT|IN_MOVERIGHT)));

		if (m_pDAPlayer->m_Shared.IsSliding() && (buttonsReleased & IN_ALT1) && m_pDAPlayer->GetCurrentTime() - m_pDAPlayer->m_Shared.GetSlideStartTime() > m_pDAPlayer->GetUserInfoFloat("da_slide_unclick_time", 0.25f))
			bGetUp = true;
		...
		{
			if (m_pDAPlayer->m_Shared.GetLastDuckPress() < 0)
				m_pDAPlayer->m_Shared.SetDuckPress();
			else
			{
				float flTime = gpGlobals->curtime - m_pDAPlayer->m_Shared.GetLastDuckPress();
				if (flTime < 0.4f)
				{
					bRoll = true;
					m_pDAPlayer->m_Shared.SetDuckPress(true);
				}
				else
					m_pDAPlayer->m_Shared.SetDuckPress();
			}
		...
		if (bStunt)
		{
			bSlide = (m_pDAPlayer->GetAbsVelocity().Length() > 10.0f) && (mv->m_nButtons & (IN_BACK|IN_FORWARD|IN_MOVELEFT|IN_MOVERIGHT)) &&
				...
				(m_pDAPlayer->GetCurrentTime() - m_pDAPlayer->m_Shared.GetSlideStartTime() > TIME_TO_RESLIDE);

			bDive = (m_pDAPlayer->GetAbsVelocity().Length() > 10.0f);
		}

		if (!bRoll && m_pDAPlayer->m_Shared.IsProne())
		{
			if (buttonsPressed&(IN_BACK|IN_FORWARD|IN_MOVELEFT|IN_MOVERIGHT|IN_ALT1))
			{
				if (m_pDAPlayer->m_Shared.GetDiveToProneLandTime() > 0 && m_pDAPlayer->GetCurrentTime() - m_pDAPlayer->m_Shared.GetDiveToProneLandTime() < da_d2p_stunt_forgiveness.GetFloat())
				{
		...
		if( bGoProne && m_pDAPlayer->m_Shared.IsProne() == false && m_pDAPlayer->m_Shared.IsGettingUpFromProne() == false &&
			!m_pDAPlayer->m_Shared.IsSliding() && !m_pDAPlayer->m_Shared.IsRolling() )
		{
			m_pDAPlayer->m_Shared.StartGoingProne();

			//Tony; here is where you'd want to do an animation for first person to give the effect of going prone.
			if ( m_pDAPlayer->m_Shared.IsDucking() )
				m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_CROUCH_TO_PRONE );
			else
				m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_STAND_TO_PRONE );

			return;
		}
		else if ( bGetUpFromProne && m_pDAPlayer->m_Shared.IsProne() && CanUnprone() )
		{
			m_pDAPlayer->m_Shared.SetProne( false );
			m_pDAPlayer->m_Shared.StandUpFromProne();
			...
			m_pDAPlayer->m_bUnProneToDuck = !CanUnduck() || (( mv->m_nButtons & IN_DUCK ) > 0);
		...
		else if (bGetUp && m_pDAPlayer->m_Shared.IsSliding() )
		{
			bool bStandingRoom = CanUnduck();
			bool bCanJumpUp = ( bStandingRoom && !m_pDAPlayer->m_Shared.IsDiveSliding() );
			...
			m_pDAPlayer->m_Shared.StandUpFromSlide( bCanJumpUp );
			SetUnSlideEyeOffset( 0.0 );

			return;
		}
		else if( bRoll && m_pDAPlayer->m_Shared.CanRoll() )
		{
			m_pDAPlayer->m_Shared.SetProne( false );
			m_pDAPlayer->m_Shared.StartRolling(bFromDive);

			SetRollEyeOffset( 0.0 );

			m_pDAPlayer->DoAnimationEvent( bFromDive?PLAYERANIMEVENT_DIVE_TO_ROLL:PLAYERANIMEVENT_STAND_TO_ROLL );

			CPASFilter filter( m_pDAPlayer->GetAbsOrigin() );
			filter.UsePredictionRules();
			m_pDAPlayer->EmitSound( filter, m_pDAPlayer->entindex(), "Player.GoRoll" );
		}
		else if( bSlide && m_pDAPlayer->m_Shared.CanSlide() )
		{
			m_pDAPlayer->m_Shared.SetProne( false );

			Assert (m_pDAPlayer->m_Shared.m_flRunSpeed);
			if (fabs (m_pDAPlayer->m_Shared.m_flRunSpeed) > 1e-5)
			{
				float flSpeedFraction = RemapValClamped(m_pDAPlayer->GetAbsVelocity().Length()/m_pDAPlayer->m_Shared.m_flRunSpeed, 0, 1, 0.2f, 1);

				if (bFromDive)
				{
					flSpeedFraction = RemapValClamped(m_pDAPlayer->GetAbsVelocity().Length()/da_dive_speed.GetFloat(), 0, 1, 0.2f, 1);
				...
				m_pDAPlayer->m_Shared.StartSliding(bFromDive);
				...
				if (!bFromDive)
					m_pDAPlayer->DoAnimationEvent( PLAYERANIMEVENT_STAND_TO_SLIDE );
			}
		}
	}

	if ( m_pDAPlayer->m_Shared.IsProne() &&
		m_pDAPlayer->m_Shared.CanChangePosition() &&
		( buttonsPressed & IN_DUCK ) && 
		CanUnprone() )	
	{
		m_pDAPlayer->m_Shared.SetProne( false );
	...
	if (m_pDAPlayer->m_Shared.IsRolling())
		return;
	...
	if (m_pDAPlayer->m_Shared.IsWallFlipping())
		return;

	if (m_pDAPlayer->m_Shared.IsProne() && !CanUnprone())
		return;
...
//-----------------------------------------------------------------------------
unsigned int CDAGameMovement::PlayerSolidMask( bool brushOnly )
{
	int mask = 0;
...
// Output : const Vector
//-----------------------------------------------------------------------------
Vector CDAGameMovement::GetPlayerMins( void ) const
{
	if ( !player )
	{
		return vec3_origin;
	...
	else if ( m_pDAPlayer->m_Shared.m_bIsTryingUnduck )
		return VEC_HULL_MIN;
	else if ( m_pDAPlayer->m_Shared.m_bIsTryingUnprone )
	...
	else
	{
		...
		else if (m_pDAPlayer->m_Shared.IsWallFlipping())
			return VEC_DIVE_HULL_MIN;
#if defined ( SDK_USE_PRONE )
		else if ( m_pDAPlayer->m_Shared.IsProne() )
...
// Output : const Vector
//-----------------------------------------------------------------------------
Vector CDAGameMovement::GetPlayerMaxs( void ) const
{	
	if ( !player )
	{
		return vec3_origin;
...
Vector CDAGameMovement::GetPlayerViewOffset( bool ducked ) const
{
	return BaseClass::GetPlayerViewOffset(ducked);
}

inline void CDAGameMovement::TraceBBox (const Vector& start, const Vector& end, const Vector &mins, const Vector &maxs, trace_t &pm)
{
	Ray_t ray;
...
bool CDAGameMovement::CheckMantel()
{
	trace_t tr;
	Vector vecMins, vecMaxs;
	Vector pr1, pr2;
	...
	if (m_pDAPlayer->m_Shared.IsManteling())
		return true;
	...
	m_pDAPlayer->m_Shared.StartManteling(tr.plane.normal);
	m_pDAPlayer->SetViewOffset( GetPlayerViewOffset( false ) );

	return true;
}

void CDAGameMovement::AccumulateWallFlipTime()
{
	if (!m_pDAPlayer->m_Shared.IsWallFlipping())
		return;

	float flRaiseAmount = (GetPlayerViewOffset( false ).z - GetPlayerViewOffset( true ).z)/ConVarRef("da_acro_wallflip_delay").GetFloat();
	m_pDAPlayer->SetViewOffset( Vector(0, 0, Approach(GetPlayerViewOffset( false ).z, m_pDAPlayer->GetViewOffset().z, gpGlobals->frametime*flRaiseAmount)) );

	if (m_pDAPlayer->GetCurrentTime() < m_pDAPlayer->m_Shared.GetWallFlipEndTime())
		return;
	...
	m_pDAPlayer->SetViewOffset( GetPlayerViewOffset( false ) );
	m_pDAPlayer->m_Shared.EndWallFlip();
}
...
void CDAGameMovement::FullWalkMove ()
{
	...
	if (m_pDAPlayer->m_Shared.runtime < 0)
	{
		if (!(m_pDAPlayer->m_Shared.daflags&DA_WRLOCK))
		{
			m_pDAPlayer->m_Shared.runtime += 1000*gpGlobals->frametime;
			if (m_pDAPlayer->m_Shared.runtime > 0)
			{
				m_pDAPlayer->m_Shared.runtime = 0;
			}
		}
	}
#endif
	...
	if (m_pDAPlayer->m_Shared.CanChangePosition ())
	{
		...
		if (pressed&IN_ALT1)
		{
			...
			if (m_pDAPlayer->m_Shared.m_iStyleSkill == SKILL_ATHLETIC)
				iWallflips = da_athletic_wallflip_limit.GetInt();
			if (m_pDAPlayer->IsStyleSkillActive(SKILL_ATHLETIC))
				iWallflips = 9999;

			if (!player->m_Local.m_bDucked && !m_pDAPlayer->m_Shared.IsProne() && m_pDAPlayer->m_Shared.GetWallFlipCount() < iWallflips)
			{
				trace_t tr;
			...
			if (m_pDAPlayer->GetAbsVelocity().Length() > 10.0f &&
				!m_pDAPlayer->m_Shared.IsWallFlipping() &&
				m_pDAPlayer->m_Shared.CanDive ())
			{
#if 0
				if (m_pDAPlayer->m_Shared.runtime > 0)
				{/*Project speed onto forward axis before dive, 
					...
					m_pDAPlayer->m_Shared.runtime = -1;
				}
#endif
				mv->m_vecVelocity = m_pDAPlayer->m_Shared.StartDiving();
	...
	{/*Wallrunning*/
		...
		VectorCopy (GetPlayerMaxs (), maxs);
		VectorCopy (mv->GetAbsOrigin (), org);
		VectorCopy (m_pDAPlayer->m_Shared.rundir, dir);
		...
		{/*Must have no obstructions*/
			...
			VectorScale (m_pDAPlayer->m_Shared.wallnormal, 8, right);
			...
			{/*Run right up along the wall*/
				...
				forward = m_pDAPlayer->m_Shared.rundir;
				d = da_acro_wallrun_duration.GetFloat ();
				t = m_pDAPlayer->m_Shared.runtime/d;
				...
				Vector &n = m_pDAPlayer->m_Shared.wallnormal.GetForModify ();
				...
				Vector &nr = m_pDAPlayer->m_Shared.rundir.GetForModify ();
				nr[0] = s*run[0];
				nr[1] = s*run[1];
				nr[2] = nr[2];
	...
	if (CheckMantel())
	{
		...
		if (!(mv->m_nButtons&IN_JUMP))
		{
			m_pDAPlayer->m_Shared.ResetManteling();
			return;
		}
		VectorCopy (GetPlayerMins (), mins);
		VectorCopy (GetPlayerMaxs (), maxs);
		...
		if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid)
		{
			...
			if (m_pDAPlayer->m_Shared.IsManteling())
			{
				...
				if (!tr.DidHit ())
				{
					mv->SetAbsOrigin (tr.endpos);
					m_pDAPlayer->m_Shared.ResetManteling();
					m_pDAPlayer->Instructor_LessonLearned("mantel");
				}
				else
					SetGroundEntity(&tr);
			}
	...
	if (m_pDAPlayer->m_Shared.IsDiving() && !m_pDAPlayer->GetGroundEntity ())
	{/*Raise player off the ground*/
		if (m_pDAPlayer->m_Shared.GetDiveLerped() < 1)
		{
			trace_t trace;
			float flDiveLerpTime = DIVE_RISE_TIME;
			float flBiasAmount = 0.7f;
			float flCurrentLerp = RemapValClamped(m_pDAPlayer->GetCurrentTime(), m_pDAPlayer->m_Shared.GetDiveTime(), m_pDAPlayer->m_Shared.GetDiveTime() + flDiveLerpTime, 0, 1);
			float flCurrentBias = Bias(flCurrentLerp, flBiasAmount);
			float flLastTimeBias = Bias(m_pDAPlayer->m_Shared.GetDiveLerped(), flBiasAmount);
			...
			m_pDAPlayer->m_Shared.IncreaseDiveLerped(flCurrentLerp - m_pDAPlayer->m_Shared.GetDiveLerped());
			Vector vecViewOffset = Lerp(flCurrentBias, GetPlayerViewOffset(false), VEC_DIVE_VIEW);
			m_pDAPlayer->SetViewOffset( vecViewOffset );
		}
		else m_pDAPlayer->SetViewOffset( VEC_DIVE_VIEW );
	}
	...
	/*The great door hack*/
	{
		...
		dt = slop*m_pDAPlayer->GetSlowMoMultiplier ();
...
void CDAGameMovement::StepMove( Vector &vecDestination, trace_t &trace )
{
	Vector vecEndPos;
...
bool CDAGameMovement::PlayerIsStuck()
{
	trace_t traceresult;
...
#ifdef STUCK_DEBUG
void CDAGameMovement::AddBotTag(const char* tag)
{
#ifndef CLIENT_DLL
	CDABot* pBot = dynamic_cast<CDABot*>(m_pDAPlayer);
	...
	pBot->m_button_history.push_back(CDABot::ButtonHistory());
