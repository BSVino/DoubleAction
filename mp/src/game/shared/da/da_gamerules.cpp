...
REGISTER_GAMERULES_CLASS( CDAGameRules );


BEGIN_NETWORK_TABLE_NOBASE( CDAGameRules, DT_DAGameRules )
...
LINK_ENTITY_TO_CLASS( da_gamerules, CDAGameRulesProxy );
IMPLEMENT_NETWORKCLASS_ALIASED( DAGameRulesProxy, DT_DAGameRulesProxy )


#ifdef CLIENT_DLL
	void RecvProxy_DAGameRules( const RecvProp *pProp, void **pOut, void *pData, int objectID )
	{
		CDAGameRules *pRules = DAGameRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( CDAGameRulesProxy, DT_DAGameRulesProxy )
		RecvPropDataTable( "da_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_DAGameRules ), RecvProxy_DAGameRules )
	END_RECV_TABLE()
#else
	void *SendProxy_DAGameRules( const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID )
	{
		CDAGameRules *pRules = DAGameRules();
		Assert( pRules );
	...
	BEGIN_SEND_TABLE( CDAGameRulesProxy, DT_DAGameRulesProxy )
		SendPropDataTable( "da_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_DAGameRules ), SendProxy_DAGameRules )
	END_SEND_TABLE()
#endif
...
const CViewVectors* CDAGameRules::GetViewVectors() const
{
	return (CViewVectors*)GetSDKViewVectors();
}

const CSDKViewVectors *CDAGameRules::GetSDKViewVectors() const
{
...
// --------------------------------------------------------------------------------------------------- //
// CDAGameRules implementation.
// --------------------------------------------------------------------------------------------------- //
extern ConVar teamplay;

CDAGameRules::CDAGameRules()
{
...
void CDAGameRules::LevelInitPostEntity()
{
...
bool CDAGameRules::ClientConnected( edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject, int maxrejectlen )
{
...
void CDAGameRules::ClientDisconnected( edict_t *pClient )
{
	BaseClass::ClientDisconnected(pClient);

	CoderHacksUpdate();

	CDAPlayer* pDAPlayer = ToDAPlayer(CBaseEntity::Instance( pClient ));
	pDAPlayer->DropBriefcase();

	if (pDAPlayer == GetBountyPlayer())
		CleanupMiniObjective();

#ifdef WITH_DATA_COLLECTION
	if (!pDAPlayer->IsBot())
		DataManager().ClientDisconnected(engine->GetClientSteamID(pDAPlayer->edict())->GetAccountID());
#endif
}

bool CDAGameRules::IsConnectedUserInfoChangeAllowed( CBasePlayer *pPlayer )
{
	return true;
}

void CDAGameRules::CoderHacksUpdate()
{
...
void CDAGameRules::ReCalculateSlowMo()
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		CDAPlayer* pDAPlayer = static_cast<CDAPlayer*>(pPlayer);

		if (pDAPlayer->GetSlowMoType() == SLOWMO_PASSIVE || pDAPlayer->GetSlowMoType() == SLOWMO_PASSIVE_SUPER)
			pDAPlayer->SetSlowMoType(SLOWMO_NONE);
	}
...
#ifdef SLOWMO_DEBUG
		if (pDAPlayer->IsBot())
			GiveSlowMoToNearbyPlayers(pDAPlayer);
		else
#endif

		if (pDAPlayer->GetSlowMoType() == SLOWMO_NONE)
			CalculateSlowMoForPlayer(pDAPlayer);
		else
			GiveSlowMoToNearbyPlayers(pDAPlayer);
	}
...
void CDAGameRules::CalculateSlowMoForPlayer(CDAPlayer* pPlayer)
{
	if (!pPlayer)
		return;
	...
	CUtlVector<CDAPlayer*> apOthersInPVS;

	CBaseEntity* pOther = NULL;

	while ((pOther = UTIL_EntitiesInPVS(pPlayer, pOther)) != NULL)
	{
		CDAPlayer* pOtherPlayer = ToDAPlayer(pOther);
		if (!pOtherPlayer)
			continue;
	...
	{
		CDAPlayer* pOtherPlayer = apOthersInPVS[i];
...
void CDAGameRules::PlayerSlowMoUpdate(CDAPlayer* pPlayer)
{
	if (!pPlayer)
		return;
...
void CDAGameRules::GiveSlowMoToNearbyPlayers(CDAPlayer* pPlayer)
{
	if (!pPlayer)
		return;
...
void CDAGameRules::GoToIntermission( void )
{
...
void CDAGameRules::CheckLevelInitialized()
{
...
// Output :
//-----------------------------------------------------------------------------
bool CDAGameRules::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
{
	CDAPlayer *pPlayer = ToDAPlayer( pEdict );
#if 0
...
void CDAGameRules::RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore )
{
...
void CDAGameRules::RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore, CBaseEntity *pEntityIgnore )
{
...
void CDAGameRules::Think()
{
	...
	{
		if (!GetGlobalDATeam(i))
			continue;

		int iStyle = 0;

		for (int j = 0; j < GetGlobalDATeam(i)->GetNumPlayers(); j++)
		{
			if (!GetGlobalDATeam(i)->GetPlayer(j))
				continue;

			iStyle += ToDAPlayer(GetGlobalDATeam(i)->GetPlayer(j))->GetTotalStyle();
		}

		GetGlobalDATeam(i)->SetScore(iStyle);
	}
...
void CDAGameRules::EndGameFrame( void )
{
	Bot_RunAll();
...
CBaseEntity *CDAGameRules::GetPlayerSpawnSpot( CBasePlayer *pPlayer )
{
...
bool CDAGameRules::IsSpawnPointValid( CBaseEntity *pSpot, CBasePlayer *pPlayer )
{
	...
	for (int i = 0; i <= gpGlobals->maxClients; i++)
	{
		...
		CDAPlayer* pOtherDAPlayer = ToDAPlayer(pOtherPlayer);

		if (pOtherPlayer == pPlayer)
			continue;

		if (!pOtherDAPlayer->IsAlive())
			continue;
		...
		trace_t tr;
		UTIL_TraceLine( pSpot->WorldSpaceCenter(), pOtherDAPlayer->WorldSpaceCenter(), MASK_VISIBLE_AND_NPCS, pPlayer, COLLISION_GROUP_NONE, &tr );
...
bool CDAGameRules::InitTeamSpawns()
{	
...
void CDAGameRules::PlayerSpawn( CBasePlayer *p )
{	
	CDAPlayer *pPlayer = ToDAPlayer( p );
	...
	if( team != TEAM_SPECTATOR )
	{
		...
		if( playerclass != PLAYERCLASS_UNDEFINED )
		{
			...
			CDATeam *pTeam = GetGlobalDATeam( team );
			const CDAPlayerClassInfo &pClassInfo = pTeam->GetPlayerClassInfo( playerclass );
			...
			CWeaponDABase *pWpn = NULL;

			// Primary Ammo
			pWpn = dynamic_cast<CWeaponDABase *>(pPrimaryWpn);
			...
			if ( pSecondaryWpn )
			{
				pWpn = dynamic_cast<CWeaponDABase *>(pSecondaryWpn);
			...
			if ( pClassInfo.m_iGrenType1 != WEAPON_NONE )
			{
				...
				pWpn = dynamic_cast<CWeaponDABase *>(pGrenade);
			...
			if ( pClassInfo.m_iGrenType2 != WEAPON_NONE )
			{
				...
				pWpn = dynamic_cast<CWeaponDABase *>(pGrenade2);
...
bool CDAGameRules::CanHavePlayerItem( CBasePlayer *pPlayer, CBaseCombatWeapon *pItem )
{
	CWeaponDABase* pWeapon = dynamic_cast<CWeaponDABase*>(pItem);
	CDAPlayer* pDAPlayer = ToDAPlayer(pPlayer);

	if (pWeapon && pWeapon->GetWeaponID() == DA_WEAPON_GRENADE && pDAPlayer->m_Shared.m_iStyleSkill != SKILL_TROLL)
	{
		// If the player doesn't have Nitrophiliac, max 2 grenades.
		if (pDAPlayer->GetAmmoCount("grenades") >= da_max_grenades.GetInt())
			return false;
	}
...
#if defined ( SDK_USE_PLAYERCLASSES )
void CDAGameRules::ChooseRandomClass( CDAPlayer *pPlayer )
{
	int i;
	int numChoices = 0;
	int choices[16];
	...
	CDATeam *pTeam = GetGlobalDATeam( pPlayer->GetTeamNumber() );
...
bool CDAGameRules::CanPlayerJoinClass( CDAPlayer *pPlayer, int cls )
{
...
bool CDAGameRules::ReachedClassLimit( int team, int cls )
{
...
int CDAGameRules::CountPlayerClass( int team, int cls )
{
	int num = 0;
	CDAPlayer *pDAPlayer;
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		pDAPlayer = ToDAPlayer( UTIL_PlayerByIndex( i ) );

		if (pDAPlayer == NULL)
			continue;

		if (FNullEnt( pDAPlayer->edict() ))
			continue;

		if( pDAPlayer->GetTeamNumber() != team )
			continue;

		if( pDAPlayer->m_Shared.DesiredPlayerClass() == cls )
			num++;
	}
...
int CDAGameRules::GetClassLimit( int team, int cls )
{
	CDATeam *pTeam = GetGlobalDATeam( team );

	Assert( pTeam );

	const CDAPlayerClassInfo &pClassInfo = pTeam->GetPlayerClassInfo( cls );
...
bool CDAGameRules::IsPlayerClassOnTeam( int cls, int team )
{
	if( cls == PLAYERCLASS_RANDOM )
		return true;
...
void CDAGameRules::InitTeams( void )
{
	...
	CTeam *pUnassigned = static_cast<CTeam*>(CreateEntityByName( "da_team_unassigned" ));
	Assert( pUnassigned );
	...
	CTeam *pSpectator = static_cast<CTeam*>(CreateEntityByName( "da_team_manager" ));
	Assert( pSpectator );
	...
	if( IsTeamplay() )
	{
		...
		CTeam *pBlue = static_cast<CTeam*>(CreateEntityByName( "da_team_blue" ));
		Assert( pBlue );
		pBlue->Init( pszTeamNames[SDK_TEAM_BLUE], SDK_TEAM_BLUE );
		g_Teams.AddToTail( pBlue );

		//Tony; create the red team
		CTeam *pRed = static_cast<CTeam*>(CreateEntityByName( "da_team_red" ));
		Assert( pRed );
	...
	CTeam *pDeathmatch = static_cast<CTeam*>(CreateEntityByName( "da_team_deathmatch" ));
	Assert( pDeathmatch );
...
void CDAGameRules::CreateStandardEntities()
{
	// Create the player resource
	g_pPlayerResource = (CPlayerResource*)CBaseEntity::Create( "da_player_manager", vec3_origin, vec3_angle );

	// Create the entity that will send our data to the client.
#ifdef _DEBUG
	CBaseEntity *pEnt = 
#endif
		CBaseEntity::Create( "da_gamerules", vec3_origin, vec3_angle );
	Assert( pEnt );
}
int CDAGameRules::SelectDefaultTeam()
{
	...
	if (gpGlobals->teamplay)
	{
		CDATeam *pBlue = GetGlobalDATeam(SDK_TEAM_BLUE);
		CDATeam *pRed = GetGlobalDATeam(SDK_TEAM_RED);
...
bool CDAGameRules::TeamFull( int team_id )
{
	switch ( team_id )
	{
	case SDK_TEAM_BLUE:
		{
			//int iNumBlue = GetGlobalDATeam(SDK_TEAM_BLUE)->GetNumPlayers();
			return false;//iNumBlue >= m_iSpawnPointCount_Blue;
		}
	case SDK_TEAM_RED:
		{
			//int iNumRed = GetGlobalDATeam(SDK_TEAM_RED)->GetNumPlayers();
...
bool CDAGameRules::TeamStacked( int iNewTeam, int iCurTeam  )
{
...
//-----------------------------------------------------------------------------
const char *CDAGameRules::GetKillingWeaponName( const CTakeDamageInfo &info, CDAPlayer *pVictim, int *iWeaponID )
{
	CBaseEntity *pInflictor = info.GetInflictor();
	CBaseEntity *pKiller = info.GetAttacker();
	CBasePlayer *pScorer = DAGameRules()->GetDeathScorer( pKiller, pInflictor, pVictim );

	const char *killer_weapon_name = "world";
	*iWeaponID = DA_WEAPON_NONE;
	...
	if ( pScorer && pInflictor && ( pInflictor == pScorer ) )
	{
		...
		if ( pScorer->GetActiveWeapon() )
		{
			...
			if ( pScorer->IsPlayer() )
			{
				*iWeaponID = ToDAPlayer(pScorer)->GetActiveDAWeapon()->GetWeaponID();
			}
		}
	}
	else if ( pInflictor )
	{
		...
		CWeaponDABase *pWeapon = dynamic_cast< CWeaponDABase * >( pInflictor );
		if ( pWeapon )
		{
...
void CDAGameRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	if (pVictim && pVictim == GetBountyPlayer())
	{
		CDAPlayer::SendBroadcastSound("MiniObjective.BountyKilled");
		...
		{
			CDAPlayer* pPlayerKiller = ToDAPlayer(info.GetAttacker());
			if (pPlayerKiller)
			{
				GiveMiniObjectiveRewardPlayer(pPlayerKiller);

				CDAPlayer::SendBroadcastNotice(NOTICE_BOUNTY_COLLECTED, pPlayerKiller);
			}
			else
				CDAPlayer::SendBroadcastNotice(NOTICE_BOUNTY_LOST);

			CleanupMiniObjective();
		}
	}
	else if (GetBountyPlayer() && GetBountyPlayer() == ToDAPlayer(info.GetAttacker()))
	{
		GetBountyPlayer()->TakeHealth(25, 0);
	}

	CDAPlayer* pLeader = GetLeader();

	RemovePlayerFromLeaders(m_ahWaypoint1RaceLeaders, ToDAPlayer(pVictim));
	RemovePlayerFromLeaders(m_ahWaypoint2RaceLeaders, ToDAPlayer(pVictim));

	CDAPlayer* pNewLeader = GetLeader();

	if (pNewLeader && pLeader && pNewLeader != pLeader)
	{
		CDAPlayer::SendBroadcastNotice(NOTICE_RATRACE_PLAYER_LEAD, pNewLeader);
...
//			*pInflictor - 
//-----------------------------------------------------------------------------
void CDAGameRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	int killer_ID = 0;

	// Find the killer & the scorer
	CDAPlayer *pDAPlayerVictim = ToDAPlayer( pVictim );
	CBaseEntity *pInflictor = info.GetInflictor();
	CBaseEntity *pKiller = info.GetAttacker();
	CBasePlayer *pScorer = GetDeathScorer( pKiller, pInflictor, pVictim );
//	CDAPlayer *pAssister = ToDAPlayer( GetAssister( pVictim, pScorer, pInflictor ) );

	// Work out what killed the player, and send a message to all clients about it
	int iWeaponID;
	const char *killer_weapon_name = GetKillingWeaponName( info, pDAPlayerVictim, &iWeaponID );
...
bool CDAGameRules::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
...
#if defined ( SDK_USE_PLAYERCLASSES )
const char *CDAGameRules::GetPlayerClassName( int cls, int team )
{
	CDATeam *pTeam = GetGlobalDATeam( team );
...
float CDAGameRules::FlPlayerFallDamage( CBasePlayer *pPlayer )
{
	CDAPlayer* pDAPlayer = ToDAPlayer(pPlayer);

	float flFallVelocity = pDAPlayer->m_Local.m_flFallVelocity;
	float flMaxSafeFallSpeed = 700;
	float flMaxSafeDiveFallSpeed = 800;

	float flExcessSpeed = flFallVelocity - flMaxSafeFallSpeed;

	if (pDAPlayer->m_Shared.IsDiving())
...
void CDAGameRules::OverrideSoundParams( const EmitSound_t& ep, CSoundParameters& oParams )
{
#ifdef CLIENT_DLL
...
CAmmoDef* GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType( "drugs",    DMG_BLAST,    TRACER_NONE,          0, 0, 2,             1,                           0 );
	}
...
const char *CDAGameRules::GetChatPrefix( bool bTeamOnly, CBasePlayer *pPlayer )
{
	//Tony; no prefix for now, it isn't needed.
	return "";
}

const char *CDAGameRules::GetChatFormat( bool bTeamOnly, CBasePlayer *pPlayer )
{
...
//-----------------------------------------------------------------------------
int CDAGameRules::PlayerRelationship( CBaseEntity *pPlayer, CBaseEntity *pTarget )
{
	if (pPlayer == pTarget)
		return GR_TEAMMATE;
...
float CDAGameRules::GetMapRemainingTime()
{
...
float CDAGameRules::GetMapElapsedTime( void )
{
	return gpGlobals->curtime;
}

Vector CDAGameRules::GetLowestSpawnPoint()
{
	return m_vecLowestSpawnPoint;
}

#ifndef CLIENT_DLL
void CDAGameRules::StartMiniObjective(const char* pszObjective)
{
	CleanupMiniObjective();
	...
	CDAPlayer::SendBroadcastNotice(GetNoticeForMiniObjective(m_eCurrentMiniObjective));

	CDAPlayer::SendBroadcastSound("MiniObjective.Begin");
}

notice_t CDAGameRules::GetNoticeForMiniObjective(miniobjective_t eObjective)
{
	if (eObjective == MINIOBJECTIVE_BRIEFCASE)
...
void CDAGameRules::MaintainMiniObjective()
{
	if (!m_eCurrentMiniObjective)
		return;
...
void CDAGameRules::CleanupMiniObjective()
{
	if (!m_eCurrentMiniObjective)
		return;
...
void CDAGameRules::RestartMiniObjective()
{
...
void CDAGameRules::GiveMiniObjectiveRewardTeam(CDAPlayer* pTeam)
{
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		CDAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(i));
...
bool CDAGameRules::SetupMiniObjective_Briefcase()
{
...
void CDAGameRules::MaintainMiniObjective_Briefcase()
{
	if (!m_hBriefcase)
	{
		CleanupMiniObjective();
	...
	{
		if (m_hBriefcase->GetAbsOrigin().z < DAGameRules()->GetLowestSpawnPoint().z - 400)
			DAGameRules()->RestartMiniObjective();
		else
			CleanupMiniObjective();
		return;
	}
...
void CDAGameRules::CleanupMiniObjective_Briefcase()
{
	UTIL_Remove(m_hBriefcase);
	UTIL_Remove(m_hCaptureZone);
}

bool CDAGameRules::ChooseRandomCapturePoint(Vector vecBriefcaseLocation)
{
...
void CDAGameRules::PlayerCapturedBriefcase(CDAPlayer* pPlayer)
{
	if (pPlayer)
	{
		GiveMiniObjectiveRewardTeam(pPlayer);

		CDAPlayer::SendBroadcastNotice(NOTICE_PLAYER_CAPTURED_BRIEFCASE, pPlayer);
		CDAPlayer::SendBroadcastSound("MiniObjective.BriefcaseCapture");
	}

	CleanupMiniObjective();
}

int PlayerKDCompare(CDAPlayer*const* l, CDAPlayer*const* r)
{
...
bool CDAGameRules::SetupMiniObjective_Bounty()
{
	...
	RandomSeed((int)(gpGlobals->curtime*1000));
	CDAPlayer* pChosen = apPlayers[RandomInt(0, apPlayers.Count()-1)];
	...
	if (IsTeamplay())
	{
		...
		CDAPlayer::SendBroadcastNotice(NOTICE_BOUNTY_ON_PLAYER, pChosen);

	return true;
}

void CDAGameRules::MaintainMiniObjective_Bounty()
{
	if (!m_hBountyPlayer)
	{
		CleanupMiniObjective();
...
void CDAGameRules::CleanupMiniObjective_Bounty()
{
...
bool CDAGameRules::SetupMiniObjective_RatRace()
{
	if (IsTeamplay())
		return false;
...
void CDAGameRules::MaintainMiniObjective_RatRace()
{
	if (gpGlobals->curtime - m_flLastPlayerWaypointTouch > 120)
		CleanupMiniObjective();
}

void CDAGameRules::CleanupMiniObjective_RatRace()
{
...
void CDAGameRules::PlayerReachedWaypoint(CDAPlayer* pPlayer, CRatRaceWaypoint* pWaypoint)
{
...
template <typename T>
void CDAGameRules::WaypointLeadersPush(T& ahWaypointLeaders, CDAPlayer* pPlayer)
{
	...
	// CDAPlayer::m_iRaceWaypoint is what determines the winner.
	DebugCheckLeaders(ahWaypointLeaders);
}

template <typename T>
void CDAGameRules::RemovePlayerFromLeaders(T& ahWaypointLeaders, CDAPlayer* pPlayer)
{
	for (int i = 0; i < ahWaypointLeaders.Count(); i++)
	{
...
template <typename T>
void CDAGameRules::CompressLeaders(T& ahWaypointLeaders)
{
	for (int i = 0; i < ahWaypointLeaders.Count(); i++)
	{
		if (ahWaypointLeaders.Get(i) == NULL)
...
template <typename T>
void CDAGameRules::DebugCheckLeaders(T& ahWaypointLeaders)
{
#ifndef _DEBUG
	return;
#endif
...
CBriefcase* CDAGameRules::GetBriefcase() const
{
	return m_hBriefcase;
}

CBriefcaseCaptureZone* CDAGameRules::GetCaptureZone() const
{
	return m_hCaptureZone;
}

CDAPlayer* CDAGameRules::GetBountyPlayer() const
{
	return m_hBountyPlayer;
}

CRatRaceWaypoint* CDAGameRules::GetWaypoint(int i) const
{
	if (i == 1)
...
CDAPlayer* CDAGameRules::GetLeader() const
{
...
CDAPlayer* CDAGameRules::GetFrontRunner1() const
{
	if (m_ahWaypoint2RaceLeaders[0])
	{
...
CDAPlayer* CDAGameRules::GetFrontRunner2() const
{
	if (m_ahWaypoint2RaceLeaders[0])
	{
		// If someone has reached the second point, then they are the leader.
...
public:
	...
	virtual bool        GetVoteOptions( CUtlVector <const char*> &vecNames )
	{
		...
		apszMapList.AddVectorToTail(DAGameRules()->GetMapList());

		static char szNextMap[MAX_MAP_NAME];
		DAGameRules()->GetNextLevelName( szNextMap, sizeof( szNextMap ) );
