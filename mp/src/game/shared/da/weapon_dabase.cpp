...
// ----------------------------------------------------------------------------- //
// CWeaponDABase tables.
// ----------------------------------------------------------------------------- //

IMPLEMENT_NETWORKCLASS_ALIASED( WeaponDABase, DT_WeaponDABase )

BEGIN_NETWORK_TABLE( CWeaponDABase, DT_WeaponDABase )
#ifdef CLIENT_DLL
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CWeaponDABase )
...
LINK_ENTITY_TO_CLASS( weapon_da_base, CWeaponDABase );
...
#ifdef CLIENT_DLL
bool CWeaponDABase::ShouldPredict()
{
...
const CSDKWeaponInfo &CWeaponDABase::GetSDKWpnData() const
{
...
bool CWeaponDABase::IsAkimbo() const
{
	return !FStrEq(GetSDKWpnData().m_szSingle, "");
}

bool CWeaponDABase::HasAkimbo() const
{
	return !FStrEq(GetSDKWpnData().m_szAkimbo, "");
}

bool CWeaponDABase::IsGrenade() const
{
	return GetSDKWpnData().m_eWeaponType == WT_GRENADE;
}

const char* CWeaponDABase::GetViewModel( int viewmodelindex ) const
{
	...
	if (IsThrowingGrenade() && GetCurrentTime() > GetGrenadeThrowWeaponHolsterTime() && GetCurrentTime() < GetGrenadeThrowWeaponDeployTime() && pPlayer && !pPlayer->IsInThirdPerson())
		return CSDKWeaponInfo::GetWeaponInfo(DA_WEAPON_GRENADE)->szViewModel;

	return BaseClass::GetViewModel(viewmodelindex);
}

bool CWeaponDABase::PlayEmptySound()
{
...
CDAPlayer* CWeaponDABase::GetPlayerOwner() const
{
	return dynamic_cast< CDAPlayer* >( GetOwner() );
}
...
void CWeaponDABase::FinishAttack (CDAPlayer *pPlayer)
{
#ifdef GAME_DLL
...
void CWeaponDABase::PrimaryAttack( void )
{
...
void CWeaponDABase::SecondaryAttack()
{
...
void CWeaponDABase::StartSwing(bool bIsSecondary, bool bIsStockAttack)
{
	// Try a ray
...
void CWeaponDABase::Swing()
{
	...
	CTakeDamageInfo triggerInfo( GetOwner(), GetOwner(), GetMeleeDamage( m_bSwingSecondary, ToDAPlayer(traceHit.m_pEnt) ), DMG_CLUB );
...
Activity CWeaponDABase::ChooseIntersectionPointAndActivity( trace_t &hitTrace, const Vector &mins, const Vector &maxs, CDAPlayer *pOwner )
{
	int			i, j, k;
	float		distance;
...
void CWeaponDABase::Hit( trace_t &traceHit, bool bIsSecondary )
{
	CDAPlayer *pPlayer = ToDAPlayer( GetOwner() );
...
//	AddViewKick();
...
#ifndef CLIENT_DLL
		float flDamage = GetMeleeDamage( bIsSecondary, ToDAPlayer(pHitEntity) );
	...
	{
		CDAPlayer* pVictim = ToDAPlayer(pHitEntity);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponDABase::ImpactEffect( trace_t &traceHit )
{
...
bool CWeaponDABase::ImpactWater( const Vector &start, const Vector &end )
{
...
float CWeaponDABase::GetMeleeDamage( bool bIsSecondary, CDAPlayer* pVictim ) const
{
	CDAPlayer *pPlayer = ToDAPlayer( GetOwner() );
	...
	//bool bIsStockAttack = pPlayer && pPlayer->GetActiveDAWeapon() && !pPlayer->GetActiveDAWeapon()->IsMeleeWeapon();
	...
	if (pVictim)
	{
		...
		// Attacks to the rear do more damage.
...
float CWeaponDABase::GetBrawlFireRate()
{
...
float CWeaponDABase::GetBrawlSecondaryFireRate()
{
...
void CWeaponDABase::StartGrenadeToss()
{
...
bool CWeaponDABase::MaintainGrenadeToss()
{
...
float CWeaponDABase::GetGrenadeThrowWeaponHolsterTime() const
{
	Assert(IsThrowingGrenade());
	return m_flGrenadeThrowStart + da_grenade_weaponlerp_time.GetFloat();
}

float CWeaponDABase::GetGrenadeThrowWeaponDeployTime() const
{
	Assert(IsThrowingGrenade());
	return m_flGrenadeThrowStart + da_grenade_throw_time.GetFloat() - da_grenade_weaponlerp_time.GetFloat();
}

float CWeaponDABase::GetGrenadeThrowEnd() const
{
	Assert(IsThrowingGrenade());
	return m_flGrenadeThrowStart + da_grenade_throw_time.GetFloat();
}

void CWeaponDABase::GetGrenadeThrowVectors(Vector& vecSrc, Vector& vecThrow, QAngle& angThrow)
{
	CDAPlayer* pPlayer = ToDAPlayer(GetOwner());
	Assert(pPlayer);
	if (!pPlayer)
...
void CWeaponDABase::AddMeleeViewKick()
{
	CDAPlayer *pPlayer = GetPlayerOwner();
...
float CWeaponDABase::GetWeaponSpread()
{
	return m_flSpread;
}

#ifdef CLIENT_DLL
bool CWeaponDABase::ShouldDrawCrosshair()
{
	return UseVR();
}

void CWeaponDABase::CreateMove(float flInputSampleTime, CUserCmd *pCmd, const QAngle &vecOldViewAngles)
{	
	if (prediction->InPrediction() && !prediction->IsFirstTimePredicted())
		return;
...
int CWeaponDABase::DrawModel(int flags)
{
	...
	float flTime = C_DAPlayer::GetLocalOrSpectatedPlayer()->GetCurrentTime() + m_flArrowSpinOffset;
	float flFrameTime = gpGlobals->frametime * C_DAPlayer::GetLocalOrSpectatedPlayer()->GetSlowMoMultiplier();
...
int C_WeaponDABase::s_iMaxClip = 0;
Vector C_WeaponDABase::s_vecAmmo1L;
Vector C_WeaponDABase::s_vecAmmo2L;
Vector C_WeaponDABase::s_vecAmmo1R;
Vector C_WeaponDABase::s_vecAmmo2R;
CUtlVector<C_WeaponDABase::CFlyingRound> C_WeaponDABase::s_aRounds;

CMaterialReference g_hBulletIcon;
void CWeaponDABase::DrawVRBullets(const Vector& vecAmmo1, const Vector& vecAmmo2, int iClip, int iMaxClip, bool bRight)
{
...
Vector CWeaponDABase::GetVRRoundPosition(int iRound, bool bRight)
{
	if (!s_iMaxClip)
		s_iMaxClip = 10;
...
void CWeaponDABase::VRBulletFired(int iRound, bool bRight)
{
	int iSpot = -1;
...
void CWeaponDABase::ClientThink()
{
	...
	if (C_DAPlayer::GetLocalOrSpectatedPlayer())
		flFrameTime *= C_DAPlayer::GetLocalDAPlayer()->GetSlowMoMultiplier();
...
void C_WeaponDABase::OnDataChanged( DataUpdateType_t type )
{
...
float C_WeaponDABase::GetMarksmanGold()
{
	return m_flMarksmanGold;
}
#endif

float CWeaponDABase::GetViewPunchMultiplier()
{
	return m_flViewPunchMultiplier;
}

float CWeaponDABase::GetRecoil()
{
	return m_flRecoil;
}

bool CWeaponDABase::HasAimInSpeedPenalty()
{
	return GetSDKWpnData().m_bAimInSpeedPenalty;
}

bool CWeaponDABase::HasAimInFireRateBonus()
{
	return GetSDKWpnData().m_bAimInFireRateBonus;
}

bool CWeaponDABase::HasAimInRecoilBonus()
{
	return GetSDKWpnData().m_bAimInRecoilBonus;
}

weapontype_t CWeaponDABase::GetWeaponType() const
{
	return GetSDKWpnData().m_eWeaponType;
}

weapontype_t CWeaponDABase::GetWeaponType(DAWeaponID eWeapon)
{
...
void CWeaponDABase::ItemPostFrame( void )
{
	CDAPlayer *pPlayer = GetPlayerOwner();
	if ( !pPlayer )
		return;
...
void CWeaponDABase::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
#ifdef GAME_DLL
	CDAPlayer *pPlayer = ToDAPlayer( pActivator );
...
void CWeaponDABase::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType, bool bUseTracerAttachment )
{
#ifdef CLIENT_DLL
	if (prediction->InPrediction() && !prediction->IsFirstTimePredicted())
		return;
...
void CWeaponDABase::WeaponIdle( void )
{
...
bool CWeaponDABase::Reload( void )
{
	bool fRet;
	...
	if ( fRet )
	{
		ToDAPlayer(GetOwner())->UseStyleCharge(SKILL_MARKSMAN, 5);

		CDAPlayer* pSDKOwner = ToDAPlayer(GetOwner());
...
void CWeaponDABase::FinishReload()
{
	CDAPlayer *pOwner = GetPlayerOwner();
...
void CWeaponDABase::SendReloadEvents()
{
	CDAPlayer *pPlayer = GetPlayerOwner();
	if ( !pPlayer )
		return;
...
void CWeaponDABase::CheckReload()
{
	if ( m_bReloadsSingly )
	{
...
bool CWeaponDABase::ReloadOrSwitchWeapons( void )
{
	m_bFireOnEmpty = false;
...
bool CWeaponDABase::Holster( CBaseCombatWeapon *pSwitchingTo )
{ 
	MDLCACHE_CRITICAL_SECTION();
...
bool CWeaponDABase::HasWeaponIdleTimeElapsed( void )
{
...
float CWeaponDABase::GetCurrentTime() const
{
	if (!GetOwner())
...
#ifdef GAME_DLL
void CWeaponDABase::SetDieThink( bool bDie )
{
	if( bDie )
		SetContextThink( &CWeaponDABase::Die, gpGlobals->curtime + 45.0f, "DieContext" );
	else
		SetContextThink( NULL, gpGlobals->curtime, "DieContext" );
}
void CWeaponDABase::Die( void )
{
	UTIL_Remove( this );
}
#endif

bool CWeaponDABase::CanBeSelected()
{
...
const Vector CWeaponDABase::GetShootPosition(CDAPlayer* pShooter)
{
	Assert(pShooter);
	if (!pShooter)
		return Vector(0, 0, 0);
