...
void CSDKWeaponInfo::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	else if (FStrEq(pszWeaponType, "grenade"))
		m_eWeaponType = WT_GRENADE;
	else
		m_eWeaponType = WT_NONE;

	Q_strncpy(m_szAkimbo, pKeyValuesData->GetString("akimbo", ""), sizeof(m_szAkimbo));
	Q_strncpy(m_szSingle, pKeyValuesData->GetString("single", ""), sizeof(m_szSingle));
}

CSDKWeaponInfo* CSDKWeaponInfo::GetWeaponInfo(DAWeaponID eWeapon)
{
