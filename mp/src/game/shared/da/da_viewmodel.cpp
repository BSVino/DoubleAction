...
{
#ifdef CLIENT_DLL
	CWeaponDABase* pWeapon = GetDAWeapon();
...
int CDAViewModel::DrawModel(int flags)
{
	...
	if (pLocalPlayer && pLocalPlayer->UseVRHUD())
	{
		...
		if (GetAttachment(iAmmo1, vecAmmo1) && GetAttachment(iAmmo2, vecAmmo2))
			CWeaponDABase::DrawVRBullets(vecAmmo1, vecAmmo2, GetActiveWeapon()->Clip1(), GetActiveWeapon()->GetMaxClip1(), true);
	...
	case AE_CLIENT_EFFECT_ATTACH:
		{
			...
			if( token )
			{
				if (pOwner->IsInThirdPerson() && pOwner->GetActiveDAWeapon())
				{
					iAttachment = pOwner->GetActiveDAWeapon()->LookupAttachment(token);
					bResult = pOwner->GetActiveDAWeapon()->GetAttachment( iAttachment, attachOrigin, attachAngles );
				}
				else
				{
...
ConVar da_weapontilt( "da_weapontilt", "20", FCVAR_REPLICATED|FCVAR_CHEAT|FCVAR_DEVELOPMENTONLY, "How much does the weapon tilt when diving laterally?" );
...
{
	CDAPlayer* pOwner = ToDAPlayer(owner);
	if (!pOwner)
		return;
