...
void CDAPlayer::FireBullet( 
						   Vector vecSrc,	// shooting postion
						   const QAngle &shootAngles,  //shooting angle
						   float vecSpread, // spread vector
						   DAWeaponID eWeaponID,	// weapon that fired this shot
						   ...
						   )
{
	...
	CDAPlayer* pSDKAttacker = ToDAPlayer(pevAttacker);
	Assert(pSDKAttacker);
	Assert(pSDKAttacker == this);

	CBulletManager::CBullet oBullet = BulletManager().MakeBullet(pSDKAttacker, vecSrc, vecDir, eWeaponID, GetActiveDAWeapon(), iDamage, iBulletType, bDoEffects);
...
void CDAPlayer::DoMuzzleFlash(int iAkimbo)
{
#ifdef CLIENT_DLL
	if (prediction->InPrediction() && !prediction->IsFirstTimePredicted())
		return;

	C_DAPlayer* pLocalPlayer = C_DAPlayer::GetLocalDAPlayer();
	C_WeaponDABase* pActiveWeapon = GetActiveDAWeapon();
...
void CDAPlayer::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType, bool bUseTracerAttachment )
{
#ifdef CLIENT_DLL
	CWeaponDABase *pWeapon = GetActiveDAWeapon();
	if (pWeapon)
		pWeapon->MakeTracer( vecTracerSrc, tr, iTracerType, bUseTracerAttachment );
#endif
}

bool CDAPlayer::CanMove( void ) const
{
...
void CDAPlayer::StartTouch(CBaseEntity *pOther)
{
...
bool CDAPlayer::PlayerUse()
{
	...
	if ( ((m_nButtons | m_afButtonPressed | m_afButtonReleased) & IN_USE) && !IsObserver() )
	{
		...
		for ( CEntitySphereQuery sphere( vecSearchCenter, GRENADE_PICKUP_RADIUS ); ( pObject = sphere.GetCurrentEntity() ) != NULL; sphere.NextEntity() )
		{
			...
			CWeaponDABase* pWeapon = dynamic_cast<CWeaponDABase*>(pObject);
			if (!pWeapon)
				continue;
...
void CDAPlayer::SetSprinting( bool bIsSprinting )
{
	m_Shared.SetSprinting( bIsSprinting );
}

bool CDAPlayer::IsSprinting( void )
{
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector CDAPlayer::GetPlayerMins( void ) const
{
	if ( IsObserver() )
	{
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector CDAPlayer::GetPlayerMaxs( void ) const
{	
	if ( IsObserver() )
	{
...
float CDAPlayer::GetStylePoints()
{
...
bool CDAPlayer::IsStyleSkillActive(SkillID eSkill) const
{
	if (da_styletime.GetBool())
	{
		if (eSkill == SKILL_NONE)
			return true;
		else if (m_Shared.m_bSuperSkill || DAGameRules()->GetBountyPlayer() == this)
			return true;
		else
...
void CDAPlayer::UseStyleCharge(SkillID eSkill, float flCharge)
{
	if (DAGameRules()->GetBountyPlayer() == this)
		return;
...
int CDAPlayer::GetStyleStars(float flPoints)
{
...
int CDAPlayer::GetStyleStars()
{
	...
	flTotalStyle = 0;
#else
	C_DA_PlayerResource *sdkPR = DAGameResources();
...
void CDAPlayer::FreezePlayer(float flAmount, float flTime)
{
	m_flFreezeAmount = flAmount;

	if (m_Shared.m_iStyleSkill == SKILL_BOUNCER || m_Shared.m_bSuperSkill || DAGameRules()->GetBountyPlayer() == this)
...
void CDAPlayer::ReadyWeapon()
{
	m_flReadyWeaponUntil = GetCurrentTime() + 3.5f;
}

bool CDAPlayer::IsWeaponReady()
{
...
// --------------------------------------------------------------------------------------------------- //
// CDAPlayerShared implementation.
// --------------------------------------------------------------------------------------------------- //
...
void CDAPlayerShared::Init( CDAPlayer *pPlayer )
{
	m_pOuter = pPlayer;
}

bool CDAPlayerShared::IsDucking( void ) const
{
	return ( m_pOuter->GetFlags() & FL_DUCKING ) ? true : false;
}

#if defined ( SDK_USE_PRONE )
bool CDAPlayerShared::IsProne() const
{
	return m_bProne;
}

bool CDAPlayerShared::IsGettingUpFromProne() const
{
	return ( m_flUnProneTime > 0 );
}

bool CDAPlayerShared::IsGoingProne() const
{
	return ( m_flGoProneTime > 0 );
}

void CDAPlayerShared::SetProne( bool bProne, bool bNoAnimation /* = false */ )
{
	m_bProne = bProne;
...
void CDAPlayerShared::StartGoingProne( void )
{
...
void CDAPlayerShared::StandUpFromProne( void )
{	
	// make the prone sound
	CPASFilter filter( m_pOuter->GetAbsOrigin() );
	filter.UsePredictionRules();
...
bool CDAPlayerShared::CanChangePosition( void ) const
{
...
bool CDAPlayerShared::IsDiveSliding() const
{
...
bool CDAPlayerShared::CanSlide() const
{
...
void CDAPlayerShared::PlayStartSlideSound()
{
	CPASFilter filter( m_pOuter->GetAbsOrigin() );
	filter.UsePredictionRules();
...
void CDAPlayerShared::PlayEndSlideSound()
{
	CPASFilter filter( m_pOuter->GetAbsOrigin() );
	filter.UsePredictionRules();
...
void CDAPlayerShared::StartSliding(bool bDiveSliding)
{
	if (!CanSlide())
		return;
...
float CDAPlayerShared::GetSlideFriction() const
{
	if (!m_bSliding)
		return 1;
...
void CDAPlayerShared::SetDuckPress(bool bReset)
{
	if (bReset)
		m_flLastDuckPress = -1;
	else
...
bool CDAPlayerShared::IsRolling() const
{
	return m_bRolling && m_pOuter->m_lifeState != LIFE_DEAD;
}

bool CDAPlayerShared::CanRoll() const
{
	if (m_pOuter->GetLocalVelocity().Length2D() < 10)
		return false;
...
void CDAPlayerShared::StartRolling(bool bFromDive)
{
	if (!CanRoll())
		return;
...
bool CDAPlayerShared::IsDiving() const
{
	return m_bDiving && m_pOuter->m_lifeState != LIFE_DEAD;
}

bool CDAPlayerShared::CanDive() const
{
	if (m_pOuter->GetLocalVelocity().Length2D() < 10)
		return false;
...
ConVar  da_dive_height( "da_dive_height", "150", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY );
ConVar  da_dive_gravity("da_dive_gravity", "0.6", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);

ConVar  da_dive_height_high("da_dive_height_high", "200", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);

ConVar  da_dive_speed_adrenaline("da_dive_speed_adrenaline", "500", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar  da_dive_height_adrenaline("da_dive_height_adrenaline", "150", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar  da_dive_gravity_adrenaline("da_dive_gravity_adrenaline", "0.6", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
...
Vector CDAPlayerShared::StartDiving()
{
	...
	float flDiveHeight = da_dive_height.GetFloat();
	...
	if (!bWasOnGround)
		flDiveHeight = da_dive_height_high.GetFloat();

	float flRatio = da_dive_height_adrenaline.GetFloat() / flDiveHeight;
	float flModifier = (flRatio - 1)/2;

	flDiveHeight = ModifySkillValue (flDiveHeight, flModifier, SKILL_ATHLETIC);

	flRatio = da_dive_gravity_adrenaline.GetFloat() / da_dive_gravity.GetFloat();
	flModifier = (flRatio - 1)/2;

	m_pOuter->SetGravity(ModifySkillValue(da_dive_gravity.GetFloat(), flModifier, SKILL_ATHLETIC));

	ConVarRef da_dive_speed("da_dive_speed");
	flRatio = da_dive_speed_adrenaline.GetFloat() / da_dive_speed.GetFloat();
	flModifier = (flRatio - 1)/2;

	return m_vecDiveDirection.Get() * (ModifySkillValue(da_dive_speed.GetFloat(), flModifier, SKILL_ATHLETIC) * flSpeedFraction) + Vector(0, 0, flDiveHeight);
}

void CDAPlayerShared::StartSuperfallDiving()
{
	m_flDiveTime = m_pOuter->GetCurrentTime();
	m_flDiveLerped = 0;
	m_flDiveToProneLandTime = -1;
...
void CDAPlayerShared::DiveLandedProne()
{
	SetProne(true, true);
...
void CDAPlayerShared::StartManteling(const Vector& vecWallNormal)
{
	EndDive();
	EndWallFlip();
...
void CDAPlayerShared::ResetManteling()
{
	m_bIsManteling = false;
}

#if defined ( SDK_USE_SPRINTING )
void CDAPlayerShared::SetSprinting( bool bSprinting )
{
...
void CDAPlayerShared::ResetSprintPenalty( void )
{
	m_bGaveSprintPenalty = false;
}

void CDAPlayerShared::StartSprinting( void )
{
	m_bIsSprinting = true;
}

void CDAPlayerShared::StopSprinting( void )
{
	m_bIsSprinting = false;
}
#endif

void CDAPlayerShared::SetJumping( bool bJumping )
{
	m_bJumping = bJumping;
...
bool CDAPlayerShared::IsWallFlipping(bool bExtend) const
{
...
void CDAPlayerShared::EndWallFlip()
{
...
bool CDAPlayerShared::IsSuperFalling()
{
	...
	if (m_pOuter->GetAbsOrigin().z > DAGameRules()->GetLowestSpawnPoint().z - 400)
		return false;
...
bool CDAPlayerShared::CanSuperFallRespawn()
{
	if (!IsSuperFalling())
		return false;
...
bool CDAPlayerShared::IsAimedIn() const
{
	if (IsDiving() || IsRolling())
		return false;

	if (GetActiveDAWeapon() && GetActiveDAWeapon()->IsThrowingGrenade())
		return false;

	return m_bAimedIn;
}

void CDAPlayerShared::SetAimIn(bool bAimIn)
{
	...
	{
		CWeaponDABase* pWeapon = m_pOuter->GetActiveDAWeapon();
...
void CDAPlayerShared::RampSlowAimIn(float flGoal)
{
	if (flGoal > 0)
...
Vector CDAPlayerShared::GetRecoil(float flFrameTime)
{
...
void CDAPlayerShared::SetRecoil(float flRecoil)
{
...
void CDAPlayerShared::PlayerOnGround( void )
{
...
void CDAPlayerShared::ForceUnzoom( void )
{
//	CWeaponDABase *pWeapon = GetActiveDAWeapon();
...
bool CDAPlayerShared::IsSniperZoomed( void ) const
{
//	CWeaponDABase *pWeapon = GetActiveDAWeapon();
//	if( pWeapon && ( pWeapon->GetSDKWpnData().m_WeaponType & WPN_MASK_GUN ) )
//	{
//		CWeaponDABaseGun *pGun = (CWeaponDABaseGun *)pWeapon;
//		Assert( pGun );
...
#if defined ( SDK_USE_PLAYERCLASSES )
void CDAPlayerShared::SetDesiredPlayerClass( int playerclass )
{
	m_iDesiredPlayerClass = playerclass;
}

int CDAPlayerShared::DesiredPlayerClass( void )
{
	return m_iDesiredPlayerClass;
}

void CDAPlayerShared::SetPlayerClass( int playerclass )
{
	m_iPlayerClass = playerclass;
}

int CDAPlayerShared::PlayerClass( void )
{
...
void CDAPlayerShared::SetStamina( float flStamina )
{
	m_flStamina = clamp( flStamina, 0, 100 );
}
#endif
CWeaponDABase* CDAPlayerShared::GetActiveDAWeapon() const
{
	CBaseCombatWeapon *pWeapon = m_pOuter->GetActiveWeapon();
	if ( pWeapon )
	{
		Assert( dynamic_cast< CWeaponDABase* >( pWeapon ) == static_cast< CWeaponDABase* >( pWeapon ) );
		return static_cast< CWeaponDABase* >( pWeapon );
	}
	else
	{
		return NULL;
...
void CDAPlayerShared::ComputeWorldSpaceSurroundingBox( Vector *pVecWorldMins, Vector *pVecWorldMaxs )
{
...
float CDAPlayer::GetPlayerMaxSpeed(bool bDucking)
{
	float flMaxSpeed;
	...
	{
		ConVarRef da_dive_speed_adrenaline("da_dive_speed_adrenaline");
		ConVarRef da_dive_speed("da_dive_speed");

		float flSpeedRatio = da_dive_speed_adrenaline.GetFloat() / da_dive_speed.GetFloat();
		flSpeedRatio -= 1; // 0 means unchanged.
		flSpeedRatio /= 2; // It gets doubled when the skill is on.

		flMaxSpeed = m_Shared.ModifySkillValue(da_dive_speed.GetFloat(), flSpeedRatio, SKILL_ATHLETIC);
	}
	else
	{
		if ( bDucking )
...
float CDAPlayer::GetMaxSlideSpeed()
{
...
//-----------------------------------------------------------------------------
bool CDAPlayer::ShouldCollide( int collisionGroup, int contentsMask ) const
{
...
void CDAPlayer::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
...
bool CDAPlayer::CanAddToLoadout(DAWeaponID eWeapon)
{
...
int CDAPlayer::GetLoadoutWeaponCount(DAWeaponID eWeapon)
{
	return m_aLoadout[eWeapon].m_iCount;
}

void CDAPlayer::GetStepSoundVelocities( float *velwalk, float *velrun )
{
...
float CDAPlayer::GetSequenceCycleRate( CStudioHdr *pStudioHdr, int iSequence )
{
	return BaseClass::GetSequenceCycleRate( pStudioHdr, iSequence ) * GetSlowMoMultiplier();
}

void CDAPlayer::ActivateSlowMo()
{
...
void CDAPlayer::ActivateSuperfallSlowMo()
{
...
void CDAPlayer::DeactivateSlowMo()
{
	if (m_flSlowMoTime == 0)
		return;
...
float CDAPlayer::GetSlowMoMultiplier() const
{
	return m_flSlowMoMultiplier * da_globalslow.GetFloat();
}

float CDAPlayer::GetSlowMoGoal() const
{
...
void CDAPlayer::UpdateCurrentTime()
{
...
void CDAPlayer::UpdateViewBobRamp()
{
#ifdef CLIENT_DLL
	// It's not filled in for remote clients, so force the local one since it's the same.
	float flMaxBobSpeed = C_DAPlayer::GetLocalDAPlayer()->m_Shared.m_flRunSpeed*0.7f;
#else
...
ConVar  da_cam_fade_distance("da_cam_fade_distance", "30", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);
ConVar  da_cam_fade_distance_aimin("da_cam_fade_distance_aimin", "25", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);
ConVar	da_cam_fade_alpha_val("da_cam_fade_alpha_val", "100", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);

void CDAPlayer::UpdateThirdCamera(const Vector& vecEye, const QAngle& angEye)
{
	if (!IsInThirdPerson())
		return;

	CWeaponDABase * pWeapon = NULL;
	if (GetActiveWeapon() != NULL){
		pWeapon = GetActiveDAWeapon();
	}
...
Vector CDAPlayer::EyePosition()
{
	...
	if (C_DAPlayer::GetLocalOrSpectatedPlayer() ==  this && C_DAPlayer::GetLocalDAPlayer() != C_DAPlayer::GetLocalOrSpectatedPlayer())
		bIsInThird = false;
#endif
...
#ifdef CLIENT_DLL
		// It's not filled in for remote clients, so force the local one since it's the same.
		float flSpeedRatio = C_DAPlayer::GetLocalDAPlayer()->m_Shared.m_flAimInSpeed/C_DAPlayer::GetLocalDAPlayer()->m_Shared.m_flRunSpeed;
#else
...
bool CDAPlayer::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	if (IsPlayer())
	{
...
CBaseCombatWeapon* CDAPlayer::GetLastWeapon()
{
	...
	CWeaponDABase* pHeaviest = NULL;
	CWeaponDABase* pBrawl = NULL;
	for (int i = 0; i < WeaponCount(); i++)
	{
		...
		CWeaponDABase* pSDKWeapon = dynamic_cast<CWeaponDABase*>(GetWeapon(i));
		if (!pSDKWeapon)
			continue;

		if (pSDKWeapon->GetWeaponID() == DA_WEAPON_BRAWL)
		{
			pBrawl = pSDKWeapon;
			continue;
		}
...
void CDAPlayer::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
...
bool CDAPlayer::IsInThirdPerson() const
{
	if (!IsAlive())
		return false;
...
const Vector CDAPlayer::CalculateThirdPersonCameraPosition(const Vector& vecEye, const QAngle& angCamera)
{
...
const Vector CDAPlayer::GetThirdPersonCameraPosition()
{
	return m_vecThirdCamera;
}

const Vector CDAPlayer::GetThirdPersonCameraTarget()
{
	return m_vecThirdTarget;
}

float CDAPlayerShared::ModifySkillValue(float flValue, float flModify, SkillID eSkill) const
{
	if (!m_bSuperSkill && DAGameRules()->GetBountyPlayer() != m_pOuter)
	{
...
CWeaponDABase* CDAPlayer::FindWeapon (DAWeaponID id)
{
	int i;
	for (i = 0; i < WeaponCount(); i++)
	{
		CWeaponDABase *wpn = (CWeaponDABase *)GetWeapon (i);
		if (wpn)
		{
...
bool CDAPlayer::HasBriefcase() const
{
	return !!m_hBriefcase;
}
