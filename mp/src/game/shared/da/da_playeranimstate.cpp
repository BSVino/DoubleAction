...
CDAPlayerAnimState* CreateDAPlayerAnimState( CDAPlayer *pPlayer )
{
	...
	CDAPlayerAnimState *pRet = new CDAPlayerAnimState( pPlayer, movementData );
...
// Input  :  - 
//-----------------------------------------------------------------------------
CDAPlayerAnimState::CDAPlayerAnimState()
{
	m_pDAPlayer = NULL;
...
//			&movementData - 
//-----------------------------------------------------------------------------
CDAPlayerAnimState::CDAPlayerAnimState( CBasePlayer *pPlayer, MultiPlayerMovementData_t &movementData )
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CDAPlayerAnimState::InitSDKAnimState( CDAPlayer *pPlayer )
{
	m_pDAPlayer = pPlayer;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CDAPlayerAnimState::ClearAnimationState( void )
{
#if defined ( SDK_USE_PRONE )
	m_bProneTransition = false;
	m_bProneTransitionFirstFrame = false;
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CDAPlayerAnimState::TranslateActivity( Activity actDesired )
{
	Activity translateActivity = BaseClass::TranslateActivity( actDesired );

	if ( GetDAPlayer()->GetActiveWeapon() )
	{
		translateActivity = GetDAPlayer()->GetActiveWeapon()->ActivityOverride( translateActivity, NULL );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CDAPlayerAnimState::Update( float eyeYaw, float eyePitch, float flCharacterYaw, float flCharacterPitch )
{
	// Profile the animation update.
	VPROF( "CMultiPlayerAnimState::Update" );

	// Get the SDK player.
	CDAPlayer *pDAPlayer = GetDAPlayer();
	if ( !pDAPlayer )
		return;

	// Get the studio header for the player.
	CStudioHdr *pStudioHdr = pDAPlayer->GetModelPtr();
	if ( !pStudioHdr )
		return;
	...
	if (pDAPlayer->GetActiveDAWeapon() && pDAPlayer->GetActiveDAWeapon()->HasAimInSpeedPenalty())
		flRampSpeed = 50;

	float flApproachSpeed = gpGlobals->frametime * pDAPlayer->GetSlowMoMultiplier() * RemapVal(m_pDAPlayer->m_Shared.GetAimIn(), 0, 1, 10, flRampSpeed);
	...
	{
		m_pDAPlayer->SetPlaybackRate( 1.0f );
	}

#if defined(WITH_VIEWBACK)
	if (pDAPlayer == C_DAPlayer::GetLocalOrSpectatedPlayer())
	{
...
bool CDAPlayerAnimState::SetupPoseParameters( CStudioHdr *pStudioHdr )
{
...
void CDAPlayerAnimState::GetOuterAbsVelocity(Vector& vel)
{
	...
	if (m_pDAPlayer != C_DAPlayer::GetLocalDAPlayer())
		vel /= m_pDAPlayer->GetSlowMoMultiplier();
...
void CDAPlayerAnimState::ComputePoseParam_AimPitch( CStudioHdr *pStudioHdr )
{
	if (m_pDAPlayer->m_Shared.IsSliding() && !m_pDAPlayer->m_Shared.IsDiveSliding())
	{
		Vector vecVelocity;
		...
		Vector vecForward;
		m_pDAPlayer->GetVectors(&vecForward, NULL, NULL);
	...
	if (m_pDAPlayer->m_Shared.IsSuperFalling() && !m_pDAPlayer->GetGroundEntity() && m_pDAPlayer->m_Shared.IsDiving())
		m_angRender[PITCH] = Approach(m_flEyePitch, m_angRender[PITCH], gpGlobals->frametime * 200);
	else
		m_angRender[PITCH] = 0;

	if (m_pDAPlayer->IsInThirdPerson())
	{
...
void CDAPlayerAnimState::ComputePoseParam_AimYaw( CStudioHdr *pStudioHdr )
{
	if (m_pDAPlayer->m_Shared.IsSliding() && !m_pDAPlayer->m_Shared.IsDiveSliding())
	{
		Vector vecVelocity;
		...
		{
			ConvergeYawAngles( m_flGoalFeetYaw, 720.0f, gpGlobals->frametime * m_pDAPlayer->GetSlowMoMultiplier(), m_flCurrentFeetYaw );
			m_flLastAimTurnTime = m_pDAPlayer->GetCurrentTime();
		}
	...
	else if (m_pDAPlayer->m_Shared.IsManteling())
	{
	...
	{
		if (m_pDAPlayer->m_Shared.IsAimedIn() || m_pDAPlayer->m_Shared.IsDiving() || m_pDAPlayer->m_Shared.IsRolling() || m_pDAPlayer->m_Shared.IsSliding())
		{
	...
	else
	{
		...
		if ( m_PoseParameterData.m_flLastAimTurnTime <= 0.0f )
		{
			...
			m_PoseParameterData.m_flLastAimTurnTime = m_pDAPlayer->GetCurrentTime();
		}
	...
	if ( m_flGoalFeetYaw != m_flCurrentFeetYaw )
	{
		...
		{
			ConvergeYawAnglesThroughZero( m_flGoalFeetYaw, 720.0f, gpGlobals->frametime * m_pDAPlayer->GetSlowMoMultiplier(), m_flCurrentFeetYaw );
			m_flLastAimTurnTime = m_pDAPlayer->GetCurrentTime();
		}
	}
...
void CDAPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
	...
	if ( bIsMoving )
	{
		if (m_pDAPlayer->m_Shared.IsAimedIn() || m_pDAPlayer->m_Shared.IsDiving() || m_pDAPlayer->m_Shared.IsRolling()
			|| m_pDAPlayer->m_Shared.IsSliding() || m_bFlipping)
		{
...
void CDAPlayerAnimState::ComputePoseParam_StuntYaw( CStudioHdr *pStudioHdr )
{
	// Get the view yaw.
	float flAngle = AngleNormalize( m_flEyeYaw );
...
void CDAPlayerAnimState::EstimateYaw( void )
{
	// Get the frame time.
	float flDeltaTime = gpGlobals->frametime * m_pDAPlayer->GetSlowMoMultiplier();
	if ( flDeltaTime == 0.0f )
		return;
	...
	else if (m_pDAPlayer->m_Shared.IsManteling() || m_bFlipping)
	{
		Vector vecWallNormal = m_pDAPlayer->m_Shared.GetMantelWallNormal();
	...
	else if (m_pDAPlayer->m_Shared.IsAimedIn() || m_pDAPlayer->m_Shared.IsDiving() || m_pDAPlayer->m_Shared.IsRolling() || m_pDAPlayer->m_Shared.IsSliding())
	{
...
void CDAPlayerAnimState::ConvergeYawAnglesThroughZero( float flGoalYaw, float flYawRate, float flDeltaTime, float &flCurrentYaw )
{
...
// Input  : event - 
//-----------------------------------------------------------------------------
void CDAPlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	...
	case PLAYERANIMEVENT_ATTACK_PRIMARY:
		{
			// Weapon primary fire.
			if ( m_pDAPlayer->m_Shared.IsProne() || m_pDAPlayer->m_Shared.IsDiveSliding() )
			...
			else if ( m_pDAPlayer->GetFlags() & FL_DUCKING )
	...
	case PLAYERANIMEVENT_ATTACK_PRE:
		{
			if ( m_pDAPlayer->GetFlags() & FL_DUCKING ) 
			{
	...
	case PLAYERANIMEVENT_JUMP:
		{
			...
			m_flJumpStartTime = m_pDAPlayer->GetCurrentTime();
	...
	if ( iGestureActivity != ACT_INVALID && GetDAPlayer() != CDAPlayer::GetLocalDAPlayer())
	{
		CBaseCombatWeapon *pWeapon = GetDAPlayer()->GetActiveWeapon();
		if ( pWeapon )
		{
...
// Input  : *idealActivity - 
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
...
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleMoving( Activity &idealActivity )
{	
	if (!(m_pDAPlayer->GetFlags() & FL_ONGROUND) &&
		 m_pDAPlayer->GetAbsVelocity ().z < -270)
	{
	...
	if ( flSpeed > 150 )
	{
		...
		else if (m_pDAPlayer->IsWeaponReady())
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( m_pDAPlayer->GetFlags() & FL_DUCKING )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleProne( Activity &idealActivity )
{
	if ( m_pDAPlayer->m_Shared.IsDiveSliding() )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleProneTransition( Activity &idealActivity )
{
	if ( m_bProneTransition )
	{
		...
		if (!m_pDAPlayer->m_Shared.IsProne() && !m_pDAPlayer->m_Shared.IsGettingUpFromProne())
			m_bProneTransition = false;
...
bool CDAPlayerAnimState::HandleDiving( Activity &idealActivity )
{
	if (!m_pDAPlayer->m_Shared.IsDiving())
		m_bDiveStart = false;
	...
	if ( !m_bDiveStart && m_pDAPlayer->m_Shared.IsDiving() )
	{
...
bool CDAPlayerAnimState::HandleSliding( Activity &idealActivity )
{
	if ( m_pDAPlayer->m_Shared.IsSliding() && !m_pDAPlayer->m_Shared.IsDiveSliding() )
	{
...
bool CDAPlayerAnimState::HandleSlideTransition( Activity &idealActivity )
{
...
bool CDAPlayerAnimState::HandleRollTransition( Activity &idealActivity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDAPlayerAnimState::HandleSprinting( Activity &idealActivity )
{
	if ( m_pDAPlayer->m_Shared.IsSprinting() )
	{
...
//-----------------------------------------------------------------------------
// Purpose: 
bool CDAPlayerAnimState::HandleJumping( Activity &idealActivity )
{
	Vector vecVelocity;
	GetOuterAbsVelocity( vecVelocity );
	...
	if ( m_bJumping )
	{
		...
		if ( m_pDAPlayer->GetWaterLevel() >= WL_Waist )
		{
			m_bJumping = false;
			RestartMainSequence();
		}
		...
		else if ( m_pDAPlayer->GetCurrentTime() - m_flJumpStartTime > 0.2f )
		{
			if ( m_pDAPlayer->GetFlags() & FL_ONGROUND )
			{
				m_bJumping = false;
		...
		if ( m_bJumping )
		{
			if ( m_pDAPlayer->GetCurrentTime() - m_flJumpStartTime > 0.5 )
				idealActivity = ACT_DA_JUMP_FLOAT;
			else
	...
	if (!m_bJumping && 
		!(m_pDAPlayer->GetFlags() & FL_ONGROUND) &&
		!m_pDAPlayer->m_Shared.IsSliding () &&
		!m_pDAPlayer->m_Shared.IsRolling () &&
		m_pDAPlayer->GetAbsVelocity ().z < -270)
	{
		idealActivity = ACT_DA_JUMP_FLOAT;
		return true;
...
bool CDAPlayerAnimState::HandleWallFlip (Activity &idealActivity)
{
	if (!m_bFlipping)
	{
	...
	if (m_pDAPlayer->GetCurrentTime () - 
		m_pDAPlayer->m_Shared.GetDiveTime () < 1e-1) 
...
bool CDAPlayerAnimState::HandleWallClimb (Activity &idealActivity)
{
...
Activity CDAPlayerAnimState::CalcMainActivity()
{
...
bool CDAPlayerAnimState::ShouldUseAimInAnims()
{
	if (!m_pDAPlayer->m_Shared.IsAimedIn())
		return false;

	if (!m_pDAPlayer->GetActiveDAWeapon())
		return false;

	if (!m_pDAPlayer->GetActiveDAWeapon()->HasAimInSpeedPenalty())
		return false;
