...
#ifdef CLIENT_DLL
	#define CDAGameRules C_DAGameRules
	#define CDAGameRulesProxy C_DAGameRulesProxy
#endif

class CBriefcaseCaptureZone;
class CRatRaceWaypoint;

class CDAGameRulesProxy : public CGameRulesProxy
{
public:
	DECLARE_CLASS( CDAGameRulesProxy, CGameRulesProxy );
...
class CDAGameRules : public CTeamplayRules
{
public:
	DECLARE_CLASS( CDAGameRules, CTeamplayRules );
	...
	bool CanPlayerJoinClass( CDAPlayer *pPlayer, int cls );
	void ChooseRandomClass( CDAPlayer *pPlayer );
	...
	void	CalculateSlowMoForPlayer(CDAPlayer* pPlayer);	// Is this player being slowed by those around?
	void	PlayerSlowMoUpdate(CDAPlayer* pPlayer);		// This player activated or deactivated slowmo, update surrounding players.

protected:
	void	GiveSlowMoToNearbyPlayers(CDAPlayer* pPlayer);
...
public:
	...
	const char *GetKillingWeaponName( const CTakeDamageInfo &info, CDAPlayer *pVictim, int *iWeaponID );
...
public:
	...
	void GiveMiniObjectiveRewardTeam(CDAPlayer* pPlayer); // Award the entire team this player is on
	void GiveMiniObjectiveRewardPlayer(CDAPlayer* pPlayer);
	...
	void PlayerCapturedBriefcase(CDAPlayer* pPlayer);
	...
	CDAPlayer* GetBountyPlayer() const;

	bool SetupMiniObjective_RatRace();
	void MaintainMiniObjective_RatRace();
	void CleanupMiniObjective_RatRace();

	void PlayerReachedWaypoint(CDAPlayer* pPlayer, CRatRaceWaypoint* pWaypoint);
	...
	template<typename T> void WaypointLeadersPush(T& ahWaypointLeaders, CDAPlayer* pPlayer);
	template<typename T> void RemovePlayerFromLeaders(T& ahWaypointLeaders, CDAPlayer* pPlayer);
	...
	CDAPlayer* GetLeader() const;
	CDAPlayer* GetFrontRunner1() const;
	CDAPlayer* GetFrontRunner2() const;
...
private:
	...
	CNetworkHandle( CDAPlayer, m_hBountyPlayer );

	CNetworkArray( CHandle<CDAPlayer>, m_ahWaypoint1RaceLeaders, 16 );
	CNetworkArray( CHandle<CDAPlayer>, m_ahWaypoint2RaceLeaders, 16 );
...
inline CDAGameRules* DAGameRules()
{
	return static_cast<CDAGameRules*>(g_pGameRules);
}
