...
void C_SDKRootPanel::RenderLetterboxing( void )
{
	...
	C_WeaponDABase* pWeapon = pPlayer->GetActiveDAWeapon();
...
void C_SDKRootPanel::RenderDeathFrame( void )
{
	...
	C_DAPlayer* pPlayer = C_DAPlayer::GetLocalOrSpectatedPlayer();
	C_DAPlayer* pKiller = pPlayer->GetKiller();
	...
	if (pKiller && pszWeaponOfChoice && pKiller->GetActiveDAWeapon())
	{
		wchar_t* pszWeaponName;
		if (pKiller->IsStyleSkillActive(SKILL_MARKSMAN))
			pszWeaponName = g_pVGuiLocalize->Find((std::string(pKiller->GetActiveDAWeapon()->GetPrintName()) + "_Golden").c_str());
		else
			pszWeaponName = g_pVGuiLocalize->Find(pKiller->GetActiveDAWeapon()->GetPrintName());
