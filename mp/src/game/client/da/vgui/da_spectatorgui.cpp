...
void CSDKSpectatorGUI::UpdateSpectatorPlayerList()
{
	...
	C_DATeam *blue = GetGlobalDATeam( SDK_TEAM_BLUE );
	if ( blue )
	{
		wchar_t frags[ 10 ];
	...
	C_DATeam *red = GetGlobalDATeam( SDK_TEAM_RED );
	if ( red )
	{
		wchar_t frags[ 10 ];
...
{
	C_DAPlayer *player = C_DAPlayer::GetLocalDAPlayer();
	if ( !player )
		return false;

	if ( m_nLastTime != (int)DAGameRules()->GetMapRemainingTime() )
		return true;
...
void CSDKSpectatorGUI::UpdateTimer()
{
	...
	m_nLastTime = (int)( DAGameRules()->GetMapRemainingTime() );
...
void CSDKSpectatorGUI::Paint()
{
	...
	C_DAPlayer* pObserved = pLocalPlayer?ToDAPlayer(pLocalPlayer->GetObserverTarget()):NULL;

	if (pObserved && DAGameRules()->CoderHacks())
	{
...
bool CSDKMapOverview::CanPlayerBeSeen( MapPlayer_t *player )
{
	...
	SDKMapPlayer_t *DAPlayer = GetSDKInfoForPlayer(player);
	...
	if( player->health <= 0 )
	{
		...
		if ( DAPlayer->overrideExpirationTime == -1  ||  DAPlayer->overrideExpirationTime <= gpGlobals->curtime )
			return false;
	}
...
{
	C_DA_PlayerResource *pSDKPR = (C_DA_PlayerResource*)GameResources();
	if ( !pSDKPR )
		return;
