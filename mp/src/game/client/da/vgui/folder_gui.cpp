...
void CFolderMenu::Update()
{
	...
	DAWeaponID eFirst = WEAPON_NONE;
	for (int i = 0; i < MAX_LOADOUT; i++)
	{
		if (!pPlayer->GetLoadoutWeaponCount((DAWeaponID)i))
			continue;
		...
		{
			if (pPlayer->GetLoadoutWeaponCount((DAWeaponID)i) > 1)
			{
				CSDKWeaponInfo* pWeaponInfo = CSDKWeaponInfo::GetWeaponInfo((DAWeaponID)i);
				if (pWeaponInfo && pWeaponInfo->m_szAkimbo[0])
				{
					DAWeaponID eAkimbo = AliasToWeaponID(pWeaponInfo->m_szAkimbo);
					if (eAkimbo < eFirst)
					{
		...
		if (pPlayer->GetLoadoutWeaponCount((DAWeaponID)i) > 1)
			sLabel << " x" << pPlayer->GetLoadoutWeaponCount((DAWeaponID)i) << "\n";
		else
			sLabel << "\n";
	...
	if ((ShouldShowCharacterAndWeapons() || ShouldShowEverything()) && pWeaponIconArea)
	{
		...
		for (int i = 0; i < MAX_LOADOUT; i++)
		{
			...
			for (int j = 0; j < pPlayer->GetLoadoutWeaponCount((DAWeaponID)i); j++)
			{
				float flMoveRight = 0;
				if (iWeapon%2 == 1)
	...
	if (pChangeTeams)
		pChangeTeams->SetVisible(DAGameRules()->IsTeamplay());
}
...
void __MsgFunc_FolderPanel( bf_read &msg )
{
	...
	if (pPlayer && DAGameRules()->IsTeamplay() && (pPlayer->GetTeamNumber() != SDK_TEAM_RED && pPlayer->GetTeamNumber() != SDK_TEAM_BLUE))
