...
public:
	...
	{
		// Create the effect.
		C_DAPlayer *pPlayer = dynamic_cast< C_DAPlayer* >( m_hPlayer.Get() );
...
void __MsgFunc_ReloadEffect( bf_read &msg )
{
	...
	C_DAPlayer *pPlayer = dynamic_cast< C_DAPlayer* >( C_BaseEntity::Instance( iPlayer ) );
	if ( pPlayer )
...
// specific to the local player ( ideally should not be in CDAPlayerShared! )
BEGIN_RECV_TABLE_NOBASE( CDAPlayerShared, DT_DASharedLocalPlayerExclusive )
...
BEGIN_RECV_TABLE_NOBASE( CDAPlayerShared, DT_DAPlayerShared )
...
BEGIN_RECV_TABLE_NOBASE( C_DAPlayer, DT_DALocalPlayerExclusive )
...
BEGIN_RECV_TABLE_NOBASE( C_DAPlayer, DT_DANonLocalPlayerExclusive )
	RecvPropVector( RECVINFO_NAME( m_vecNetworkOrigin, m_vecOrigin ) ),
...
// main table
IMPLEMENT_CLIENTCLASS_DT( C_DAPlayer, DT_DAPlayer, CDAPlayer )
	RecvPropDataTable( RECVINFO_DT( m_Shared ), 0, &REFERENCE_RECV_TABLE( DT_DAPlayerShared ) ),

	RecvPropDataTable( "dalocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_DALocalPlayerExclusive) ),
	RecvPropDataTable( "danonlocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_DANonLocalPlayerExclusive) ),
...
// Prediction tables.
// ------------------------------------------------------------------------------------------ //
BEGIN_PREDICTION_DATA_NO_BASE( CDAPlayerShared )
#if defined ( SDK_USE_STAMINA ) || defined ( SDK_USE_SPRINTING )
...
BEGIN_PREDICTION_DATA( C_DAPlayer )
	DEFINE_PRED_TYPEDESCRIPTION( m_Shared, CDAPlayerShared ),
...
LINK_ENTITY_TO_CLASS( player, C_DAPlayer );
...
class C_DARagdoll : public C_BaseAnimatingOverlay
{
public:
	DECLARE_CLASS( C_DARagdoll, C_BaseAnimatingOverlay );
	DECLARE_CLIENTCLASS();
...
IMPLEMENT_CLIENTCLASS_DT_NOBASE( C_DARagdoll, DT_DARagdoll, CDARagdoll )
...
void C_DARagdoll::Interp_Copy( C_BaseAnimatingOverlay *pSourceEntity )
{
	if ( !pSourceEntity )
		return;
...
bool C_DARagdoll::TestHitboxes( const Ray_t &ray, unsigned int fContentsMask, trace_t& tr )
{
...
void C_DARagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
//	DevMsg("C_DARagdoll::ImpactTrace: %i\n", iDamageType);
...
void C_DARagdoll::CreateRagdoll()
{
...
void C_DARagdoll::OnDataChanged( DataUpdateType_t type )
{
...
bool C_DARagdoll::IsRagdollVisible()
{
...
void C_DARagdoll::ClientThink( void )
{
	...
	{
		C_DAPlayer *pEnt = static_cast< C_DAPlayer *> ( UTIL_PlayerByIndex( iClient ) );
...
void C_DARagdoll::StartFadeOut( float fDelay )
{
	m_fDeathTime = gpGlobals->curtime + fDelay;
	SetNextClientThink( CLIENT_THINK_ALWAYS );
}
IRagdoll* C_DARagdoll::GetIRagdoll() const
{
	return m_pRagdoll;
}

C_BaseAnimating * C_DAPlayer::BecomeRagdollOnClient()
{
	return NULL;
}


IRagdoll* C_DAPlayer::GetRepresentativeRagdoll() const
{
	if ( m_hRagdoll.Get() )
	{
		C_DARagdoll *pRagdoll = (C_DARagdoll*)m_hRagdoll.Get();
...
C_DAPlayer::C_DAPlayer() : 
	m_iv_angEyeAngles( "C_DAPlayer::m_iv_angEyeAngles" )
{
...
void C_DAPlayer::PreThink()
{
	...
	if (GetObserverMode() == OBS_MODE_IN_EYE && GetObserverTarget() && GetObserverTarget()->IsPlayer())
		ToDAPlayer(GetObserverTarget())->UpdateViewBobRamp();

	BaseClass::PreThink();

	Instructor_Think();

	if (C_DAPlayer::GetLocalDAPlayer() == this && GetClientVoiceMgr()->IsLocalPlayerSpeaking())
		Instructor_LessonLearned("voicechat");

#if defined(CLIENT_DLL) && defined(WITH_VIEWBACK)
	bool bSend = (C_DAPlayer::GetLocalOrSpectatedPlayer() == this);
...
C_DAPlayer* C_DAPlayer::GetLocalDAPlayer()
{
	return ToDAPlayer( C_BasePlayer::GetLocalPlayer() );
}

C_DAPlayer* C_DAPlayer::GetLocalOrSpectatedPlayer()
{
	C_DAPlayer* pLocal = GetLocalDAPlayer();

	if (pLocal && pLocal->GetObserverMode() == OBS_MODE_IN_EYE && pLocal->GetObserverTarget() && pLocal->GetObserverTarget()->IsPlayer())
		return ToDAPlayer(pLocal->GetObserverTarget());

	return pLocal;
}

const QAngle &C_DAPlayer::EyeAngles()
{
	if( IsLocalPlayer() )
	{
...
const QAngle& C_DAPlayer::GetRenderAngles()
{
	if ( IsRagdoll() )
	{
		return vec3_angle;
...
const Vector& C_DAPlayer::GetRenderOrigin( void )
{
	return BaseClass::GetRenderOrigin();
}

void C_DAPlayer::UpdateClientSideAnimation()
{
	QAngle angEyeAngles = EyeAngles();
	QAngle angCharacterEyeAngles = angEyeAngles;

	if (IsInThirdPerson() && this == GetLocalDAPlayer())
...
// Purpose: 
//-----------------------------------------------------------------------------
CStudioHdr *C_DAPlayer::OnNewModel( void )
{
...
//-----------------------------------------------------------------------------
void C_DAPlayer::InitializePoseParams( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
/*void C_DAPlayer::OnPlayerClassChange( void )
{
...
*/
void C_DAPlayer::PostDataUpdate( DataUpdateType_t updateType )
{
...
void C_DAPlayer::LocalPlayerRespawn( void )
{
	...
	if (C_DAPlayer::GetLocalDAPlayer() == this)
...
void C_DAPlayer::OnDataChanged( DataUpdateType_t type )
{
	BaseClass::OnDataChanged( type );
...
void C_DAPlayer::PlayReloadEffect()
{
	// Only play the effect for other players.
	if ( this == C_DAPlayer::GetLocalDAPlayer() )
	{
...
void C_DAPlayer::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	if ( IsLocalPlayer() )
	{
		if ( ( prediction->InPrediction() && !prediction->IsFirstTimePredicted() ) )
...
bool C_DAPlayer::ShouldDraw( void )
{
...
CWeaponDABase* C_DAPlayer::GetActiveDAWeapon() const
{
	return dynamic_cast< CWeaponDABase* >( GetActiveWeapon() );
}
...
//-----------------------------------------------------------------------------
void C_DAPlayer::CalcVehicleView(IClientVehicle *pVehicle,	Vector& eyeOrigin, QAngle& eyeAngles,	float& zNear, float& zFar, float& fov )
{
	Assert( pVehicle );
...
void C_DAPlayer::CalcInEyeCamView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
...
void C_DAPlayer::CalcFreezeCamView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
	int iTotal = 6;
	...
	if ( !m_bSentFreezeFrame )
	{
		if (da_deathframe_showvectors.GetBool())
		{
			...
			if (bDebugTargets)
			{
				...
				if (ToDAPlayer(GetObserverTarget()))
					ToDAPlayer(GetObserverTarget())->DrawClientHitboxes( flTime, true );
...
bool C_DAPlayer::CalcFreezeCamThreeQuartersView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov, float flSide, Vector& vecCamTarget, Vector& vecCamPlayer, Vector& vecCamLookAt )
{
...
bool C_DAPlayer::CalcFreezeCamHalfHalfView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov, float flSide, Vector& vecCamTarget, Vector& vecCamPlayer, Vector& vecCamLookAt )
{
	C_DAPlayer* pTarget = ToDAPlayer(GetObserverTarget());
...
bool C_DAPlayer::CalcFreezeCamPortraitView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
...
bool C_DAPlayer::CalcFreezeCamKillerAimInView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
	C_DAPlayer* pTarget = ToDAPlayer(GetObserverTarget());
	...
	if (!pTarget->GetActiveDAWeapon())
		return false;

	if (pTarget->GetActiveDAWeapon()->IsGrenade())
		return false;

	if (pTarget->GetActiveDAWeapon()->IsMeleeWeapon())
		return false;
...
Vector C_DAPlayer::GetFreezeCamOrigin(C_BaseEntity* pTarget)
{
	C_DAPlayer* pSDKTarget = ToDAPlayer(pTarget);
	if (!pSDKTarget)
...
#if defined ( SDK_USE_PLAYERCLASSES )
bool C_DAPlayer::CanShowClassMenu( void )
{
	#if defined ( SDK_USE_TEAMS )
...
#if defined ( SDK_USE_TEAMS )
bool C_DAPlayer::CanShowTeamMenu( void )
{
	return true;
}
#endif

bool C_DAPlayer::CanShowBuyMenu( void )
{
	return ( GetTeamNumber() != TEAM_SPECTATOR );
}

bool C_DAPlayer::CanShowSkillMenu( void )
{
	return ( GetTeamNumber() != TEAM_SPECTATOR );
}
...
/*static*/ bool C_BasePlayer::ShouldDrawLocalPlayer()
{
	...
	{
		if (C_DAPlayer::GetLocalDAPlayer()->IsInThirdPerson())
			return true;
...
void C_DAPlayer::ClientThink()
{
	...
	if (IsStyleSkillActive(SKILL_BOUNCER))
	{
		...
		{
			if (GetActiveDAWeapon()->GetWeaponID() == DA_WEAPON_BRAWL)
				bBrawlEffectOn = true;
			else if (GetActiveDAWeapon()->GetSwingTime() > 0)
				bBrawlEffectOn = true;
		}
	}
	...
	bool bLocalPlayer = (C_DAPlayer::GetLocalOrSpectatedPlayer() == this);
	if (bLocalPlayer)
	...
	if (bLocalPlayer)
	{
		...
		if (!C_DAPlayer::GetLocalOrSpectatedPlayer()->IsAlive())
			bNowInSlow = false;

		if (!bWasInSlow && bNowInSlow)
		{
			C_DAPlayer::GetLocalOrSpectatedPlayer()->EmitSound( "SlowMo.Start" );
			C_DAPlayer::GetLocalOrSpectatedPlayer()->EmitSound( "SlowMo.Loop" );
		}
		else if (bWasInSlow && !bNowInSlow)
		{
			C_DAPlayer::GetLocalOrSpectatedPlayer()->StopSound( "SlowMo.Loop" );
			C_DAPlayer::GetLocalOrSpectatedPlayer()->EmitSound( "SlowMo.End" );
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_DAPlayer::UpdateLookAt( void )
{
	// head yaw
...
int C_DAPlayer::GetIDTarget() const
{
...
//-----------------------------------------------------------------------------
void C_DAPlayer::UpdateIDTarget()
{
	if ( !IsLocalPlayer() )
		return;
...
void C_DAPlayer::AvoidPlayers( CUserCmd *pCmd )
{
// Player Avoidance is only active with teams
	...
	C_DATeam *pTeam = ( C_DATeam * )GetTeam();
	if ( !pTeam )
		return;
	...
	Vector vecDAPlayerCenter = GetAbsOrigin();
	Vector vecDAPlayerMin = GetPlayerMins();
	Vector vecDAPlayerMax = GetPlayerMaxs();
	float flZHeight = vecDAPlayerMax.z - vecDAPlayerMin.z;
	vecDAPlayerCenter.z += 0.5f * flZHeight;
	VectorAdd( vecDAPlayerMin, vecDAPlayerCenter, vecDAPlayerMin );
	VectorAdd( vecDAPlayerMax, vecDAPlayerCenter, vecDAPlayerMax );

	// Find an intersecting player or object.
	int nAvoidPlayerCount = 0;
	C_DAPlayer *pAvoidPlayerList[MAX_PLAYERS];

	C_DAPlayer *pIntersectPlayer = NULL;
	float flAvoidRadius = 0.0f;

	Vector vecAvoidCenter, vecAvoidMin, vecAvoidMax;
	for ( int i = 0; i < pTeam->GetNumPlayers(); ++i )
	{
		C_DAPlayer *pAvoidPlayer = static_cast< C_DAPlayer * >( pTeam->GetPlayer( i ) );
		...
		if ( IsBoxIntersectingBox( vecDAPlayerMin, vecDAPlayerMax, vecAvoidMin, vecAvoidMax ) )
		{
	...
	if ( pIntersectPlayer )
	{
		VectorSubtract( pIntersectPlayer->WorldSpaceCenter(), vecDAPlayerCenter, vecDelta );
...
bool C_DAPlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{	
...
void C_DAPlayer::UpdateSoundEvents()
{
	int iNext;
...
static ConVar da_cam_back("da_cam_back", "80", FCVAR_USERINFO|FCVAR_ARCHIVE, "How far back from the eye position the third person camera sits", true, 5, true, 150);
static ConVar da_cam_right("da_cam_right", "7", FCVAR_USERINFO|FCVAR_ARCHIVE, "How far right from the eye position the third person camera sits", true, -30, true, 30);
static ConVar da_cam_up("da_cam_up", "20", FCVAR_USERINFO|FCVAR_ARCHIVE, "How far up from the eye position the third person camera sits", true, -20, true, 30);
...
void C_DAPlayer::OverrideView( CViewSetup *pSetup )
{
	if (C_DAPlayer::GetLocalDAPlayer() == this && IsInThirdPerson())
	{
	...
	C_WeaponDABase* pWeapon = GetActiveDAWeapon();
...
void C_DAPlayer::UpdateTeamMenu()
{
...
void RecvProxy_Loadout( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	...
	if (pData && C_DAPlayer::GetLocalDAPlayer() && pData->m_ObjectID == C_DAPlayer::GetLocalDAPlayer()->entindex())
	{
		static_cast<CFolderMenu*>(gViewPortInterface->FindPanelByName(PANEL_FOLDER))->MarkForUpdate();
	}
}
...
public:
	...
	{
		ToDAPlayer(player)->TurnOffFlashlight();
		return true;
	}
};

void C_DAPlayer::UpdateFlashlight()
{
	//Tony; keeping this variable for later.
	C_DAPlayer *pFlashlightPlayer = this;

	if ( !IsAlive() )
	{
		if ( GetObserverMode() == OBS_MODE_IN_EYE )
			pFlashlightPlayer = ToDAPlayer( GetObserverTarget() );
	...
	if ( this == C_BasePlayer::GetLocalPlayer() )
	{
		...
		if (GetActiveDAWeapon())
			pFlashlightPlayer->m_vecFlashlightOrigin = GetActiveDAWeapon()->GetShootPosition(this);
		else
...
//-----------------------------------------------------------------------------
void C_DAPlayer::TurnOffFlashlight( void )
{
...
float C_DAPlayer::GetUserInfoFloat(const char* pszCVar, float flBotDefault)
{
...
int C_DAPlayer::GetUserInfoInt(const char* pszCVar, int iBotDefault)
{
	ConVarRef UserInfoVar(pszCVar);
...
bool C_DAPlayer::UseVRHUD() const
{
...
{
	C_WeaponDABase* pWeapon = dynamic_cast<C_WeaponDABase*>(pEnt);
