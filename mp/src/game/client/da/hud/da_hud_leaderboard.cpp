...
public:
	...
	void         ListPlayer(C_DAPlayer *pPlayer, int iPosition);
	void         ListSpecialPlayer(C_DAPlayer* pPlayer, CHudTexture* pTexture, int iPosition);
	void         PaintPlayerAvatar(C_DAPlayer *pPlayer, float flX, float flY, float flW, float flH);
	virtual void Paint();
	virtual void PaintBackground() {};

protected:
	CHandle<C_DAPlayer> m_ahPlayerRanks[TOP_RANKS];
...
void InsertPlayer(CHandle<C_DAPlayer>* ahPlayerRanks, C_DAPlayer* pPlayer, int i)
{
	// Move them all down.
...
void CHudLeaderboard::OnThink()
{
	...
	CHandle<C_DAPlayer> ahPlayerRanks[TOP_RANKS];
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		for (int j = 0; j < TOP_RANKS; j++)
		{
			...
			{
				InsertPlayer(ahPlayerRanks, ToDAPlayer(UTIL_PlayerByIndex(i)), j);
				break;
			}
		}
	}
...
void CHudLeaderboard::ListPlayer(C_DAPlayer *pPlayer, int iPosition)
{
	...
	if (pPlayer->IsStyleSkillActive())
		surface()->DrawSetTextColor( Color(255, 190, 20, 255) );
	else
...
void CHudLeaderboard::PaintPlayerAvatar(C_DAPlayer *pPlayer, float flX, float flY, float flW, float flH)
{
	player_info_t pi;
...
void CHudLeaderboard::ListSpecialPlayer(C_DAPlayer* pPlayer, CHudTexture* pTexture, int iPosition)
{
	if (!pPlayer)
		return;
...
void CHudLeaderboard::Paint()
{
	...
	if (DAGameRules()->GetBountyPlayer())
	{
		...
		ListSpecialPlayer(DAGameRules()->GetBountyPlayer(), m_pBounty, 1);
		return;
	}
	else if (DAGameRules()->GetBriefcase() && ToDAPlayer(DAGameRules()->GetBriefcase()->GetOwnerEntity()))
	{
		wchar_t* pszGameMode = g_pVGuiLocalize->Find("#DA_LeaderBoard_CTB");
		if (pszGameMode)
			surface()->DrawUnicodeString( pszGameMode, vgui::FONT_DRAW_NONADDITIVE );

		ListSpecialPlayer(ToDAPlayer(DAGameRules()->GetBriefcase()->GetOwnerEntity()), m_pBriefcase, 1);
		return;
	}
	else if (DAGameRules()->GetLeader())
	{
		wchar_t* pszGameMode = g_pVGuiLocalize->Find("#DA_LeaderBoard_RatRace");
		if (pszGameMode)
			surface()->DrawUnicodeString( pszGameMode, vgui::FONT_DRAW_NONADDITIVE );

		ListSpecialPlayer(DAGameRules()->GetLeader(), m_pBounty, 1);
		ListSpecialPlayer(DAGameRules()->GetFrontRunner1(), m_pBounty, 2);
		ListSpecialPlayer(DAGameRules()->GetFrontRunner2(), m_pBounty, 3);
		return;
	}
	...
	for ( int i = 0; i < TOP_RANKS; i++ )
	{
		C_DAPlayer* pRanked = m_ahPlayerRanks[i];
		if (!pRanked)
			continue;
