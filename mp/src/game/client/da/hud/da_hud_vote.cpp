...
void CVoteSetupDialog::Activate()
{
	...
	m_pVoteSetupList->AddColumnToSection( 0, "Issue", "#DA_Vote_Column_Issue", SectionedListPanel::COLUMN_CENTER, m_iIssueWidth );
	...
	m_pVoteParameterList->AddColumnToSection( 0, "Name", "#DA_Vote_Column_Name", 0, m_iParameterWidth * 0.6 );
	m_pVoteParameterList->AddColumnToSection( 0, "Properties", "#DA_Vote_Column_Properties", SectionedListPanel::COLUMN_CENTER, m_iParameterWidth * 0.3 );
	...
	for ( int index = 0; index < m_VoteIssues.Count(); index++ )
	{
		...
		pKeyValues->SetString( "Issue", VarArgs("#DA_VoteIssue_Name_%s", pszIssue) );
...
void CVoteSetupDialog::OnCommand(const char *command)
{
	...
	if ( V_stricmp( command, "CallVote" ) == 0 )
	{
		...
		if ( iSelectedItem >= 0 )
		{
			...
			const char *szRealIssue = pIssueKeyValues->GetString( "Issue" );
			const char *szIssue = szRealIssue;

			if (V_strnicmp( "#DA_VoteIssue_Name_", szRealIssue, strlen("#DA_VoteIssue_Name_") ) == 0 )
				szIssue = szRealIssue + strlen("#DA_VoteIssue_Name_");
...
void CVoteSetupDialog::OnItemSelected( vgui::Panel *panel )
{
	if ( panel == m_pVoteSetupList )
	{
		...
		if ( iSelectedItem >= 0 )
		{
			...
			else if ( V_stristr( szName, "changelevel" ) || V_stristr( szName, "nextlevel" ) )
			{
				...
				{
					KeyValues *pKeyValues = new KeyValues( "Name" );
					pKeyValues->SetString( "Name", "#DA_Vote_no_maps" );
...
#ifdef TF_CLIENT_DLL
				SetDialogVariable( "combo_label", g_pVGuiLocalize->Find( "#DA_VoteKickReason" ) );
				m_pComboBox->AddItem( g_pVGuiLocalize->Find( "DA_VoteKickReason_Other" ), new KeyValues( "other" ) );
				m_pComboBox->AddItem( g_pVGuiLocalize->Find( "DA_VoteKickReason_Cheating" ), new KeyValues( "cheating" ) );
				m_pComboBox->AddItem( g_pVGuiLocalize->Find( "DA_VoteKickReason_Idle" ), new KeyValues( "idle" ) );
				m_pComboBox->AddItem( g_pVGuiLocalize->Find( "DA_VoteKickReason_Scamming" ), new KeyValues( "scamming" ) );
			...
			else if ( V_stricmp( "ChangeMission", szName ) == 0 )
			{
				...
				{
					KeyValues *pKeyValues = new KeyValues( "Name" );
					pKeyValues->SetString( "Name", "#DA_Vote_no_challenges" );
					pKeyValues->SetInt( "index", 1 );
					m_pVoteParameterList->AddItem( 0, pKeyValues );
					pKeyValues->deleteThis();
				}
...
void CHudVote::MsgFunc_CallVoteFailed( bf_read &msg )
{
	...
	if (ToDAPlayer(pLocalPlayer) && ToDAPlayer(pLocalPlayer)->GetInstructor())
		ToDAPlayer(pLocalPlayer)->GetInstructor()->HideLesson();
