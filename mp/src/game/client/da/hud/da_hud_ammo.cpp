...
void CHudAmmo::ShotFired(C_WeaponDABase* pWeapon, bool bAkimbo, bool bRight)
{
	UpdateAmmoDisplays();
...
void CHudAmmo::Reload(C_WeaponDABase* pWeapon)
{
	UpdateAmmoDisplays();
...
CHudTexture* CHudAmmo::GetTexture()
{
	...
	if (FStrEq(pPlayer->GetActiveDAWeapon()->GetSDKWpnData().szAmmo1, "9x19mm"))
		return m_p9mmRound;
	else if (FStrEq(pPlayer->GetActiveDAWeapon()->GetSDKWpnData().szAmmo1, "762x51mm"))
		return m_p762Round;
	else if (FStrEq(pPlayer->GetActiveDAWeapon()->GetSDKWpnData().szAmmo1, "45acp"))
		return m_p45acpRound;
	else if (FStrEq(pPlayer->GetActiveDAWeapon()->GetSDKWpnData().szAmmo1, "buckshot"))
...
Vector2D CHudAmmo::GetRoundPosition(int i)
{
	...
	int iMaxClip = pPlayer->GetActiveDAWeapon()->GetMaxClip1();
...
void CHudAmmo::Paint()
{
	...
	if (pszActivate && m_iAmmo == 0 && !pPlayer->GetActiveDAWeapon()->m_bInReload)
	{
#define WSTRLEN 512
