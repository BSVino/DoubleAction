...
bool WhoCaresConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	return true;
}

bool ValidPlayerConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!pPlayer)
		return false;
...
bool PlayerAliveConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
...
bool PlayerDeadConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!ValidPlayerConditions(pPlayer, pLesson))
		return false;
...
bool PlayerDeadNoKillCamConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
...
bool PlayerCanReloadConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;

	CWeaponDABase* pWeapon = pPlayer->GetActiveDAWeapon();
...
bool PlayerHasMultipleWeaponsConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
	...
	for (int i = 0; i < pPlayer->WeaponCount(); i++)
	{
		...
		CWeaponDABase* pWeapon = dynamic_cast<CWeaponDABase*>(pPlayer->GetWeapon(i));
		if (!pWeapon)
			continue;
...
bool PlayerOutOfAmmoAndMultipleWeaponsConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
...
bool PlayerActiveWeaponHasAimInConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
...
bool PlayerHasGrenadesConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
...
bool PlayerHasSlowMoConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
...
bool PlayerInThirdPersonConditions( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
...
bool PlayerSuperFalling( C_DAPlayer *pPlayer, class CLesson *pLesson )
{
	if (!PlayerAliveConditions(pPlayer, pLesson))
		return false;
...
{
	return C_DAPlayer::GetLocalDAPlayer()->GetInstructor()->GetBaseConditions(sCondition);
}
...
void C_DAPlayer::Instructor_Initialize()
{
	m_flLastLesson = -1;
...
void C_DAPlayer::Instructor_Respawn()
{
	if (!m_pInstructor)
		return;
...
void C_DAPlayer::Instructor_Reset()
{
...
typedef C_DAPlayer::CLessonProgress* LessonProgressPointer;
bool LessonPriorityCompare( const LessonProgressPointer& l, const LessonProgressPointer& r )
{
	//C_DAPlayer* pPlayer = Game()->GetLocalPlayer();
	...
	CLesson* pLessonL = C_DAPlayer::GetLocalDAPlayer()->GetInstructor()->GetLesson(l->m_sLessonName);
	CLesson* pLessonR = C_DAPlayer::GetLocalDAPlayer()->GetInstructor()->GetLesson(r->m_sLessonName);
...
void C_DAPlayer::Instructor_Think()
{
	if (!m_pInstructor)
		return;
...
void C_DAPlayer::Instructor_LessonLearned(const CUtlString& sLesson)
{
	if (!m_pInstructor)
		return;
...
void C_DAPlayer::Instructor_LessonShowed(const CUtlString& sLesson)
{
	if (!m_pInstructor)
		return;
...
bool C_DAPlayer::Instructor_IsLessonLearned(const CLessonProgress* pLessonProgress)
{
	if (!m_pInstructor)
		return false;
...
int C_DAPlayer::Instructor_GetLessonTrainings(const CUtlString& sLesson)
{
	if (!m_pInstructor)
		return -1;
...
bool C_DAPlayer::Instructor_IsLessonValid(const CLessonProgress* pLessonProgress)
{
	if (!m_pInstructor)
		return false;
...
C_DAPlayer::CLessonProgress* C_DAPlayer::Instructor_GetBestLesson()
{
...
bool CDAPlayer::LessonPriorityLess::Less( const LessonPointer& lhs, const LessonPointer& rhs, void *pCtx )
{
	C_DAPlayer* pPlayer = C_DAPlayer::GetLocalDAPlayer();
