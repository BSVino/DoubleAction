...
void CHudNotices::MsgFunc_Notice( bf_read &msg )
{
	...
	if (eNotice >= NOTICE_FIRST_TOPNOTICE)
	{
		if (iSubject > 0)
		{
			CDAPlayer* pDAPlayer = ToDAPlayer(UTIL_PlayerByIndex(iSubject));
			if (!pDAPlayer)
				return;

			g_pVGuiLocalize->ConvertANSIToUnicode( pDAPlayer->GetPlayerName(),  m_wszPlayerSubject, sizeof(m_wszPlayerSubject) );
		}
