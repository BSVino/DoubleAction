...
void CSDKScoreboard::UpdateTeamInfo()
{
	...
	{	
		if ( DAGameResources()->IsConnected( j ) )
		{
			iNumPlayersInGame++;
		}
	}
	...
	for ( int i = TEAM_SPECTATOR; i < TEAM_MAXCOUNT; i++ )
	{
		...
		if ( team )
		{
			...
			if ( i != TEAM_SPECTATOR && DAGameRules()->IsTeamplay() == false )
			{
...
#if defined ( SDK_USE_TEAMS )
		if ( !DAGameRules()->IsTeamplay() || teamNumber != TEAM_UNASSIGNED )
...
void CSDKScoreboard::UpdatePlayerInfo()
{
		...
		if ( shouldShow )
		{
			...
			if (pOther )
			{
				...
				if ( ot == localteam || localteam == TEAM_UNASSIGNED)
					kv->SetString("class", DAGameRules()->GetPlayerClassName( DAGameResources()->GetPlayerClass(i), ot ) );
				else
					kv->SetString("class", "");
			}
			else
	...
	if (pMostLabelPlayer)
	{
		C_DAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(sdkPR->GetHighestStylePlayer()));
		if (pPlayer)
	...
	if (pMostLabelPlayer)
	{
		C_DAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(sdkPR->GetHighestStuntKillPlayer()));
		if (pPlayer)
			pMostLabelPlayer->SetText(pPlayer->GetPlayerName());
		else
			pMostLabelPlayer->SetText("");
	...
	if (pMostLabelPlayer)
	{
		C_DAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(sdkPR->GetHighestBrawlKillPlayer()));
		if (pPlayer)
			pMostLabelPlayer->SetText(pPlayer->GetPlayerName());
		else
			pMostLabelPlayer->SetText("");
	...
	if (pMostLabelPlayer)
	{
		C_DAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(sdkPR->GetHighestKillStreakPlayer()));
		if (pPlayer)
			pMostLabelPlayer->SetText(pPlayer->GetPlayerName());
		else
			pMostLabelPlayer->SetText("");
	...
	if (pMostLabelPlayer)
	{
		C_DAPlayer* pPlayer = ToDAPlayer(UTIL_PlayerByIndex(sdkPR->GetHighestGrenadeKillPlayer()));
		if (pPlayer)
			pMostLabelPlayer->SetText(pPlayer->GetPlayerName());
		else
			pMostLabelPlayer->SetText("");
