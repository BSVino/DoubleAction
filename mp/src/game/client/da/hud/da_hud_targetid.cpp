...
void CSDKTargetId::Paint()
{
	...
	if (C_DAPlayer::GetLocalDAPlayer()->GetObserverMode() == OBS_MODE_FREEZECAM)
		return;
	...
	if (DAGameRules()->GetBriefcase() && DAGameRules()->GetBriefcase()->GetOwnerEntity())
	{
		C_BriefcaseCaptureZone* pBriefcase = DAGameRules()->GetCaptureZone();
	...
	if (m_pBriefcase && DAGameRules()->GetBriefcase())
	{
		C_Briefcase* pBriefcase = DAGameRules()->GetBriefcase();
		...
		if (DAGameRules()->IsTeamplay() && pBriefcase->GetOwnerEntity() && ToDAPlayer(pBriefcase->GetOwnerEntity())
			&& ToDAPlayer(pBriefcase->GetOwnerEntity())->GetTeamNumber() == C_DAPlayer::GetLocalDAPlayer()->GetTeamNumber())
	...
	if (DAGameRules()->GetBountyPlayer() && DAGameRules()->GetBountyPlayer() != C_DAPlayer::GetLocalDAPlayer())
	{
		C_DAPlayer* pBounty = DAGameRules()->GetBountyPlayer();
		...
		if (DAGameRules()->IsTeamplay() && pBounty->GetTeamNumber() == C_DAPlayer::GetLocalDAPlayer()->GetTeamNumber())
	...
	{
		if (C_DAPlayer::GetLocalDAPlayer()->GetRaceWaypoint() == 0)
		{
			m_Targets[TARGET_WAYPOINT1].m_vecLastKnownTarget = DAGameRules()->GetWaypoint(0)->WorldSpaceCenter();
			m_Targets[TARGET_WAYPOINT1].m_hEntity = DAGameRules()->GetWaypoint(0);
		...
		if (C_DAPlayer::GetLocalDAPlayer()->GetRaceWaypoint() == 1 || DAGameRules()->GetLeader() && DAGameRules()->GetLeader()->GetRaceWaypoint() == 1)
		{
			m_Targets[TARGET_WAYPOINT2].m_vecLastKnownTarget = DAGameRules()->GetWaypoint(1)->WorldSpaceCenter();
			m_Targets[TARGET_WAYPOINT2].m_hEntity = DAGameRules()->GetWaypoint(1);

			m_Targets[TARGET_WAYPOINT2].m_pTargetTexture = m_pCapturePoint;

			if (C_DAPlayer::GetLocalDAPlayer()->GetRaceWaypoint() == 1)
			{
		...
		if (C_DAPlayer::GetLocalDAPlayer()->GetRaceWaypoint() == 2 || DAGameRules()->GetLeader() && DAGameRules()->GetLeader()->GetRaceWaypoint() == 2)
		{
			m_Targets[TARGET_WAYPOINT3].m_vecLastKnownTarget = DAGameRules()->GetWaypoint(2)->WorldSpaceCenter();
			m_Targets[TARGET_WAYPOINT3].m_hEntity = DAGameRules()->GetWaypoint(1);

			m_Targets[TARGET_WAYPOINT3].m_pTargetTexture = m_pCapturePoint;

			if (C_DAPlayer::GetLocalDAPlayer()->GetRaceWaypoint() == 2)
			{
		...
		if (DAGameRules()->GetLeader() && DAGameRules()->GetLeader() != C_DAPlayer::GetLocalDAPlayer())
		{
			C_DAPlayer* pLeader = DAGameRules()->GetLeader();
		...
		if (DAGameRules()->GetFrontRunner1() && DAGameRules()->GetFrontRunner1() != C_DAPlayer::GetLocalDAPlayer())
		{
			C_DAPlayer* pLeader = DAGameRules()->GetFrontRunner1();
		...
		if (DAGameRules()->GetFrontRunner2() && DAGameRules()->GetFrontRunner2() != C_DAPlayer::GetLocalDAPlayer())
		{
			C_DAPlayer* pLeader = DAGameRules()->GetFrontRunner2();
	...
	for (int i = 0; i < TARGET_TOTAL; i++)
	{
		...
		if (GetVectorInHudSpace(m_Targets[i].m_vecLastKnownTarget, iX, iY))
		{
			...
			else if (pTarget)
			{
				...
				{
					trace_t tr;
					UTIL_TraceLine(CurrentViewOrigin(), m_Targets[i].m_vecLastKnownTarget, MASK_BLOCKLOS, C_DAPlayer::GetLocalDAPlayer(), COLLISION_GROUP_NONE, &tr);
...
#define MAX_ID_STRING 256
	...
	if ( iEntIndex )
	{
		...
		if ( IsPlayerIndex( iEntIndex ) )
		{
			...
			if ( DAGameRules()->IsTeamplay() == true && pPlayer->InSameTeam(pLocalPlayer) )
			{
