...
float ClientModeSDKNormal::GetViewModelFOV( void )
{
	...
	C_WeaponDABase *pWeapon = (C_WeaponDABase*)GetActiveWeapon();
	if ( pWeapon )
	{
...
void ClientModeSDKNormal::OverrideView( CViewSetup *pSetup )
{
	...
	C_BasePlayer *pPlayer = C_DAPlayer::GetLocalOrSpectatedPlayer();
	if(!pPlayer)
		return;
...
public:
	...
	virtual void Update(float frametime)
	{
		...
		if (!C_DAPlayer::GetLocalDAPlayer()->IsAlive())
			return;
