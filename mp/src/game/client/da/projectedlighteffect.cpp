...
void C_DAPlayer::GetFlashlightOffset( const Vector &vecForward, const Vector &vecRight, const Vector &vecUp, Vector *pVecOffset ) const
{
														  ...
														  Vector& vecFinalPos, Quaternion& quatOrientation, bool bTracePlayers )
{
	...
	C_DAPlayer *pPlayer = ToDAPlayer(UTIL_PlayerByIndex( m_nEntIndex ));
	if ( !pPlayer )
	{
		pPlayer = C_DAPlayer::GetLocalDAPlayer();
		if ( !pPlayer )
		{
			Assert( false );
			return false;
...
void CProjectedLightEffectManager::UpdateFlashlight( const Vector &vecPos, const Vector &vecDir, const Vector &vecRight, const Vector &vecUp, float flFov, bool castsShadows, float flFarZ, float flLinearAtten, const char* pTextureName)
{
	...
	C_DAPlayer *pPlayer = ToDAPlayer(UTIL_PlayerByIndex( m_nFlashlightEntIndex ));
